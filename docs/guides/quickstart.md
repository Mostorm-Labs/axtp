...
这份文档给研发两条最短可跑方案。当前跨 runtime 一致性验证的推荐基准是 `AXTP-TCP` Standard Framed + Node mock-server；`AXTP-WS-JSON` 保留为轻量 RPC-only 控制面路径。

1. 用 `AXTP-TCP` 接通 Standard Framed：transport connect -> `CONTROL OPEN / ACCEPT` -> Hello / Identify / Identified -> generated business RPC。
2. 用 `AXTP-WS-JSON` 接通 RPC-only 控制面：WebSocket open -> Hello / Identify / Identified -> Request / Response / Event。

适用场景：

- App、Web、Node、Python 或云端先接入 RPC-only 控制面。
...
没有真实设备时，先用 [axtp-mock-server](https://github.com/Mostorm-Labs/axtp-mock-server) 作为 Logical Server。当前生成的 Node mock-server 默认用于 TCP Standard Framed 互操作基准；WS JSON 只在 runtime 或 mock 明确声明 `websocket-jsonrpc` profile 时作为轻量控制面路径。mock server 应加载本仓库的 `protocol/axtp.protocol.yaml`、`docs/generated/protocol.json` 和 `docs/conformance/**`，让 SDK / runtime 先通过握手、请求、响应和事件验收。

## 0. 最短方案选型

| 方案 | 什么时候用 | 是否需要 Frame Header | 是否支持 STREAM | 最短接入建议 |
|---|---|---:|---:|---|
| `AXTP-TCP` | 跨 runtime / mock-server 一致性验证，或局域网设备直连 | 是 | 是 | 当前推荐基准 |
| `AXTP-WS-JSON` | App / Web / Node / Python 快速 RPC-only 接入 | 否 | 否 | 轻量控制面 |
...
```text
TCP Standard Framed 先 transport connect，再 CONTROL OPEN / ACCEPT；
ACCEPT 后 Logical Server 发送 Hello，客户端再 Identify；
Identified 分配 sid 后，业务 Request / Response / Event 才进入 generated protocol 验收。

WebSocket JSON 没有 CONTROL 和 STREAM，只保留 RPC-only 的 Hello / Identify / Request / Response / Event。
```
...
sequenceDiagram
    ...
    C->>T: 跑 docs/conformance/** 验收 framed-binary 行为
```

这条链路是当前 C++ / TypeScript / mock-server 互操作调试的基准。它验证 TCP byte stream 上的 Standard Frame 拆包、CONTROL gate、RPC session gate、`sid` 复用和 generated business RPC。
...
sequenceDiagram
    ...
    S-->>C: Hello op=0, sid=""
    C->>S: Identify op=2, sid="", randomSeed
...
| 发送 Identify | 新 session 的 `sid` 填 `""`，并携带 `randomSeed:uint32`；断线恢复用 `resumeSid`。 |
...
客户端不得在收到 Hello 前发送 Identify；Hello 是 `axtpVersion` 和认证要求的入口。`rpcVersion` 是早期 rc1 过渡兼容字段，新实现应省略。
...
以下只展示新 session、无认证、无事件订阅的最常用路径。断线恢复、认证响应和 `eventMasks` 编码完整规则见 [RPC Session Spec](../specs/1-core/06-RPC-Session.md)。
...
| op | 名称 | 方向 | 最小内容 |
|---:|---|---|---|
| 0 | Hello | Server -> Client | `{ "sid": "", "op": 0, "d": { "axtpVersion": "..." } }` |
| 2 | Identify | Client -> Server | `{ "sid": "", "op": 2, "d": { "randomSeed": 305419896 } }` |
| 3 | Identified | Server -> Client | `{ "sid": "12345678", "op": 3, "d": {} }` |
...
Phase 1 MVP 只需要实现 `status: mvp` 的 op。`reserved` op 按保留编号处理，不应当作业务能力；未实现的 `draft` op 可以返回 `RPC_OP_INVALID` 或 `NOT_SUPPORTED`。

## 5. TCP / HID Standard Framed 包内容

TCP / USB HID 走 Standard Frame；`AXTP-WS-JSON` 没有 Frame Header：
...
| 检查项 | 通过标准 |
|---|---|
| 能建立 transport | TCP / USB HID 能完成 `CONTROL OPEN / ACCEPT`；WS JSON 能打开 WebSocket。 |
| 能建立 session | 客户端发送带 `randomSeed:uint32` 的 `Identify op=2` 后收到 `Identified op=3` 和 `sid`。 |
...
| 目标 | 文档 |
|---|---|
| 理解协议维护工作流 | [protocol-maintainer-sop.md](protocol-maintainer-sop.md) |
