...
没有真实设备时，先用 [axtp-mock-server](https://github.com/Mostorm-Labs/axtp-mock-server) 作为 Logical Server 跑通 `AXTP-WS-JSON`。mock server 应加载本仓库的 `protocol/axtp.protocol.yaml`、`docs/generated/protocol.json` 和 `docs/conformance/**`，让 SDK / runtime 先通过握手、请求、响应和事件验收。
...
```text
WebSocket 打开后，设备或 mock server 作为 Logical Server 先发 Hello；
客户端必须等待 Hello 后再发送 Identify；
