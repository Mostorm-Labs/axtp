...
| 设备名称编辑 | 修改设备显示名。 | Legacy `SetDeviceName` → draft `software.config` v0.6（target: `"launcher"`，`displayName` 字段）。 |
...
| 播放列表管理 | 服务端全量同步播放列表，设备读取当前列表，资源 URL 即将过期时设备请求刷新。 | Legacy `SetPlaylistConfig` / `GetPlaylistConfig` / `GetPlaylistItemUrl` → draft `signage.playlist` v1.2。废弃 `slideshow` 类型，Adapter 映射为 `image`。 |
| 外观设置 | 管理 `panelLayout`、`autoHidePanel`、`autoHideDelay`。 | Legacy `GetAppearanceConfig` / `SetAppearanceConfig` → draft `software.config` v0.6（target: `"launcher"`，`appearance` 子对象）。 |
| 更新策略设置 | 管理自动更新开关、时间窗口和通道。 | Legacy `GetUpdateConfig` / `SetUpdateConfig` → draft `software.updatePolicy` v0.7（target: `"launcher"`，`updateMode` 枚举 + `schedule` + `channel` + `conditions`）。 |
...
| 修改设备名 | Drafted only | `software.config`（target: `"launcher"`） | `docs/protocol/software/software.config.md` v0.6（含 `displayName` 字段） | `displayName` 写入路径已归入 `software.config`，`device.info` 只读返回。`[REVIEW-RESOLVED]` 归属已确认（`software.config` v0.6 写入 + `device.info` 只读），转 Stage 30 采纳。 |
...
| 播放列表能力查询 | Drafted only | `signage.playlist` | `docs/protocol/signage/signage.playlist.md` v1.2 | `signage.getPlaylistCapabilities`；转 Stage 30 采纳。 |
| 播放列表全量同步 | Drafted only | `signage.playlist` | `docs/protocol/signage/signage.playlist.md` v1.2 | playlists/items/settings schema 已完成；全量替换语义已确认（硬替换）。转 Stage 30 采纳。 |
| 播放列表查询 | Drafted only | `signage.playlist` | `docs/protocol/signage/signage.playlist.md` v1.2 | set/get 结构一致。转 Stage 30 采纳。 |
| 播放项 URL 刷新 | Drafted only | `signage.playlist` | `docs/protocol/signage/signage.playlist.md` v1.2 | URL refresh 重构为 `type` + `settings` 显式类型判别模式。转 Stage 30 采纳。 |
| 外观/面板配置 | Drafted only | `software.config`（target: `"launcher"`） | `docs/protocol/software/software.config.md` v0.6 | target 枚举列出 `"launcher"`/`"signagePlayer"`/`"agent"`；外观字段嵌套为 `appearance` 子对象。`[REVIEW-ASK]` target 完整枚举确认后转 Stage 30 采纳。 |
| 软件更新策略 | Drafted only | `software.updatePolicy`（target: `"launcher"`） | `docs/protocol/software/software.updatePolicy.md` v0.7 | `updateMode` 枚举已定义（`auto`/`manual`/`notify`）；跨午夜 `schedule` 语义草案候选中（`end < start` 表示跨日），采纳前确认 `[REVIEW-ASK]`。转 Stage 30 采纳。 |
...
sequenceDiagram
    ...
    rect rgb(255, 248, 230)
        ...
        Cloud->>Device: GetPlaylistCapabilities (draft signage.playlist v1.2)
        Device-->>Cloud: 支持的播放项类型、数量限制
        Cloud->>Device: GetPlaylistConfig (draft signage.playlist v1.2)
        Device-->>Cloud: 当前播放列表
        Cloud->>Device: software.getConfig(target: "launcher") (draft software.config v0.6)
        Device-->>Cloud: displayName, panelLayout, autoHidePanel, autoHideDelay
        Cloud->>Device: software.getUpdatePolicy(target: "launcher") (draft software.updatePolicy v0.7)
...
| 6 | P2 | Cloud / Device | 查询播放列表能力。 | Draft `signage.playlist` v1.2 | `signage.getPlaylistCapabilities`；请求为空。 | 返回支持的播放项类型、数量限制、功能开关。 | 设备不支持时返回 `NOT_SUPPORTED`。 |
| 7 | P2 | Cloud / Device | 查询播放列表配置。 | Draft `signage.playlist` v1.2 | `signage.getPlaylistConfig`；请求为空。 | 返回当前完整 playlist config。 | 保持 set/get 结构一致。 |
| 8 | P2 | Cloud / Device | 查询外观配置。 | Draft `software.config` v0.6（target: `"launcher"`） | `software.getConfig(target: "launcher")`。 | displayName, appearance: { panelLayout, autoHidePanel, autoHideDelay }。 | target 枚举列出 `"launcher"`/`"signagePlayer"`/`"agent"`。 |
| 9 | P2 | Cloud / Device | 查询更新策略。 | Draft `software.updatePolicy` v0.7（target: `"launcher"`） | `software.getUpdatePolicy(target: "launcher")`。 | updateMode（`auto`/`manual`/`notify`）, schedule（含 timezone）, channel, conditions。 | `updateMode` 枚举已定义；`schedule.end < schedule.start` 表示跨日。 |
...
| 16 | P4 | User / Console / Cloud / Device | 修改设备名称。 | Draft `software.config` v0.6（target: `"launcher"`） | `software.setConfig(target: "launcher", config: { displayName })`。字段映射：`devName` → `displayName`。 | 返回标准成功响应（无 result body）；触发 `software.configChanged` 事件，通过 `getConfig` 确认。 | 写入路径在 `software.config`，`device.info` 只读返回。 |
| 17 | P4 | User / Console / Cloud / Device | 全量同步播放列表。 | Draft `signage.playlist` v1.2 | `signage.setPlaylistConfig`：`playlists[]`，含日期/时间/星期、items、settings。硬替换语义。 | 播放器替换当前配置；触发 `signage.playlistConfigChanged` 事件。 | 第二次全量下发删除缺失项；废弃 `slideshow` 类型由 Adapter 映射为 `image`。 |
| 18 | P4 | Device / Cloud | 刷新播放项资源 URL。 | Draft `signage.playlist` v1.2 | `signage.getPlaylistItemUrl`：`{ itemId }`；返回 `type` + `settings` 显式类型判别。 | 设备获得新资源 URL（含 expiresAt）；`clock` 类型返回 `NOT_SUPPORTED`。 | URL refresh 重构为 type+settings 模式。 |
| 19 | P4 | User / Console / Cloud / Device | 设置外观配置。 | Draft `software.config` v0.6（target: `"launcher"`） | `software.setConfig(target: "launcher", config: { appearance: { panelLayout, autoHidePanel, autoHideDelay } })`。partial update 语义。 | 返回标准成功响应（无 result body）；触发 `software.configChanged` 事件，通过 `getConfig` 确认。 | 外观字段包裹在 `appearance` 子对象中。 |
...
| 21 | P4 | User / Console / Cloud / Device | 设置软件更新策略。 | Draft `software.updatePolicy` v0.7（target: `"launcher"`） | `software.setUpdatePolicy(target: "launcher", policy: { updateMode, schedule, channel, conditions })`。partial update 语义。 | 返回标准成功响应（无 result body）；触发 `software.updatePolicyChanged` 事件，通过 `getUpdatePolicy` 确认。 | `updateMode` 枚举（`auto`/`manual`/`notify`）；跨午夜 `schedule` 语义草案候选中，采纳前确认 `[REVIEW-ASK]`。 |
| 22 | P4 | User / Console / Cloud / Device | 恢复默认播放列表。 | Draft `signage.playlist` v1.2 | `signage.resetPlaylistConfig`；请求为空。 | 返回重置后的 `PlaylistConfigResult`。 | 不影响系统配置。 |
| 23 | P4 | User / Console / Cloud / Device | 恢复默认软件配置。 | Draft `software.config` v0.6 | `software.resetConfig(target: "launcher")`；请求含 target。 | 返回重置后的完整 `SoftwareConfig`。 | 仅恢复软件配置，不触发设备重启；legacy `ResetConfig` 映射到 `system.restoreFactorySettings`。 |
| 24 | P4 | User / Console / Cloud / Device | 恢复默认更新策略。 | Draft `software.updatePolicy` v0.7 | `software.resetUpdatePolicy(target: "launcher")`；请求含 target。 | 返回重置后的完整 `SoftwareUpdatePolicy`。 | 不影响固件更新策略。 |
...
| 播放列表配置变更 | `signage.setPlaylistConfig` 或 `signage.resetPlaylistConfig` 成功 | `signage.playlistConfigChanged` | `PlaylistConfigChangedEvent`（payload schema，见 `signage.playlist` v1.2 §4.1：`reason` + 可选 `playlists`） | Cloud 更新播放列表状态；设备端播放器应用新配置 | draft |
...
| `signage.playlist` | `SetPlaylistConfig`, `GetPlaylistConfig` | `signage.getPlaylistCapabilities` / `signage.getPlaylistConfig` / `signage.setPlaylistConfig` / `signage.resetPlaylistConfig` / `signage.getPlaylistItemUrl` / `signage.playlistConfigChanged` | `docs/protocol/signage/signage.playlist.md` v1.2 |
| `software.config`（target: `"launcher"`） | `GetAppearanceConfig`, `SetAppearanceConfig`, `SetDeviceName` | `software.getConfig` / `software.setConfig` / `software.resetConfig` / `software.configChanged` | `docs/protocol/software/software.config.md` v0.6 |
| `software.updatePolicy`（target: `"launcher"`） | `GetUpdateConfig`, `SetUpdateConfig` | `software.getUpdatePolicy` / `software.setUpdatePolicy` / `software.resetUpdatePolicy` / `software.updatePolicyChanged` | `docs/protocol/software/software.updatePolicy.md` v0.7 |
...
| `SetDeviceName` | Server -> Device, Device -> Server | `software.setConfig(target: "launcher", config: { displayName })` | Drafted only | 草案 `software.config` v0.6 已含 `displayName` 字段；`device.info` 只读返回。 |
...
| `SetPlaylistConfig` | Server -> Device | `signage.setPlaylistConfig` | Drafted only | 草案 `signage.playlist` v1.2；全量替换 schema 已完成；硬替换语义已确认。 |
| `GetPlaylistConfig` | Server <-> Device | `signage.getPlaylistConfig` | Drafted only | 草案 `signage.playlist` v1.2；set/get 结构一致。 |
| `GetPlaylistItemUrl` | Device -> Server | `signage.getPlaylistItemUrl` | Drafted only | 草案 `signage.playlist` v1.2；重构为 `type` + `settings` 显式类型判别模式；`clock` 类型返回 `NOT_SUPPORTED`。 |
| `GetAppearanceConfig` | Server <-> Device | `software.getConfig(target: "launcher")` | Drafted only | 草案 `software.config` v0.6；字段嵌套为 `config.appearance.*`。 |
| `SetAppearanceConfig` | Server <-> Device | `software.setConfig(target: "launcher")` | Drafted only | 草案 `software.config` v0.6；旧 flat 字段需 adapter 包装为 `config.appearance.*`。 |
| `GetUpdateConfig` | Server <-> Device | `software.getUpdatePolicy(target: "launcher")` | Drafted only | 草案 `software.updatePolicy` v0.7；旧 `autoUpdate`(bool) → `updateMode`(enum: `auto`/`manual`/`notify`)。 |
| `SetUpdateConfig` | Server <-> Device | `software.setUpdatePolicy(target: "launcher")` | Drafted only | 草案 `software.updatePolicy` v0.7；旧 `true` → `"auto"`，`false` → `"manual"`。新增 `timezone` 和 `conditions` 字段。 |
...
|---|---|---|---|---|
| `SetDeviceName` 无对应 AXTP 草案 | `software.config` v0.6（target: `"launcher"`） | `software.setConfig(target: "launcher", config: { displayName })` | `adopt-protocol-draft` | `[REVIEW-RESOLVED]` 设备名写入路径归入 `software.config` 的 `displayName` 字段；`device.info` 只读返回。 |
...
| 播放项 URL 刷新已定域 | `signage.playlist` v1.2 | `signage.getPlaylistItemUrl` | `adopt-protocol-draft` | `[REVIEW-RESOLVED]` URL 刷新是播放项级操作，已归属 `signage.playlist`；`signage.media` 草案已删除，功能已合并。 |
| Launcher 外观/面板配置 | `software.config` v0.6（target: `"launcher"`） | `software.getConfig` / `software.setConfig` / `software.resetConfig` / `software.configChanged` | `adopt-protocol-draft` | `[REVIEW-RESOLVED]` 旧草案 `device.appearance` 已删除；外观配置实为 Launcher 软件配置，迁入 `software.config` 统一管理。 |
| 软件更新策略 | `software.updatePolicy` v0.7（target: `"launcher"`） | `software.getUpdatePolicy` / `software.setUpdatePolicy` / `software.resetUpdatePolicy` / `software.updatePolicyChanged` | `adopt-protocol-draft` | `[REVIEW-RESOLVED]` 旧草案 `firmware.updatePolicy` 已回退为骨架；更新策略迁入 `software.updatePolicy`，通过 target 区分组件。 |
...
| `SetDeviceName` 应归属 `device.info` 还是 `software.config`？ | protocol | 架构 | [REVIEW-RESOLVED] | `software.config` v0.6 已明确：`displayName` 在 software.config 写入，device.info 只读。采纳前最终确认。 |
