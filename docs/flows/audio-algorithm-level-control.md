...
|---|---|---|---|---|
| 确认设备是否支持音频算法配置 | generated | `audio.getAlgorithmCapabilities`, `audio.algorithm` capability | `docs/generated/protocol.md`, `registry/domains/audio/domain.yaml` | App 实现能力门禁。 |
| 查询可展示的算法和 slider 范围 | generated | `audio.getAlgorithmCapabilities` | `docs/generated/protocol.md` | App 按返回 descriptor 渲染 UI。 |
| 查询当前等级值 | generated | `audio.getAlgorithmConfig` | `docs/generated/protocol.md` | App 初始化页面。 |
| 调整单个或多个等级值 | generated | `audio.setAlgorithmConfig` | `docs/generated/protocol.md` | App 在松手、防抖或保存时提交 partial config。 |
| 恢复默认等级 | generated | `audio.resetAlgorithmConfig` | `docs/generated/protocol.md` | App 传入本页 slider 字段 selector。 |
| 多端或设备策略改变配置后同步 UI | generated | `audio.algorithmConfigChanged` | `docs/generated/protocol.md` | App 处理 event 并刷新展示。 |
| UI 需要新增算法对象或新字段 | adopted | `audio.algorithm` schema extension | `registry/domains/audio/domain.yaml`, `docs/protocol/audio/audio.algorithm.md` | 转 `amend-adopted-protocol`。 |
