...
- `AXTP-Core-Framing-Verification-Design-v0.1.md` — **A1 / P20 Verification Design** mapping A1-D01..A1-D10 into raw-frame corpus, state oracle, symbolic virtual time, frame-conformance DSL, and evidence gates. It is evidence design, not protocol authority, and does not modify runtime requirements until later materialization/promotion.
...
Authority status, verification-design status, conformance-promotion status, and publication status are intentionally separated. A runtime or SDK continues to bind to its exact published spec tag/commit/release artifact until it explicitly adopts a future publication that contains A1.
