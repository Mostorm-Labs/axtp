...
2. protocol/axtp.protocol.yaml
3. docs/generated/protocol.md and docs/generated/protocol.json
4. docs/conformance/**
5. docs/specs/**
```
...
- `protocol/axtp.protocol.yaml` 是 generated Protocol IR，适合工具、runtime、mock server 消费。
- `docs/generated/protocol.md` 和 `docs/generated/protocol.json` 是当前 generated reference。
- `docs/conformance/**` 是 runtime 行为验收输入。
- `docs/specs/**` 是人工维护的正式规范，用来解释 wire、session、registry、codec、tooling 和 versioning 规则。

Runtime MUST NOT implement draft-only `docs/protocol/**` entries unless explicitly building a mock/prototype.
...
- `docs/specs/**` 维护规则和治理约束。
- `registry/**/*.yaml` 与 `registry/domains/**/*.yaml` 是手写机器事实源。
- `generators/**` 定义如何从事实源生成 Protocol IR、generated reference、snapshots 和测试向量。
- `protocol/axtp.protocol.yaml` 与 `docs/generated/**` 是输出，不是手写输入。
...
| 冲突 | 判断方式 | 处理 |
|---|---|---|
| generated 与 draft 冲突 | `docs/generated/**` 或 Protocol IR 与 `docs/protocol/**` 不一致。 | runtime 相信 generated；维护者回到草案或采纳流程修正。 |
| generated 与 specs/YAML 冲突 | generated 输出不能反映 specs 或 registry YAML。 | 维护者修正 specs、YAML 或 Generator 后重新生成。 |
| conformance 与 specs/generated 冲突 | case 要求和正式合同不一致。 | 修 `docs/conformance/**` 或 specs/YAML，不让 runtime 绕过。 |
