...
The audit scanned every Markdown file under `docs/specs`, reviewed section structure and keyword concentration, and checked the specific overlap/mixing concerns requested by the repository maintainer. The review focused on document responsibility and source-of-truth boundaries, not protocol correctness.
...
The registry documents often define both the meta-model and current full ID tables. Once YAML and `docs/generated` exist, complete current tables in specs become stale-risk duplicates.
...
Recommendation: formal specs should contain only compatibility invariants that protect the protocol. Mapping from old CmdValue, old JSON-RPC, old firmware/file/log protocols, adapter architecture, and migration phases should live under `docs/legacy-migration`.
...
Recommended decision: keep field rules and state machine in the main body; replace long scenarios with links to `docs/guides`, `docs/legacy-migration`, and `docs/generated`.
...
- `docs/generated` owns: current rendered tables.
- `docs/protocol` owns: draft/planning candidate tables before adoption.
- `docs/legacy-migration` owns: old protocol mapping evidence.
...
- Move capability model, capability IDs, masks, and discovery semantics into a registry/capability spec, for example `2-registry/06-Capabilities-Registry.md`.
...
Recommended decision: split the formal versioning spec from the legacy migration manual. The formal spec should stay under `docs/specs`; the migration manual should move to `docs/legacy-migration`.
...
7. Update `docs/specs/README.md` to mark which files are normative and which are supplemental.
...
### 6.5 Content to Move to `docs/legacy-migration`
...
### 6.6 Content Owned by `docs/generated`
...
### 6.7 Content to Move to `docs/dev/skills`
...
| Phase 2 | Remove duplicate generated facts | Replace complete current registry tables with links to `docs/generated` references. |
| Phase 3 | Separate legacy migration | Extract legacy-heavy sections from core/registry/codec/tooling specs into `docs/legacy-migration`. |
...
Do not start by moving files. First make the current `docs/specs` tree self-declaring:
