...
> Source inputs: `docs/workspace/business/cast-reciever-uxplay.md`, `docs/workspace/legacy-migration/evidence/WEBSOCKET_PROTOCOL.md`, `contract/generated/protocol.md`, `docs/workspace/protocol/auth/auth.session.md`, `docs/workspace/protocol/auth/auth.token.md`, `docs/workspace/protocol/auth/auth.permission.md`
...
本文不是最终协议事实源。已采纳事实以 `contract/registry/**/*.yaml`、`contract/registry/domains/**/*.yaml` 和 `contract/generated/**` 为准；新增或修改协议必须进入 `docs/workspace/protocol/**` 草案，并经过后续采纳和生成流程。
...
| 是否需要新增协议草案 | yes |
| 是否涉及 Legacy | yes，`docs/workspace/legacy-migration/evidence/WEBSOCKET_PROTOCOL.md` 是主要旧协议证据。 |
...
| Non-goal | 本文不修改 `docs/workspace/protocol/**`、`contract/registry/**`、`contract/protocol/axtp.protocol.yaml`、`contract/generated/**`。 | `[REVIEW-OK]` |
...
| 外部控制口 token、权限和 LAN 安全策略 | draft | `Identify.d.authentication`, draft `auth.session` / `auth.token` / `auth.permission` | `docs/workspace/protocol/auth/auth.session.md`, `docs/workspace/protocol/auth/auth.token.md`, `docs/workspace/protocol/auth/auth.permission.md` | Stage 20 细化本场景 auth 策略。 |
| Launcher 启动 runtime | local-only | App process orchestration | `docs/workspace/business/cast-reciever-uxplay.md` | UI/runtime 实现，不进 AXTP。 |
| Electron 启动 UxPlay backend | non-protocol | Backend adapter to `ws://127.0.0.1:7001/` | `docs/workspace/legacy-migration/evidence/WEBSOCKET_PROTOCOL.md` | 只作为 adapter 实现。 |
...
| UxPlay 内部控制口 `ws://127.0.0.1:7001/` | non-protocol | Adapter implementation detail | `docs/workspace/legacy-migration/evidence/WEBSOCKET_PROTOCOL.md` | 不进入公共 AXTP 协议。 |
| mDNS 发现和 AirPlay 媒体传输 | non-protocol | AirPlay/UxPlay implementation detail | `docs/workspace/business/cast-reciever-uxplay.md` | 不进入 AXTP cast 控制协议。 |
