...
> Source inputs: `docs/workspace/business/video-framing-modes.md`, `docs/workspace/protocol/video/video.framing.md`, `docs/workspace/protocol/camera/camera.ptz.md`, `docs/workspace/legacy-migration/classification/video.md`, `contract/generated/protocol.md`
...
本文不是最终协议事实源。当前 generated 协议没有 `video.framing` 的业务 method / event；相关能力仍处于 `docs/workspace/protocol/video/video.framing.md` 草案阶段。Flow 文档只负责描述场景、覆盖状态和协议缺口，不定义完整 method / event / schema。
...
| 下一步 | draft protocol；修订 `docs/workspace/protocol/video/video.framing.md` 后进入 adoption。 |
...
| 查询 framing 能力 | draft | `video.getFramingCapabilities` | `docs/workspace/protocol/video/video.framing.md` | 补齐 modes、variants、tracking carriers、config ranges。 |
| 查询当前 mode/config/state | draft | `video.getFramingMode`, `video.getFramingConfig`, `video.getFramingState` | `docs/workspace/protocol/video/video.framing.md` | 统一是否保留 mode 独立方法或收敛到 config/state。 |
| 设置 framing mode | draft | `video.setFramingMode` / `video.setFramingConfig` | `docs/workspace/protocol/video/video.framing.md` | 增加 mode enum 和 apply state。 |
| 配置区域追踪 | draft | `video.setFramingConfig` | `docs/workspace/protocol/video/video.framing.md` | 补 `trackingRegion`, `trackingCarrier`, horizontal/vertical strategy。 |
...
| speaker tracking 参数 | draft | `video.setFramingConfig`; possible audio dependency | `docs/workspace/protocol/video/video.framing.md`, legacy classification | 补 delay/strategy/dependency/unavailable reason。 |
| framing 状态变化事件 | draft | `video.framingConfigChanged`, `video.framingModeChanged`, `video.framingStateChanged` | `docs/workspace/protocol/video/video.framing.md`, appendix event candidates | 确认事件命名和 payload 是完整状态还是 patch。 |
| 物理 PTZ 被算法接管 | draft | `camera.ptzStateChanged` plus framing state | `docs/workspace/protocol/camera/camera.ptz.md` | 明确手动 PTZ 与 framing 算法的冲突策略。 |

Coverage 取值：`generated` 表示已进入 generated；`draft` 表示已有 `docs/workspace/protocol/**` 草案；`missing` 表示还没有合适草案；`local-only` 表示 App 本地逻辑。
