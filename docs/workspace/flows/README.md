...
`docs/workspace/flows/` 存放场景级协议交互方案，是 Stage 10 `plan-protocol-flow` 的默认输出目录。

如果同名或相近主题已经有 `docs/workspace/business/**`，business 文档只作为 PRD 源头；交互步骤、actor 责任、协议覆盖、缺口和下一步路由由 flow 文档维护。
...
`docs/workspace/flows/**` 不是最终协议事实源。稳定实现合同仍然来自：
...
| 输入 | Stage 10 做什么 | 后续 |
|---|---|---|
| UI 原型，例如音频算法强度滑条和恢复默认按钮 | 识别屏幕控件、读写流程、默认值、错误路径和已有协议覆盖 | 写 `docs/workspace/flows/<scenario>.md`，缺口转 20/40 |
| 用户 story，例如“用户在 App 中切换输出布局” | 拆 actor、步骤、请求、响应、事件和状态同步 | 判断复用已有协议还是新增草案 |
| 旧协议迁移场景 | 把旧命令映射到 story 步骤，区分兼容保留和 AXTP 新增能力 | 缺口转 `docs/workspace/protocol/**` 草案 |
| 跨端业务流程 | 标出 App、后台、设备、SDK、测试各自消费的协议事实 | 给研发和测试形成共同流程图 |

## 工作流

```text
docs/workspace/business/<requirement>.md 或业务场景 / UI 原型 / 用户 story
