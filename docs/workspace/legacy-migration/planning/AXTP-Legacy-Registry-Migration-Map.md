...
|---|---|
| Affects binary wire format | `specs/02`, `04`, `05`, `06`, `07` |
...
| Complete Capability Model | Not carried forward as a current v1 input; restart through `docs/workspace/protocol/` RFC if needed |
...
| Section | Disposition |
|---|---|
| Registry positioning, single source of truth | Keep as governance in `08`; source of truth is now `contract/registry/**/*.yaml` and `contract/registry/domains/**/*.yaml` |
...
| Complete capability tree and query methods | Not carried forward as a current v1 input; restart through `docs/workspace/protocol/` RFC if needed |
