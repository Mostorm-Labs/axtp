...
> Source evidence: `docs/workspace/legacy-migration/evidence/NearHub-Launcher数字标牌设备管理通用管理命令.md`, `docs/workspace/legacy-migration/evidence/NearHub-Launcher设备管理命令.md`
> Classification reference: `docs/workspace/legacy-migration/classification/by-source/signage_sdk.md`, `docs/workspace/legacy-migration/classification/signage.md`
> AXTP draft inputs: `docs/workspace/protocol/signage/**` plus the common domain drafts referenced below
...
| 协议事实源 | `docs/workspace/protocol/**` 是草案输入，`contract/registry/domains/**` 和 `contract/registry/**/*.yaml` 是手写机器事实源，`contract/protocol/axtp.protocol.yaml` 是 Generator 输出。 |

## 2. 非目标

- 不修改 `contract/protocol/axtp.protocol.yaml`。
- 不修改 `contract/registry/**/*.yaml`、`contract/registry/domains/**/*.yaml`。
- 不手写 `contract/generated/**`、`docs/workspace/legacy-migration/generated/**` 或 runtime generated headers。
- 不把 `docs/workspace/legacy-migration/generated/registry-patches.generated.yaml` 直接当作稳定协议事实源。
...
| 状态 | 能力 | 说明 |
|---|---|---|
| 已在当前 contract/registry/generated 中出现 | `device.info`, `firmware.update` | 可复用现有正式事实，但 signage 的字段映射仍需检查是否完全覆盖旧 SDK 返回。 |
...
当前 `docs/workspace/protocol/signage/**` 已给出 `signage.playlist`、`signage.media`、`signage.schedule`、`signage.osd`、`signage.playback` 的 feature 边界和候选方法，但大多仍是占位型草案。落地前必须把旧数字标牌 payload 的字段、枚举、默认值、方向、错误语义和 legacyRefs 补进去，再进入 YAML 采纳。
...
按覆盖矩阵补齐或重写相关 `docs/workspace/protocol/**`：

| 优先级 | 草案 |
|---|---|
| P0 signage | `docs/workspace/protocol/signage/signage.playlist.md`, `docs/workspace/protocol/signage/signage.media.md`, `docs/workspace/protocol/signage/signage.osd.md` |
...
```text
docs/workspace/protocol/** review
  -> contract/registry/domains/<domain>/domain.yaml or contract/registry/**/*.yaml
...
- signage 专属能力写入 `contract/registry/domains/signage/domain.yaml`。
...
- 采纳 YAML 后运行 `pnpm --dir tooling/generators --config.verify-deps-before-run=false build`。
- 采纳 YAML 后运行 `pnpm --dir tooling/generators --config.verify-deps-before-run=false test`。
- 检查 `contract/protocol/axtp.protocol.yaml` 和 `contract/generated/protocol.md` 中 method/schema/capability 与草案一致。
...
- `docs/workspace/protocol/signage/**` 与相关 common 草案的字段覆盖旧 payload。
