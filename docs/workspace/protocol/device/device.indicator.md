...
`device.indicator` 用于设备指示灯、LED 模式和指示状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | device.indicator 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 device.indicator 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 device.indicator 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `device.getIndicatorCapabilities` | query | 查询 device.indicator 能力范围 | `GetIndicatorCapabilitiesParams` | `GetIndicatorCapabilitiesResult` | 否 | draft |
| `device.getIndicatorConfig` | query | 查询 device.indicator 配置 | `GetIndicatorConfigParams` | `GetIndicatorConfigResult` | 否 | draft |
| `device.setIndicatorConfig` | command | 设置 device.indicator 配置 | `SetIndicatorConfigParams` | `SetIndicatorConfigResult` | 是，`device.indicatorConfigChanged` | draft |
| `device.resetIndicatorConfig` | action | 恢复 device.indicator 默认配置 | `ResetIndicatorConfigParams` | `ResetIndicatorConfigResult` | 是，`device.indicatorConfigChanged` | draft |

### 3.1 `device.getIndicatorCapabilities`

**用途**：查询 device.indicator 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetIndicatorCapabilitiesParams` |
| Result Schema | `GetIndicatorCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetIndicatorCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetIndicatorCapabilitiesResult`
...
### 3.2 `device.getIndicatorConfig`

**用途**：查询 device.indicator 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetIndicatorConfigParams` |
| Result Schema | `GetIndicatorConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetIndicatorConfigParams`
...
#### 3.2.3 返回结果 Result：`GetIndicatorConfigResult`
...
### 3.3 `device.setIndicatorConfig`

**用途**：设置 device.indicator 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetIndicatorConfigParams` |
| Result Schema | `SetIndicatorConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `device.indicatorConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetIndicatorConfigParams`
...
#### 3.3.3 返回结果 Result：`SetIndicatorConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `device.indicatorConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `IndicatorConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "device.indicatorConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `device.resetIndicatorConfig`

**用途**：恢复 device.indicator 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetIndicatorConfigParams` |
| Result Schema | `ResetIndicatorConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `device.indicatorConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetIndicatorConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetIndicatorConfigResult`
...
|---|---|---|---|---|
| `device.indicatorConfigChanged` | device.indicator 配置变化 | `IndicatorConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `device.indicatorConfigChanged`

**触发条件**：device.indicator 配置变化。

#### 4.1.1 Payload：`IndicatorConfigChangedEvent`
...
Capability name: `device.indicator`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `device.indicator` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetIndicatorCapabilitiesParams` | `device.getIndicatorCapabilities` request params | 见 `device.getIndicatorCapabilities` 方法小节。 |
| `GetIndicatorCapabilitiesResult` | `device.getIndicatorCapabilities` result | 见 `device.getIndicatorCapabilities` 方法小节。 |
| `GetIndicatorConfigParams` | `device.getIndicatorConfig` request params | 见 `device.getIndicatorConfig` 方法小节。 |
| `GetIndicatorConfigResult` | `device.getIndicatorConfig` result | 见 `device.getIndicatorConfig` 方法小节。 |
| `SetIndicatorConfigParams` | `device.setIndicatorConfig` request params | 见 `device.setIndicatorConfig` 方法小节。 |
| `SetIndicatorConfigResult` | `device.setIndicatorConfig` result | 见 `device.setIndicatorConfig` 方法小节。 |
| `ResetIndicatorConfigParams` | `device.resetIndicatorConfig` request params | 见 `device.resetIndicatorConfig` 方法小节。 |
| `ResetIndicatorConfigResult` | `device.resetIndicatorConfig` result | 见 `device.resetIndicatorConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `IndicatorConfigChangedEvent` | `device.indicatorConfigChanged` | 见 `device.indicatorConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `IndicatorState` | 表达 `device.indicator` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `IndicatorConfig` | 表达 `device.indicator` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `device.indicator` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `device.indicator` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `device.indicator` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
