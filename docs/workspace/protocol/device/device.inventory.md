...
#### 3.1.3 返回结果 Result：`GetInventoryCapabilitiesResult`
...
#### 3.2.3 返回结果 Result：`GetInventoryConfigResult`
...
#### 3.3.3 返回结果 Result：`SetInventoryConfigResult`
...
#### 3.4.3 返回结果 Result：`ResetInventoryConfigResult`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `device.inventory` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `device.inventory` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
