...
#### 3.2.3 返回结果 Result：`DeviceTopology`
...
#### 3.3.3 返回结果 Result：`GetChildrenResult`
...
#### 3.4.3 返回结果 Result：`ChildDeviceInfo`
...
### 7.1 场景：读取或修改 `device.childDevice`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `device.childDevice` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `device.childDevice` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
