...
`input.kvm` 用于KVM 输入切换、键鼠路由和 KVM 状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | input.kvm 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 input.kvm 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 input.kvm 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `input.getKvmCapabilities` | query | 查询 input.kvm 能力范围 | `GetKvmCapabilitiesParams` | `GetKvmCapabilitiesResult` | 否 | draft |
| `input.getKvmConfig` | query | 查询 input.kvm 配置 | `GetKvmConfigParams` | `GetKvmConfigResult` | 否 | draft |
| `input.setKvmConfig` | command | 设置 input.kvm 配置 | `SetKvmConfigParams` | `SetKvmConfigResult` | 是，`input.kvmConfigChanged` | draft |
| `input.resetKvmConfig` | action | 恢复 input.kvm 默认配置 | `ResetKvmConfigParams` | `ResetKvmConfigResult` | 是，`input.kvmConfigChanged` | draft |

### 3.1 `input.getKvmCapabilities`

**用途**：查询 input.kvm 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetKvmCapabilitiesParams` |
| Result Schema | `GetKvmCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetKvmCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetKvmCapabilitiesResult`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetKvmConfigParams` |
| Result Schema | `GetKvmConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetKvmConfigParams`
...
#### 3.2.3 返回结果 Result：`GetKvmConfigResult`
...
|---|---|
| 调用类型 | command |
| Params Schema | `SetKvmConfigParams` |
| Result Schema | `SetKvmConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `input.kvmConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetKvmConfigParams`
...
#### 3.3.3 返回结果 Result：`SetKvmConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `input.kvmConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `KvmConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "input.kvmConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `input.resetKvmConfig`

**用途**：恢复 input.kvm 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetKvmConfigParams` |
| Result Schema | `ResetKvmConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `input.kvmConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetKvmConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetKvmConfigResult`
...
|---|---|---|---|---|
| `input.kvmConfigChanged` | input.kvm 配置变化 | `KvmConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `input.kvmConfigChanged`

**触发条件**：input.kvm 配置变化。

#### 4.1.1 Payload：`KvmConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `input.kvm` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetKvmCapabilitiesParams` | `input.getKvmCapabilities` request params | 见 `input.getKvmCapabilities` 方法小节。 |
| `GetKvmCapabilitiesResult` | `input.getKvmCapabilities` result | 见 `input.getKvmCapabilities` 方法小节。 |
| `GetKvmConfigParams` | `input.getKvmConfig` request params | 见 `input.getKvmConfig` 方法小节。 |
| `GetKvmConfigResult` | `input.getKvmConfig` result | 见 `input.getKvmConfig` 方法小节。 |
| `SetKvmConfigParams` | `input.setKvmConfig` request params | 见 `input.setKvmConfig` 方法小节。 |
| `SetKvmConfigResult` | `input.setKvmConfig` result | 见 `input.setKvmConfig` 方法小节。 |
| `ResetKvmConfigParams` | `input.resetKvmConfig` request params | 见 `input.resetKvmConfig` 方法小节。 |
| `ResetKvmConfigResult` | `input.resetKvmConfig` result | 见 `input.resetKvmConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `KvmConfigChangedEvent` | `input.kvmConfigChanged` | 见 `input.kvmConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `KvmState` | 表达 `input.kvm` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `KvmConfig` | 表达 `input.kvm` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `input.kvm` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `input.kvm` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `input.kvm` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
