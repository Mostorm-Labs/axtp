...
`input.source` 用于控制输入源、外部输入通道和输入选择。
...
| 类型 | 内容 |
|---|---|
| 包含 | input.source 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 input.source 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 input.source 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `input.getSourceCapabilities` | query | 查询 input.source 能力范围 | `GetSourceCapabilitiesParams` | `GetSourceCapabilitiesResult` | 否 | draft |
| `input.getSourceConfig` | query | 查询 input.source 配置 | `GetSourceConfigParams` | `GetSourceConfigResult` | 否 | draft |
| `input.setSourceConfig` | command | 设置 input.source 配置 | `SetSourceConfigParams` | `SetSourceConfigResult` | 是，`input.sourceConfigChanged` | draft |
| `input.resetSourceConfig` | action | 恢复 input.source 默认配置 | `ResetSourceConfigParams` | `ResetSourceConfigResult` | 是，`input.sourceConfigChanged` | draft |

### 3.1 `input.getSourceCapabilities`

**用途**：查询 input.source 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSourceCapabilitiesParams` |
| Result Schema | `GetSourceCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetSourceCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetSourceCapabilitiesResult`
...
### 3.2 `input.getSourceConfig`

**用途**：查询 input.source 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSourceConfigParams` |
| Result Schema | `GetSourceConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetSourceConfigParams`
...
#### 3.2.3 返回结果 Result：`GetSourceConfigResult`
...
### 3.3 `input.setSourceConfig`

**用途**：设置 input.source 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetSourceConfigParams` |
| Result Schema | `SetSourceConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `input.sourceConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetSourceConfigParams`
...
#### 3.3.3 返回结果 Result：`SetSourceConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `input.sourceConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `SourceConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "input.sourceConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `input.resetSourceConfig`

**用途**：恢复 input.source 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetSourceConfigParams` |
| Result Schema | `ResetSourceConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `input.sourceConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetSourceConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetSourceConfigResult`
...
|---|---|---|---|---|
| `input.sourceConfigChanged` | input.source 配置变化 | `SourceConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `input.sourceConfigChanged`

**触发条件**：input.source 配置变化。

#### 4.1.1 Payload：`SourceConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `input.source` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetSourceCapabilitiesParams` | `input.getSourceCapabilities` request params | 见 `input.getSourceCapabilities` 方法小节。 |
| `GetSourceCapabilitiesResult` | `input.getSourceCapabilities` result | 见 `input.getSourceCapabilities` 方法小节。 |
| `GetSourceConfigParams` | `input.getSourceConfig` request params | 见 `input.getSourceConfig` 方法小节。 |
| `GetSourceConfigResult` | `input.getSourceConfig` result | 见 `input.getSourceConfig` 方法小节。 |
| `SetSourceConfigParams` | `input.setSourceConfig` request params | 见 `input.setSourceConfig` 方法小节。 |
| `SetSourceConfigResult` | `input.setSourceConfig` result | 见 `input.setSourceConfig` 方法小节。 |
| `ResetSourceConfigParams` | `input.resetSourceConfig` request params | 见 `input.resetSourceConfig` 方法小节。 |
| `ResetSourceConfigResult` | `input.resetSourceConfig` result | 见 `input.resetSourceConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `SourceConfigChangedEvent` | `input.sourceConfigChanged` | 见 `input.sourceConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `SourceState` | 表达 `input.source` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `SourceConfig` | 表达 `input.source` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `input.source` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `input.source` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `input.source` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
