...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | HID 输入、HID 上报开关和 HID 状态。 |
| 当前状态 | draft |
...
`input.hid` 用于HID 输入、HID 上报开关和 HID 状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | input.hid 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 input.hid 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 input.hid 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `input.getHidCapabilities` | query | 查询 input.hid 能力范围 | `GetHidCapabilitiesParams` | `GetHidCapabilitiesResult` | 否 | draft |
| `input.getHidConfig` | query | 查询 input.hid 配置 | `GetHidConfigParams` | `GetHidConfigResult` | 否 | draft |
| `input.setHidConfig` | command | 设置 input.hid 配置 | `SetHidConfigParams` | `SetHidConfigResult` | 是，`input.hidConfigChanged` | draft |
| `input.resetHidConfig` | action | 恢复 input.hid 默认配置 | `ResetHidConfigParams` | `ResetHidConfigResult` | 是，`input.hidConfigChanged` | draft |

### 3.1 `input.getHidCapabilities`

**用途**：查询 input.hid 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetHidCapabilitiesParams` |
| Result Schema | `GetHidCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetHidCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetHidCapabilitiesResult`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetHidConfigParams` |
| Result Schema | `GetHidConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetHidConfigParams`
...
#### 3.2.3 返回结果 Result：`GetHidConfigResult`
...
|---|---|
| 调用类型 | command |
| Params Schema | `SetHidConfigParams` |
| Result Schema | `SetHidConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `input.hidConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetHidConfigParams`
...
#### 3.3.3 返回结果 Result：`SetHidConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `input.hidConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `HidConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "input.hidConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `input.resetHidConfig`

**用途**：恢复 input.hid 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetHidConfigParams` |
| Result Schema | `ResetHidConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `input.hidConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetHidConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetHidConfigResult`
...
|---|---|---|---|---|
| `input.hidConfigChanged` | input.hid 配置变化 | `HidConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `input.hidConfigChanged`

**触发条件**：input.hid 配置变化。

#### 4.1.1 Payload：`HidConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `input.hid` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetHidCapabilitiesParams` | `input.getHidCapabilities` request params | 见 `input.getHidCapabilities` 方法小节。 |
| `GetHidCapabilitiesResult` | `input.getHidCapabilities` result | 见 `input.getHidCapabilities` 方法小节。 |
| `GetHidConfigParams` | `input.getHidConfig` request params | 见 `input.getHidConfig` 方法小节。 |
| `GetHidConfigResult` | `input.getHidConfig` result | 见 `input.getHidConfig` 方法小节。 |
| `SetHidConfigParams` | `input.setHidConfig` request params | 见 `input.setHidConfig` 方法小节。 |
| `SetHidConfigResult` | `input.setHidConfig` result | 见 `input.setHidConfig` 方法小节。 |
| `ResetHidConfigParams` | `input.resetHidConfig` request params | 见 `input.resetHidConfig` 方法小节。 |
| `ResetHidConfigResult` | `input.resetHidConfig` result | 见 `input.resetHidConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `HidConfigChangedEvent` | `input.hidConfigChanged` | 见 `input.hidConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `HidState` | 表达 `input.hid` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `HidConfig` | 表达 `input.hid` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `input.hid` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `input.hid` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `input.hid` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
