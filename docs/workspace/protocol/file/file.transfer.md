...
#### 3.1.3 返回结果 Result：`BeginTransferResult`
...
#### 3.2.3 返回结果 Result：`EndTransferResult`
...
#### 3.3.3 返回结果 Result：`CancelTransferResult`
...
#### 3.4.3 返回结果 Result：`ResumeTransferResult`
...
#### 3.5.3 返回结果 Result：`GetTransferStateResult`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `file.transfer` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `file.transfer` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `file.transfer` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
