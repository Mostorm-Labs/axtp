...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 通用文件资源列表、文件元信息、删除和文件存储状态。 |
| 当前状态 | draft |
...
`file.storage` 用于通用文件资源列表、文件元信息、删除和文件存储状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | file.storage 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 file.storage 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 file.storage 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `file.listFiles` | query | 列出文件资源 | `ListFilesParams` | `ListFilesResult` | 否 | draft |
| `file.getFileInfo` | query | 查询文件元信息 | `GetFileInfoParams` | `GetFileInfoResult` | 否 | draft |
| `file.deleteFiles` | action | 删除文件资源 | `DeleteFilesParams` | `DeleteFilesResult` | 是，`file.fileListChanged` | draft |
...
#### 3.1.3 返回结果 Result：`ListFilesResult`
...
#### 3.2.3 返回结果 Result：`GetFileInfoResult`
...
| 是否触发事件 | 是，状态实际变化后触发 `file.fileListChanged`。 |
...
#### 3.3.3 返回结果 Result：`DeleteFilesResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `file.fileListChanged` | 该方法导致状态、配置或动作状态实际变化。 | `FileListChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "file.fileListChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
|---|---|---|---|---|
| `file.fileListChanged` | 文件列表变化 | `FileListChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `file.fileListChanged`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `file.storage` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `ListFilesParams` | `file.listFiles` request params | 见 `file.listFiles` 方法小节。 |
| `ListFilesResult` | `file.listFiles` result | 见 `file.listFiles` 方法小节。 |
| `GetFileInfoParams` | `file.getFileInfo` request params | 见 `file.getFileInfo` 方法小节。 |
| `GetFileInfoResult` | `file.getFileInfo` result | 见 `file.getFileInfo` 方法小节。 |
| `DeleteFilesParams` | `file.deleteFiles` request params | 见 `file.deleteFiles` 方法小节。 |
| `DeleteFilesResult` | `file.deleteFiles` result | 见 `file.deleteFiles` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `FileListChangedEvent` | `file.fileListChanged` | 见 `file.fileListChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `StorageState` | 表达 `file.storage` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `StorageConfig` | 表达 `file.storage` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `file.storage` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `file.storage` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `file.storage` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
