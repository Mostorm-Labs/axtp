...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | RTSP 视频访问服务的启用、端口、路径、认证、URL 生成和服务状态。 |
| 当前状态 | draft |
...
`video.rtsp` 用于RTSP 视频访问服务的启用、端口、路径、认证、URL 生成和服务状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | video.rtsp 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 video.rtsp 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 video.rtsp 的语义归类。 |
| 不包含 | RTSP 服务配置不等同于 AXTP video.stream 数据面，也不归基础 network。 |
...
|---|---|---|---|---|---|---|
| `video.getRtspCapabilities` | query | 查询 RTSP 服务能力 | `GetRtspCapabilitiesParams` | `GetRtspCapabilitiesResult` | 否 | draft |
| `video.setRtspConfig` | command | 设置 RTSP 服务配置 | `SetRtspConfigParams` | `SetRtspConfigResult` | 是，`video.rtspConfigChanged` | draft |
| `video.getRtspConfig` | query | 查询 RTSP 服务配置 | `GetRtspConfigParams` | `GetRtspConfigResult` | 否 | draft |
| `video.resetRtspConfig` | action | 恢复 RTSP 默认配置 | `ResetRtspConfigParams` | `ResetRtspConfigResult` | 是，`video.rtspConfigChanged` | draft |

### 3.1 `video.getRtspCapabilities`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetRtspCapabilitiesParams` |
| Result Schema | `GetRtspCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetRtspCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetRtspCapabilitiesResult`
...
|---|---|
| 调用类型 | command |
| Params Schema | `SetRtspConfigParams` |
| Result Schema | `SetRtspConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.rtspConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.2.1 请求参数 Params：`SetRtspConfigParams`
...
#### 3.2.3 返回结果 Result：`SetRtspConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `video.rtspConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `RtspConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetRtspConfigParams` |
| Result Schema | `GetRtspConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.3.1 请求参数 Params：`GetRtspConfigParams`
...
#### 3.3.3 返回结果 Result：`GetRtspConfigResult`
...
|---|---|
| 调用类型 | action |
| Params Schema | `ResetRtspConfigParams` |
| Result Schema | `ResetRtspConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.rtspConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetRtspConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetRtspConfigResult`
...
|---|---|---|---|---|
| `video.rtspConfigChanged` | RTSP 配置变化 | `RtspConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |
| `video.rtspStateChanged` | RTSP 服务状态变化 | `RtspStateChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `video.rtspConfigChanged`

**触发条件**：RTSP 配置变化。

#### 4.1.1 Payload：`RtspConfigChangedEvent`
...
### 4.2 `video.rtspStateChanged`

**触发条件**：RTSP 服务状态变化。

#### 4.2.1 Payload：`RtspStateChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `video.rtsp` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetRtspCapabilitiesParams` | `video.getRtspCapabilities` request params | 见 `video.getRtspCapabilities` 方法小节。 |
| `GetRtspCapabilitiesResult` | `video.getRtspCapabilities` result | 见 `video.getRtspCapabilities` 方法小节。 |
| `SetRtspConfigParams` | `video.setRtspConfig` request params | 见 `video.setRtspConfig` 方法小节。 |
| `SetRtspConfigResult` | `video.setRtspConfig` result | 见 `video.setRtspConfig` 方法小节。 |
| `GetRtspConfigParams` | `video.getRtspConfig` request params | 见 `video.getRtspConfig` 方法小节。 |
| `GetRtspConfigResult` | `video.getRtspConfig` result | 见 `video.getRtspConfig` 方法小节。 |
| `ResetRtspConfigParams` | `video.resetRtspConfig` request params | 见 `video.resetRtspConfig` 方法小节。 |
| `ResetRtspConfigResult` | `video.resetRtspConfig` result | 见 `video.resetRtspConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `RtspConfigChangedEvent` | `video.rtspConfigChanged` | 见 `video.rtspConfigChanged` 事件小节。 |
| `RtspStateChangedEvent` | `video.rtspStateChanged` | 见 `video.rtspStateChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `RtspState` | 表达 `video.rtsp` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `RtspConfig` | 表达 `video.rtsp` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `video.rtsp` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `video.rtsp` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `video.rtsp` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
