...
`video.layout` 用于多画面布局、画面排列和 layout 模板。
...
| 类型 | 内容 |
|---|---|
| 包含 | video.layout 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 video.layout 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 video.layout 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `video.getLayoutCapabilities` | query | 查询 video.layout 能力范围 | `GetLayoutCapabilitiesParams` | `GetLayoutCapabilitiesResult` | 否 | draft |
| `video.getLayoutConfig` | query | 查询 video.layout 配置 | `GetLayoutConfigParams` | `GetLayoutConfigResult` | 否 | draft |
| `video.setLayoutConfig` | command | 设置 video.layout 配置 | `SetLayoutConfigParams` | `SetLayoutConfigResult` | 是，`video.layoutConfigChanged` | draft |
| `video.resetLayoutConfig` | action | 恢复 video.layout 默认配置 | `ResetLayoutConfigParams` | `ResetLayoutConfigResult` | 是，`video.layoutConfigChanged` | draft |

### 3.1 `video.getLayoutCapabilities`

**用途**：查询 video.layout 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetLayoutCapabilitiesParams` |
| Result Schema | `GetLayoutCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetLayoutCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetLayoutCapabilitiesResult`
...
### 3.2 `video.getLayoutConfig`

**用途**：查询 video.layout 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetLayoutConfigParams` |
| Result Schema | `GetLayoutConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetLayoutConfigParams`
...
#### 3.2.3 返回结果 Result：`GetLayoutConfigResult`
...
### 3.3 `video.setLayoutConfig`

**用途**：设置 video.layout 配置。
...
#### 3.3.3 返回结果 Result：`SetLayoutConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `video.layoutConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `LayoutConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "video.layoutConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `video.resetLayoutConfig`

**用途**：恢复 video.layout 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetLayoutConfigParams` |
| Result Schema | `ResetLayoutConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.layoutConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetLayoutConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetLayoutConfigResult`
...
|---|---|---|---|---|
| `video.layoutConfigChanged` | video.layout 配置变化 | `LayoutConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `video.layoutConfigChanged`

**触发条件**：video.layout 配置变化。

#### 4.1.1 Payload：`LayoutConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `video.layout` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetLayoutCapabilitiesParams` | `video.getLayoutCapabilities` request params | 见 `video.getLayoutCapabilities` 方法小节。 |
| `GetLayoutCapabilitiesResult` | `video.getLayoutCapabilities` result | 见 `video.getLayoutCapabilities` 方法小节。 |
| `GetLayoutConfigParams` | `video.getLayoutConfig` request params | 见 `video.getLayoutConfig` 方法小节。 |
| `GetLayoutConfigResult` | `video.getLayoutConfig` result | 见 `video.getLayoutConfig` 方法小节。 |
| `SetLayoutConfigParams` | `video.setLayoutConfig` request params | 见 `video.setLayoutConfig` 方法小节。 |
| `SetLayoutConfigResult` | `video.setLayoutConfig` result | 见 `video.setLayoutConfig` 方法小节。 |
| `ResetLayoutConfigParams` | `video.resetLayoutConfig` request params | 见 `video.resetLayoutConfig` 方法小节。 |
| `ResetLayoutConfigResult` | `video.resetLayoutConfig` result | 见 `video.resetLayoutConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `LayoutConfigChangedEvent` | `video.layoutConfigChanged` | 见 `video.layoutConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `LayoutState` | 表达 `video.layout` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `LayoutConfig` | 表达 `video.layout` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `video.layout` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `video.layout` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `video.layout` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
