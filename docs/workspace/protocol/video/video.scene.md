...
`video.scene` 用于视频场景、preset、演示场景和切换策略。
...
| 类型 | 内容 |
|---|---|
| 包含 | video.scene 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 video.scene 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 video.scene 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `video.getSceneCapabilities` | query | 查询 video.scene 能力范围 | `GetSceneCapabilitiesParams` | `GetSceneCapabilitiesResult` | 否 | draft |
| `video.getSceneConfig` | query | 查询 video.scene 配置 | `GetSceneConfigParams` | `GetSceneConfigResult` | 否 | draft |
| `video.setSceneConfig` | command | 设置 video.scene 配置 | `SetSceneConfigParams` | `SetSceneConfigResult` | 是，`video.sceneConfigChanged` | draft |
| `video.resetSceneConfig` | action | 恢复 video.scene 默认配置 | `ResetSceneConfigParams` | `ResetSceneConfigResult` | 是，`video.sceneConfigChanged` | draft |

### 3.1 `video.getSceneCapabilities`

**用途**：查询 video.scene 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSceneCapabilitiesParams` |
| Result Schema | `GetSceneCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetSceneCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetSceneCapabilitiesResult`
...
### 3.2 `video.getSceneConfig`

**用途**：查询 video.scene 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSceneConfigParams` |
| Result Schema | `GetSceneConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetSceneConfigParams`
...
#### 3.2.3 返回结果 Result：`GetSceneConfigResult`
...
### 3.3 `video.setSceneConfig`

**用途**：设置 video.scene 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetSceneConfigParams` |
| Result Schema | `SetSceneConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.sceneConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetSceneConfigParams`
...
#### 3.3.3 返回结果 Result：`SetSceneConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `video.sceneConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `SceneConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "video.sceneConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `video.resetSceneConfig`

**用途**：恢复 video.scene 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetSceneConfigParams` |
| Result Schema | `ResetSceneConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.sceneConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetSceneConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetSceneConfigResult`
...
|---|---|---|---|---|
| `video.sceneConfigChanged` | video.scene 配置变化 | `SceneConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `video.sceneConfigChanged`

**触发条件**：video.scene 配置变化。

#### 4.1.1 Payload：`SceneConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `video.scene` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetSceneCapabilitiesParams` | `video.getSceneCapabilities` request params | 见 `video.getSceneCapabilities` 方法小节。 |
| `GetSceneCapabilitiesResult` | `video.getSceneCapabilities` result | 见 `video.getSceneCapabilities` 方法小节。 |
| `GetSceneConfigParams` | `video.getSceneConfig` request params | 见 `video.getSceneConfig` 方法小节。 |
| `GetSceneConfigResult` | `video.getSceneConfig` result | 见 `video.getSceneConfig` 方法小节。 |
| `SetSceneConfigParams` | `video.setSceneConfig` request params | 见 `video.setSceneConfig` 方法小节。 |
| `SetSceneConfigResult` | `video.setSceneConfig` result | 见 `video.setSceneConfig` 方法小节。 |
| `ResetSceneConfigParams` | `video.resetSceneConfig` request params | 见 `video.resetSceneConfig` 方法小节。 |
| `ResetSceneConfigResult` | `video.resetSceneConfig` result | 见 `video.resetSceneConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `SceneConfigChangedEvent` | `video.sceneConfigChanged` | 见 `video.sceneConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `SceneState` | 表达 `video.scene` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `SceneConfig` | 表达 `video.scene` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `video.scene` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `video.scene` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `video.scene` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
