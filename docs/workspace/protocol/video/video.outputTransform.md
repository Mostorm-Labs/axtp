...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 输出镜像、翻转、旋转、横竖屏、比例和缩放等通用输出变换。 |
| 当前状态 | draft |
...
`video.outputTransform` 用于输出镜像、翻转、旋转、横竖屏、比例和缩放等通用输出变换。
...
| 类型 | 内容 |
|---|---|
| 包含 | video.outputTransform 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 video.outputTransform 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 video.outputTransform 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `video.getOutputTransformCapabilities` | query | 查询输出变换能力 | `GetOutputTransformCapabilitiesParams` | `GetOutputTransformCapabilitiesResult` | 否 | draft |
| `video.setOutputTransform` | command | 设置镜像、翻转、旋转、比例等输出变换 | `SetOutputTransformParams` | `SetOutputTransformResult` | 是，`video.outputTransformChanged` | draft |
| `video.getOutputTransform` | query | 查询输出变换 | `GetOutputTransformParams` | `GetOutputTransformResult` | 否 | draft |
| `video.resetOutputTransform` | action | 恢复输出变换默认值 | `ResetOutputTransformParams` | `ResetOutputTransformResult` | 是，`video.outputTransformChanged` | draft |

### 3.1 `video.getOutputTransformCapabilities`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetOutputTransformCapabilitiesParams` |
| Result Schema | `GetOutputTransformCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetOutputTransformCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetOutputTransformCapabilitiesResult`
...
### 3.2 `video.setOutputTransform`

**用途**：设置镜像、翻转、旋转、比例等输出变换。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetOutputTransformParams` |
| Result Schema | `SetOutputTransformResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.outputTransformChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.2.1 请求参数 Params：`SetOutputTransformParams`
...
#### 3.2.3 返回结果 Result：`SetOutputTransformResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `video.outputTransformChanged` | 该方法导致状态、配置或动作状态实际变化。 | `OutputTransformChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetOutputTransformParams` |
| Result Schema | `GetOutputTransformResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.3.1 请求参数 Params：`GetOutputTransformParams`
...
#### 3.3.3 返回结果 Result：`GetOutputTransformResult`
...
### 3.4 `video.resetOutputTransform`
...
|---|---|
| 调用类型 | action |
| Params Schema | `ResetOutputTransformParams` |
| Result Schema | `ResetOutputTransformResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.outputTransformChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetOutputTransformParams`
...
#### 3.4.3 返回结果 Result：`ResetOutputTransformResult`
...
|---|---|---|---|---|
| `video.outputTransformChanged` | 输出变换变化 | `OutputTransformChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `video.outputTransformChanged`

**触发条件**：输出变换变化。

#### 4.1.1 Payload：`OutputTransformChangedEvent`
...
Capability name: `video.outputTransform`。
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetOutputTransformCapabilitiesParams` | `video.getOutputTransformCapabilities` request params | 见 `video.getOutputTransformCapabilities` 方法小节。 |
| `GetOutputTransformCapabilitiesResult` | `video.getOutputTransformCapabilities` result | 见 `video.getOutputTransformCapabilities` 方法小节。 |
| `SetOutputTransformParams` | `video.setOutputTransform` request params | 见 `video.setOutputTransform` 方法小节。 |
| `SetOutputTransformResult` | `video.setOutputTransform` result | 见 `video.setOutputTransform` 方法小节。 |
| `GetOutputTransformParams` | `video.getOutputTransform` request params | 见 `video.getOutputTransform` 方法小节。 |
| `GetOutputTransformResult` | `video.getOutputTransform` result | 见 `video.getOutputTransform` 方法小节。 |
| `ResetOutputTransformParams` | `video.resetOutputTransform` request params | 见 `video.resetOutputTransform` 方法小节。 |
| `ResetOutputTransformResult` | `video.resetOutputTransform` result | 见 `video.resetOutputTransform` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `OutputTransformChangedEvent` | `video.outputTransformChanged` | 见 `video.outputTransformChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `OutputTransformState` | 表达 `video.outputTransform` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `OutputTransformConfig` | 表达 `video.outputTransform` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `video.outputTransform`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `video.outputTransform` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `video.outputTransform` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `video.outputTransform` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
