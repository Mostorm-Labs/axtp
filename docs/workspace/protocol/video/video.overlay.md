...
`video.overlay` 用于视频图层、水印、图形叠加和区域叠加。
...
| 类型 | 内容 |
|---|---|
| 包含 | video.overlay 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 video.overlay 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 video.overlay 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `video.getOverlayCapabilities` | query | 查询 video.overlay 能力范围 | `GetOverlayCapabilitiesParams` | `GetOverlayCapabilitiesResult` | 否 | draft |
| `video.getOverlayConfig` | query | 查询 video.overlay 配置 | `GetOverlayConfigParams` | `GetOverlayConfigResult` | 否 | draft |
| `video.setOverlayConfig` | command | 设置 video.overlay 配置 | `SetOverlayConfigParams` | `SetOverlayConfigResult` | 是，`video.overlayConfigChanged` | draft |
| `video.resetOverlayConfig` | action | 恢复 video.overlay 默认配置 | `ResetOverlayConfigParams` | `ResetOverlayConfigResult` | 是，`video.overlayConfigChanged` | draft |

### 3.1 `video.getOverlayCapabilities`

**用途**：查询 video.overlay 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetOverlayCapabilitiesParams` |
| Result Schema | `GetOverlayCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetOverlayCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetOverlayCapabilitiesResult`
...
### 3.2 `video.getOverlayConfig`

**用途**：查询 video.overlay 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetOverlayConfigParams` |
| Result Schema | `GetOverlayConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetOverlayConfigParams`
...
#### 3.2.3 返回结果 Result：`GetOverlayConfigResult`
...
### 3.3 `video.setOverlayConfig`

**用途**：设置 video.overlay 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetOverlayConfigParams` |
| Result Schema | `SetOverlayConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.overlayConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetOverlayConfigParams`
...
#### 3.3.3 返回结果 Result：`SetOverlayConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `video.overlayConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `OverlayConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "video.overlayConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `video.resetOverlayConfig`

**用途**：恢复 video.overlay 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetOverlayConfigParams` |
| Result Schema | `ResetOverlayConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.overlayConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetOverlayConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetOverlayConfigResult`
...
|---|---|---|---|---|
| `video.overlayConfigChanged` | video.overlay 配置变化 | `OverlayConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `video.overlayConfigChanged`

**触发条件**：video.overlay 配置变化。

#### 4.1.1 Payload：`OverlayConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `video.overlay` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetOverlayCapabilitiesParams` | `video.getOverlayCapabilities` request params | 见 `video.getOverlayCapabilities` 方法小节。 |
| `GetOverlayCapabilitiesResult` | `video.getOverlayCapabilities` result | 见 `video.getOverlayCapabilities` 方法小节。 |
| `GetOverlayConfigParams` | `video.getOverlayConfig` request params | 见 `video.getOverlayConfig` 方法小节。 |
| `GetOverlayConfigResult` | `video.getOverlayConfig` result | 见 `video.getOverlayConfig` 方法小节。 |
| `SetOverlayConfigParams` | `video.setOverlayConfig` request params | 见 `video.setOverlayConfig` 方法小节。 |
| `SetOverlayConfigResult` | `video.setOverlayConfig` result | 见 `video.setOverlayConfig` 方法小节。 |
| `ResetOverlayConfigParams` | `video.resetOverlayConfig` request params | 见 `video.resetOverlayConfig` 方法小节。 |
| `ResetOverlayConfigResult` | `video.resetOverlayConfig` result | 见 `video.resetOverlayConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `OverlayConfigChangedEvent` | `video.overlayConfigChanged` | 见 `video.overlayConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `OverlayState` | 表达 `video.overlay` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `OverlayConfig` | 表达 `video.overlay` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `video.overlay` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `video.overlay` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `video.overlay` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
