...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 视频编码参数、码率、GOP、profile 和编码状态。 |
| 当前状态 | draft |
...
`video.encoder` 用于视频编码参数、码率、GOP、profile 和编码状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | video.encoder 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 video.encoder 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 video.encoder 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `video.getEncoderCapabilities` | query | 查询 video.encoder 能力范围 | `GetEncoderCapabilitiesParams` | `GetEncoderCapabilitiesResult` | 否 | draft |
| `video.getEncoderConfig` | query | 查询 video.encoder 配置 | `GetEncoderConfigParams` | `GetEncoderConfigResult` | 否 | draft |
| `video.setEncoderConfig` | command | 设置 video.encoder 配置 | `SetEncoderConfigParams` | `SetEncoderConfigResult` | 是，`video.encoderConfigChanged` | draft |
| `video.resetEncoderConfig` | action | 恢复 video.encoder 默认配置 | `ResetEncoderConfigParams` | `ResetEncoderConfigResult` | 是，`video.encoderConfigChanged` | draft |

### 3.1 `video.getEncoderCapabilities`

**用途**：查询 video.encoder 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetEncoderCapabilitiesParams` |
| Result Schema | `GetEncoderCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetEncoderCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetEncoderCapabilitiesResult`
...
### 3.2 `video.getEncoderConfig`

**用途**：查询 video.encoder 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetEncoderConfigParams` |
| Result Schema | `GetEncoderConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetEncoderConfigParams`
...
#### 3.2.3 返回结果 Result：`GetEncoderConfigResult`
...
### 3.3 `video.setEncoderConfig`

**用途**：设置 video.encoder 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetEncoderConfigParams` |
| Result Schema | `SetEncoderConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.encoderConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetEncoderConfigParams`
...
#### 3.3.3 返回结果 Result：`SetEncoderConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `video.encoderConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `EncoderConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "video.encoderConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `video.resetEncoderConfig`

**用途**：恢复 video.encoder 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetEncoderConfigParams` |
| Result Schema | `ResetEncoderConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `video.encoderConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetEncoderConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetEncoderConfigResult`
...
|---|---|---|---|---|
| `video.encoderConfigChanged` | video.encoder 配置变化 | `EncoderConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `video.encoderConfigChanged`

**触发条件**：video.encoder 配置变化。

#### 4.1.1 Payload：`EncoderConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `video.encoder` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetEncoderCapabilitiesParams` | `video.getEncoderCapabilities` request params | 见 `video.getEncoderCapabilities` 方法小节。 |
| `GetEncoderCapabilitiesResult` | `video.getEncoderCapabilities` result | 见 `video.getEncoderCapabilities` 方法小节。 |
| `GetEncoderConfigParams` | `video.getEncoderConfig` request params | 见 `video.getEncoderConfig` 方法小节。 |
| `GetEncoderConfigResult` | `video.getEncoderConfig` result | 见 `video.getEncoderConfig` 方法小节。 |
| `SetEncoderConfigParams` | `video.setEncoderConfig` request params | 见 `video.setEncoderConfig` 方法小节。 |
| `SetEncoderConfigResult` | `video.setEncoderConfig` result | 见 `video.setEncoderConfig` 方法小节。 |
| `ResetEncoderConfigParams` | `video.resetEncoderConfig` request params | 见 `video.resetEncoderConfig` 方法小节。 |
| `ResetEncoderConfigResult` | `video.resetEncoderConfig` result | 见 `video.resetEncoderConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `EncoderConfigChangedEvent` | `video.encoderConfigChanged` | 见 `video.encoderConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `EncoderState` | 表达 `video.encoder` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `EncoderConfig` | 表达 `video.encoder` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `video.encoder` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `video.encoder` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `video.encoder` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
