...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 设备输出端的画面布局、幕墙/拼接/画中画输出模式、输出画布上的窗口排列。 |
| 当前状态 | draft |
...
`output.layout` 用于设备输出端的画面布局、幕墙/拼接/画中画输出模式、输出画布上的窗口排列。
...
| 类型 | 内容 |
|---|---|
| 包含 | 输出端 layout 能力发现、当前配置查询、配置设置和变化事件。 |
| 包含 | 幕墙、拼接、画中画、单画面、多窗口等输出端布局模式。 |
| 包含 | 输出画布坐标、窗口矩形、层级、缩放策略和应用策略。 |
| 包含 | 已确认 legacy 协议到 output.layout 的语义归类。 |
| 不包含 | 视频内容构图、自动取景、多路视频混合模板；这些归 video.layout / video.framing。 |
| 不包含 | 会议室场景、协作空间场景编排；这些归 room.layout / room.scene。 |
| 不包含 | 显示输出端口状态、端口启停、分辨率或链路状态；这些归 display.output 或 output.source。 |
| 不包含 | 输入到输出的源选择和路由关系；这些归 output.source / output.routing，本文只引用 source/routing 标识。 |
...
|---|---|---|---|---|---|---|
| `output.getLayoutConfig` | query | 原草案中出现的候选方法 | `0x0B05` | `OutputLayoutGetParams` | 否 | draft |
| `output.setLayoutConfig` | command | 原草案中出现的候选方法 | `0x0B06` | `OutputLayoutSetParams` | 是，`output.layoutChanged` | draft |
...
### 3.3 `output.getLayoutCapabilities`

**用途**：查询布局能力范围；10 当前未规划该 method，采纳时需决定是否新增，或由 capability 查询统一覆盖。
...
|---|---|---|---|---|
| `output.layoutChanged` | 原草案中出现的候选事件 | `0x0B03` | 更新 UI 或调用对应 get method 校准 | draft |
...
| Schema | 用途 | 状态 |
|---|---|---|
| `LayoutState` | 表达 `output.layout` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `LayoutConfig` | 表达 `output.layout` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `output.layout` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `output.layout` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `output.layout` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
