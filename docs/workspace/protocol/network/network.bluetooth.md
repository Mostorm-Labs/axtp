...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 蓝牙基础网络邻近连接信息，例如 MAC、名称、开关状态和基础恢复动作。 |
| 当前状态 | draft |
...
`network.bluetooth` 用于蓝牙基础网络邻近连接信息，例如 MAC、名称、开关状态和基础恢复动作。
...
| 类型 | 内容 |
|---|---|
| 包含 | network.bluetooth 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 network.bluetooth 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 network.bluetooth 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `network.getBluetoothInfo` | query | 查询蓝牙基础信息 | `GetBluetoothInfoParams` | `GetBluetoothInfoResult` | 否 | draft |
| `network.setBluetoothConfig` | command | 设置蓝牙名称等基础配置 | `SetBluetoothConfigParams` | `SetBluetoothConfigResult` | 是，`network.bluetoothStateChanged` | draft |
| `network.resetBluetoothConfig` | action | 恢复蓝牙默认配置 | `ResetBluetoothConfigParams` | `ResetBluetoothConfigResult` | 是，`network.bluetoothStateChanged` | draft |

### 3.1 `network.getBluetoothInfo`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetBluetoothInfoParams` |
| Result Schema | `GetBluetoothInfoResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetBluetoothInfoParams`
...
#### 3.1.3 返回结果 Result：`GetBluetoothInfoResult`
...
### 3.2 `network.setBluetoothConfig`
...
|---|---|
| 调用类型 | command |
| Params Schema | `SetBluetoothConfigParams` |
| Result Schema | `SetBluetoothConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `network.bluetoothStateChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.2.1 请求参数 Params：`SetBluetoothConfigParams`
...
#### 3.2.3 返回结果 Result：`SetBluetoothConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `network.bluetoothStateChanged` | 该方法导致状态、配置或动作状态实际变化。 | `BluetoothStateChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "network.bluetoothStateChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.3 `network.resetBluetoothConfig`
...
|---|---|
| 调用类型 | action |
| Params Schema | `ResetBluetoothConfigParams` |
| Result Schema | `ResetBluetoothConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `network.bluetoothStateChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`ResetBluetoothConfigParams`
...
#### 3.3.3 返回结果 Result：`ResetBluetoothConfigResult`
...
|---|---|---|---|---|
| `network.bluetoothStateChanged` | 蓝牙状态变化 | `BluetoothStateChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `network.bluetoothStateChanged`

**触发条件**：蓝牙状态变化。

#### 4.1.1 Payload：`BluetoothStateChangedEvent`
...
Capability name: `network.bluetooth`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `network.bluetooth` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetBluetoothInfoParams` | `network.getBluetoothInfo` request params | 见 `network.getBluetoothInfo` 方法小节。 |
| `GetBluetoothInfoResult` | `network.getBluetoothInfo` result | 见 `network.getBluetoothInfo` 方法小节。 |
| `SetBluetoothConfigParams` | `network.setBluetoothConfig` request params | 见 `network.setBluetoothConfig` 方法小节。 |
| `SetBluetoothConfigResult` | `network.setBluetoothConfig` result | 见 `network.setBluetoothConfig` 方法小节。 |
| `ResetBluetoothConfigParams` | `network.resetBluetoothConfig` request params | 见 `network.resetBluetoothConfig` 方法小节。 |
| `ResetBluetoothConfigResult` | `network.resetBluetoothConfig` result | 见 `network.resetBluetoothConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `BluetoothStateChangedEvent` | `network.bluetoothStateChanged` | 见 `network.bluetoothStateChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `BluetoothState` | 表达 `network.bluetooth` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `BluetoothConfig` | 表达 `network.bluetooth` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `network.bluetooth`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `network.bluetooth` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `network.bluetooth` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `network.bluetooth` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
