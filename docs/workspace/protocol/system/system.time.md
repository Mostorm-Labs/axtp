...
`system.time` 用于系统时间、时区、NTP 和时间同步。
...
| 类型 | 内容 |
|---|---|
| 包含 | system.time 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 system.time 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 system.time 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `system.getTimeCapabilities` | query | 查询 system.time 能力范围 | `GetTimeCapabilitiesParams` | `GetTimeCapabilitiesResult` | 否 | draft |
| `system.getTimeConfig` | query | 查询 system.time 配置 | `GetTimeConfigParams` | `GetTimeConfigResult` | 否 | draft |
| `system.setTimeConfig` | command | 设置 system.time 配置 | `SetTimeConfigParams` | `SetTimeConfigResult` | 是，`system.timeConfigChanged` | draft |
| `system.resetTimeConfig` | action | 恢复 system.time 默认配置 | `ResetTimeConfigParams` | `ResetTimeConfigResult` | 是，`system.timeConfigChanged` | draft |

### 3.1 `system.getTimeCapabilities`

**用途**：查询 system.time 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetTimeCapabilitiesParams` |
| Result Schema | `GetTimeCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetTimeCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetTimeCapabilitiesResult`
...
### 3.2 `system.getTimeConfig`

**用途**：查询 system.time 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetTimeConfigParams` |
| Result Schema | `GetTimeConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetTimeConfigParams`
...
#### 3.2.3 返回结果 Result：`GetTimeConfigResult`
...
### 3.3 `system.setTimeConfig`

**用途**：设置 system.time 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetTimeConfigParams` |
| Result Schema | `SetTimeConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `system.timeConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetTimeConfigParams`
...
#### 3.3.3 返回结果 Result：`SetTimeConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `system.timeConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `TimeConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "system.timeConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `system.resetTimeConfig`

**用途**：恢复 system.time 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetTimeConfigParams` |
| Result Schema | `ResetTimeConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `system.timeConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetTimeConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetTimeConfigResult`
...
|---|---|---|---|---|
| `system.timeConfigChanged` | system.time 配置变化 | `TimeConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `system.timeConfigChanged`

**触发条件**：system.time 配置变化。

#### 4.1.1 Payload：`TimeConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `system.time` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetTimeCapabilitiesParams` | `system.getTimeCapabilities` request params | 见 `system.getTimeCapabilities` 方法小节。 |
| `GetTimeCapabilitiesResult` | `system.getTimeCapabilities` result | 见 `system.getTimeCapabilities` 方法小节。 |
| `GetTimeConfigParams` | `system.getTimeConfig` request params | 见 `system.getTimeConfig` 方法小节。 |
| `GetTimeConfigResult` | `system.getTimeConfig` result | 见 `system.getTimeConfig` 方法小节。 |
| `SetTimeConfigParams` | `system.setTimeConfig` request params | 见 `system.setTimeConfig` 方法小节。 |
| `SetTimeConfigResult` | `system.setTimeConfig` result | 见 `system.setTimeConfig` 方法小节。 |
| `ResetTimeConfigParams` | `system.resetTimeConfig` request params | 见 `system.resetTimeConfig` 方法小节。 |
| `ResetTimeConfigResult` | `system.resetTimeConfig` result | 见 `system.resetTimeConfig` 方法小节。 |
...
| Schema | 用途 | 状态 |
|---|---|---|
| `TimeState` | 表达 `system.time` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `TimeConfig` | 表达 `system.time` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `system.time` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `system.time` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `system.time` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
