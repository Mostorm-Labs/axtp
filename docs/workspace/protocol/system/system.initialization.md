...
`system.initialization` 用于首次初始化、初始化向导和初始化状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | system.initialization 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 system.initialization 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 system.initialization 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `system.getInitializationCapabilities` | query | 查询 system.initialization 能力范围 | `GetInitializationCapabilitiesParams` | `GetInitializationCapabilitiesResult` | 否 | draft |
| `system.getInitializationConfig` | query | 查询 system.initialization 配置 | `GetInitializationConfigParams` | `GetInitializationConfigResult` | 否 | draft |
| `system.setInitializationConfig` | command | 设置 system.initialization 配置 | `SetInitializationConfigParams` | `SetInitializationConfigResult` | 是，`system.initializationConfigChanged` | draft |
| `system.resetInitializationConfig` | action | 恢复 system.initialization 默认配置 | `ResetInitializationConfigParams` | `ResetInitializationConfigResult` | 是，`system.initializationConfigChanged` | draft |

### 3.1 `system.getInitializationCapabilities`

**用途**：查询 system.initialization 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetInitializationCapabilitiesParams` |
| Result Schema | `GetInitializationCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetInitializationCapabilitiesParams`
...
{
  "id": 101,
  "method": "system.getInitializationCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetInitializationCapabilitiesResult`
...
### 3.2 `system.getInitializationConfig`

**用途**：查询 system.initialization 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetInitializationConfigParams` |
| Result Schema | `GetInitializationConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetInitializationConfigParams`
...
{
  "id": 102,
  "method": "system.getInitializationConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetInitializationConfigResult`
...
### 3.3 `system.setInitializationConfig`

**用途**：设置 system.initialization 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetInitializationConfigParams` |
| Result Schema | `SetInitializationConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `system.initializationConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetInitializationConfigParams`
...
{
  "id": 103,
  "method": "system.setInitializationConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetInitializationConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `system.initializationConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `InitializationConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "system.initializationConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `system.resetInitializationConfig`

**用途**：恢复 system.initialization 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetInitializationConfigParams` |
| Result Schema | `ResetInitializationConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `system.initializationConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetInitializationConfigParams`
...
{
  "id": 104,
  "method": "system.resetInitializationConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetInitializationConfigResult`
...
|---|---|---|---|---|
| `system.initializationConfigChanged` | system.initialization 配置变化 | `InitializationConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `system.initializationConfigChanged`

**触发条件**：system.initialization 配置变化。

#### 4.1.1 Payload：`InitializationConfigChangedEvent`
...
Capability name: `system.initialization`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `system.initialization` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetInitializationCapabilitiesParams` | `system.getInitializationCapabilities` request params | 见 `system.getInitializationCapabilities` 方法小节。 |
| `GetInitializationCapabilitiesResult` | `system.getInitializationCapabilities` result | 见 `system.getInitializationCapabilities` 方法小节。 |
| `GetInitializationConfigParams` | `system.getInitializationConfig` request params | 见 `system.getInitializationConfig` 方法小节。 |
| `GetInitializationConfigResult` | `system.getInitializationConfig` result | 见 `system.getInitializationConfig` 方法小节。 |
| `SetInitializationConfigParams` | `system.setInitializationConfig` request params | 见 `system.setInitializationConfig` 方法小节。 |
| `SetInitializationConfigResult` | `system.setInitializationConfig` result | 见 `system.setInitializationConfig` 方法小节。 |
| `ResetInitializationConfigParams` | `system.resetInitializationConfig` request params | 见 `system.resetInitializationConfig` 方法小节。 |
| `ResetInitializationConfigResult` | `system.resetInitializationConfig` result | 见 `system.resetInitializationConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `InitializationConfigChangedEvent` | `system.initializationConfigChanged` | 见 `system.initializationConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `InitializationState` | 表达 `system.initialization` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `InitializationConfig` | 表达 `system.initialization` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `system.initialization`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `system.initialization` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `system.initialization` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `system.initialization` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
