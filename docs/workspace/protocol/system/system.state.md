...
|---|---|---|---|---|---|---|
| `system.recoverRuntimeState` | action | 原草案中出现的候选方法 | `RecoverRuntimeStateParams` | `RecoverRuntimeStateResult` | 是，`system.stateChanged` | draft |
...
|---|---|---|---|---|
| `system.stateChanged` | 原草案中出现的候选事件 | `online 变化、uptime reset、CPU/内存摘要跨阈值、runtime 状态变化、runtime recovery 请求或完成。` | 更新 UI 或调用对应 get method 校准 | draft |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `system.state` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `system.state` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `system.state` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
