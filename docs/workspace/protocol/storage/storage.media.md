...
`storage.media` 用于媒体文件索引、媒体资源元信息和媒体清理。
...
| 类型 | 内容 |
|---|---|
| 包含 | storage.media 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 storage.media 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 storage.media 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `storage.getMediaCapabilities` | query | 查询 storage.media 能力范围 | `GetMediaCapabilitiesParams` | `GetMediaCapabilitiesResult` | 否 | draft |
| `storage.getMediaConfig` | query | 查询 storage.media 配置 | `GetMediaConfigParams` | `GetMediaConfigResult` | 否 | draft |
| `storage.setMediaConfig` | command | 设置 storage.media 配置 | `SetMediaConfigParams` | `SetMediaConfigResult` | 是，`storage.mediaConfigChanged` | draft |
| `storage.resetMediaConfig` | action | 恢复 storage.media 默认配置 | `ResetMediaConfigParams` | `ResetMediaConfigResult` | 是，`storage.mediaConfigChanged` | draft |

### 3.1 `storage.getMediaCapabilities`

**用途**：查询 storage.media 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetMediaCapabilitiesParams` |
| Result Schema | `GetMediaCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetMediaCapabilitiesParams`
...
{
  "id": 101,
  "method": "storage.getMediaCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetMediaCapabilitiesResult`
...
### 3.2 `storage.getMediaConfig`

**用途**：查询 storage.media 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetMediaConfigParams` |
| Result Schema | `GetMediaConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetMediaConfigParams`
...
#### 3.2.3 返回结果 Result：`GetMediaConfigResult`
...
### 3.3 `storage.setMediaConfig`

**用途**：设置 storage.media 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetMediaConfigParams` |
| Result Schema | `SetMediaConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.mediaConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetMediaConfigParams`
...
#### 3.3.3 返回结果 Result：`SetMediaConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `storage.mediaConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `MediaConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "storage.mediaConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `storage.resetMediaConfig`

**用途**：恢复 storage.media 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetMediaConfigParams` |
| Result Schema | `ResetMediaConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.mediaConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetMediaConfigParams`
...
{
  "id": 104,
  "method": "storage.resetMediaConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetMediaConfigResult`
...
|---|---|---|---|---|
| `storage.mediaConfigChanged` | storage.media 配置变化 | `MediaConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `storage.mediaConfigChanged`

**触发条件**：storage.media 配置变化。

#### 4.1.1 Payload：`MediaConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `storage.media` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetMediaCapabilitiesParams` | `storage.getMediaCapabilities` request params | 见 `storage.getMediaCapabilities` 方法小节。 |
| `GetMediaCapabilitiesResult` | `storage.getMediaCapabilities` result | 见 `storage.getMediaCapabilities` 方法小节。 |
| `GetMediaConfigParams` | `storage.getMediaConfig` request params | 见 `storage.getMediaConfig` 方法小节。 |
| `GetMediaConfigResult` | `storage.getMediaConfig` result | 见 `storage.getMediaConfig` 方法小节。 |
| `SetMediaConfigParams` | `storage.setMediaConfig` request params | 见 `storage.setMediaConfig` 方法小节。 |
| `SetMediaConfigResult` | `storage.setMediaConfig` result | 见 `storage.setMediaConfig` 方法小节。 |
| `ResetMediaConfigParams` | `storage.resetMediaConfig` request params | 见 `storage.resetMediaConfig` 方法小节。 |
| `ResetMediaConfigResult` | `storage.resetMediaConfig` result | 见 `storage.resetMediaConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `MediaConfigChangedEvent` | `storage.mediaConfigChanged` | 见 `storage.mediaConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `MediaState` | 表达 `storage.media` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `MediaConfig` | 表达 `storage.media` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `storage.media` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `storage.media` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `storage.media` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
