...
| Schema | 用途 | 状态 |
|---|---|---|
| `RecordingState` | 表达 `storage.recording` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `RecordingConfig` | 表达 `storage.recording` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `storage.recording`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `storage.recording` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `storage.recording` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `storage.recording` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
