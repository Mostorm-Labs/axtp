...
`storage.volume` 用于存储卷、分区、挂载点和可用空间。
...
| 类型 | 内容 |
|---|---|
| 包含 | storage.volume 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 storage.volume 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 storage.volume 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `storage.getVolumeCapabilities` | query | 查询 storage.volume 能力范围 | `GetVolumeCapabilitiesParams` | `GetVolumeCapabilitiesResult` | 否 | draft |
| `storage.getVolumeConfig` | query | 查询 storage.volume 配置 | `GetVolumeConfigParams` | `GetVolumeConfigResult` | 否 | draft |
| `storage.setVolumeConfig` | command | 设置 storage.volume 配置 | `SetVolumeConfigParams` | `SetVolumeConfigResult` | 是，`storage.volumeConfigChanged` | draft |
| `storage.resetVolumeConfig` | action | 恢复 storage.volume 默认配置 | `ResetVolumeConfigParams` | `ResetVolumeConfigResult` | 是，`storage.volumeConfigChanged` | draft |

### 3.1 `storage.getVolumeCapabilities`

**用途**：查询 storage.volume 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetVolumeCapabilitiesParams` |
| Result Schema | `GetVolumeCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetVolumeCapabilitiesParams`
...
{
  "id": 101,
  "method": "storage.getVolumeCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetVolumeCapabilitiesResult`
...
### 3.2 `storage.getVolumeConfig`

**用途**：查询 storage.volume 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetVolumeConfigParams` |
| Result Schema | `GetVolumeConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetVolumeConfigParams`
...
{
  "id": 102,
  "method": "storage.getVolumeConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetVolumeConfigResult`
...
### 3.3 `storage.setVolumeConfig`

**用途**：设置 storage.volume 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetVolumeConfigParams` |
| Result Schema | `SetVolumeConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.volumeConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetVolumeConfigParams`
...
{
  "id": 103,
  "method": "storage.setVolumeConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetVolumeConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `storage.volumeConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `VolumeConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "storage.volumeConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `storage.resetVolumeConfig`

**用途**：恢复 storage.volume 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetVolumeConfigParams` |
| Result Schema | `ResetVolumeConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.volumeConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetVolumeConfigParams`
...
{
  "id": 104,
  "method": "storage.resetVolumeConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetVolumeConfigResult`
...
|---|---|---|---|---|
| `storage.volumeConfigChanged` | storage.volume 配置变化 | `VolumeConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `storage.volumeConfigChanged`

**触发条件**：storage.volume 配置变化。

#### 4.1.1 Payload：`VolumeConfigChangedEvent`
...
Capability name: `storage.volume`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `storage.volume` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetVolumeCapabilitiesParams` | `storage.getVolumeCapabilities` request params | 见 `storage.getVolumeCapabilities` 方法小节。 |
| `GetVolumeCapabilitiesResult` | `storage.getVolumeCapabilities` result | 见 `storage.getVolumeCapabilities` 方法小节。 |
| `GetVolumeConfigParams` | `storage.getVolumeConfig` request params | 见 `storage.getVolumeConfig` 方法小节。 |
| `GetVolumeConfigResult` | `storage.getVolumeConfig` result | 见 `storage.getVolumeConfig` 方法小节。 |
| `SetVolumeConfigParams` | `storage.setVolumeConfig` request params | 见 `storage.setVolumeConfig` 方法小节。 |
| `SetVolumeConfigResult` | `storage.setVolumeConfig` result | 见 `storage.setVolumeConfig` 方法小节。 |
| `ResetVolumeConfigParams` | `storage.resetVolumeConfig` request params | 见 `storage.resetVolumeConfig` 方法小节。 |
| `ResetVolumeConfigResult` | `storage.resetVolumeConfig` result | 见 `storage.resetVolumeConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `VolumeConfigChangedEvent` | `storage.volumeConfigChanged` | 见 `storage.volumeConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `VolumeState` | 表达 `storage.volume` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `VolumeConfig` | 表达 `storage.volume` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `storage.volume` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `storage.volume` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `storage.volume` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
