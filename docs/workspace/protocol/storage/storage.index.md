...
`storage.index` 用于存储索引构建、查询和索引状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | storage.index 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 storage.index 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 storage.index 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `storage.getIndexCapabilities` | query | 查询 storage.index 能力范围 | `GetIndexCapabilitiesParams` | `GetIndexCapabilitiesResult` | 否 | draft |
| `storage.getIndexConfig` | query | 查询 storage.index 配置 | `GetIndexConfigParams` | `GetIndexConfigResult` | 否 | draft |
| `storage.setIndexConfig` | command | 设置 storage.index 配置 | `SetIndexConfigParams` | `SetIndexConfigResult` | 是，`storage.indexConfigChanged` | draft |
| `storage.resetIndexConfig` | action | 恢复 storage.index 默认配置 | `ResetIndexConfigParams` | `ResetIndexConfigResult` | 是，`storage.indexConfigChanged` | draft |

### 3.1 `storage.getIndexCapabilities`

**用途**：查询 storage.index 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetIndexCapabilitiesParams` |
| Result Schema | `GetIndexCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetIndexCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetIndexCapabilitiesResult`
...
**用途**：查询 storage.index 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetIndexConfigParams` |
| Result Schema | `GetIndexConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetIndexConfigParams`
...
#### 3.2.3 返回结果 Result：`GetIndexConfigResult`
...
**用途**：设置 storage.index 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetIndexConfigParams` |
| Result Schema | `SetIndexConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.indexConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetIndexConfigParams`
...
#### 3.3.3 返回结果 Result：`SetIndexConfigResult`
...
**用途**：恢复 storage.index 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetIndexConfigParams` |
| Result Schema | `ResetIndexConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.indexConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetIndexConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetIndexConfigResult`
...
|---|---|---|---|---|
| `storage.indexConfigChanged` | storage.index 配置变化 | `IndexConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `storage.indexConfigChanged`

**触发条件**：storage.index 配置变化。

#### 4.1.1 Payload：`IndexConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `storage.index` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetIndexCapabilitiesParams` | `storage.getIndexCapabilities` request params | 见 `storage.getIndexCapabilities` 方法小节。 |
| `GetIndexCapabilitiesResult` | `storage.getIndexCapabilities` result | 见 `storage.getIndexCapabilities` 方法小节。 |
| `GetIndexConfigParams` | `storage.getIndexConfig` request params | 见 `storage.getIndexConfig` 方法小节。 |
| `GetIndexConfigResult` | `storage.getIndexConfig` result | 见 `storage.getIndexConfig` 方法小节。 |
| `SetIndexConfigParams` | `storage.setIndexConfig` request params | 见 `storage.setIndexConfig` 方法小节。 |
| `SetIndexConfigResult` | `storage.setIndexConfig` result | 见 `storage.setIndexConfig` 方法小节。 |
| `ResetIndexConfigParams` | `storage.resetIndexConfig` request params | 见 `storage.resetIndexConfig` 方法小节。 |
| `ResetIndexConfigResult` | `storage.resetIndexConfig` result | 见 `storage.resetIndexConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `IndexConfigChangedEvent` | `storage.indexConfigChanged` | 见 `storage.indexConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `IndexState` | 表达 `storage.index` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `IndexConfig` | 表达 `storage.index` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `storage.index` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `storage.index` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `storage.index` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
