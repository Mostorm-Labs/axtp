...
`storage.disk` 用于磁盘设备、容量、健康状态和挂载状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | storage.disk 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 storage.disk 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 storage.disk 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `storage.getDiskCapabilities` | query | 查询 storage.disk 能力范围 | `GetDiskCapabilitiesParams` | `GetDiskCapabilitiesResult` | 否 | draft |
| `storage.getDiskConfig` | query | 查询 storage.disk 配置 | `GetDiskConfigParams` | `GetDiskConfigResult` | 否 | draft |
| `storage.setDiskConfig` | command | 设置 storage.disk 配置 | `SetDiskConfigParams` | `SetDiskConfigResult` | 是，`storage.diskConfigChanged` | draft |
| `storage.resetDiskConfig` | action | 恢复 storage.disk 默认配置 | `ResetDiskConfigParams` | `ResetDiskConfigResult` | 是，`storage.diskConfigChanged` | draft |

### 3.1 `storage.getDiskCapabilities`

**用途**：查询 storage.disk 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetDiskCapabilitiesParams` |
| Result Schema | `GetDiskCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetDiskCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetDiskCapabilitiesResult`
...
### 3.2 `storage.getDiskConfig`

**用途**：查询 storage.disk 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetDiskConfigParams` |
| Result Schema | `GetDiskConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetDiskConfigParams`
...
#### 3.2.3 返回结果 Result：`GetDiskConfigResult`
...
### 3.3 `storage.setDiskConfig`

**用途**：设置 storage.disk 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetDiskConfigParams` |
| Result Schema | `SetDiskConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.diskConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetDiskConfigParams`
...
#### 3.3.3 返回结果 Result：`SetDiskConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `storage.diskConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `DiskConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "storage.diskConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `storage.resetDiskConfig`

**用途**：恢复 storage.disk 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetDiskConfigParams` |
| Result Schema | `ResetDiskConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `storage.diskConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetDiskConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetDiskConfigResult`
...
|---|---|---|---|---|
| `storage.diskConfigChanged` | storage.disk 配置变化 | `DiskConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `storage.diskConfigChanged`

**触发条件**：storage.disk 配置变化。

#### 4.1.1 Payload：`DiskConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `storage.disk` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetDiskCapabilitiesParams` | `storage.getDiskCapabilities` request params | 见 `storage.getDiskCapabilities` 方法小节。 |
| `GetDiskCapabilitiesResult` | `storage.getDiskCapabilities` result | 见 `storage.getDiskCapabilities` 方法小节。 |
| `GetDiskConfigParams` | `storage.getDiskConfig` request params | 见 `storage.getDiskConfig` 方法小节。 |
| `GetDiskConfigResult` | `storage.getDiskConfig` result | 见 `storage.getDiskConfig` 方法小节。 |
| `SetDiskConfigParams` | `storage.setDiskConfig` request params | 见 `storage.setDiskConfig` 方法小节。 |
| `SetDiskConfigResult` | `storage.setDiskConfig` result | 见 `storage.setDiskConfig` 方法小节。 |
| `ResetDiskConfigParams` | `storage.resetDiskConfig` request params | 见 `storage.resetDiskConfig` 方法小节。 |
| `ResetDiskConfigResult` | `storage.resetDiskConfig` result | 见 `storage.resetDiskConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `DiskConfigChangedEvent` | `storage.diskConfigChanged` | 见 `storage.diskConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `DiskState` | 表达 `storage.disk` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `DiskConfig` | 表达 `storage.disk` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `storage.disk` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `storage.disk` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `storage.disk` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
