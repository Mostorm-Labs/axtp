...
`camera.calibration` 用于摄像头校准、镜头标定和校准状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | camera.calibration 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 camera.calibration 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 camera.calibration 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `camera.getCalibrationCapabilities` | query | 查询 camera.calibration 能力范围 | `GetCalibrationCapabilitiesParams` | `GetCalibrationCapabilitiesResult` | 否 | draft |
| `camera.getCalibrationConfig` | query | 查询 camera.calibration 配置 | `GetCalibrationConfigParams` | `GetCalibrationConfigResult` | 否 | draft |
| `camera.setCalibrationConfig` | command | 设置 camera.calibration 配置 | `SetCalibrationConfigParams` | `SetCalibrationConfigResult` | 是，`camera.calibrationConfigChanged` | draft |
| `camera.resetCalibrationConfig` | action | 恢复 camera.calibration 默认配置 | `ResetCalibrationConfigParams` | `ResetCalibrationConfigResult` | 是，`camera.calibrationConfigChanged` | draft |

### 3.1 `camera.getCalibrationCapabilities`

**用途**：查询 camera.calibration 能力范围。
...
### 3.2 `camera.getCalibrationConfig`

**用途**：查询 camera.calibration 配置。
...
### 3.3 `camera.setCalibrationConfig`

**用途**：设置 camera.calibration 配置。
...
| 是否触发事件 | 是，状态实际变化后触发 `camera.calibrationConfigChanged`。 |
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `camera.calibrationConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `CalibrationConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "camera.calibrationConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `camera.resetCalibrationConfig`

**用途**：恢复 camera.calibration 默认配置。
...
|---|---|---|---|---|
| `camera.calibrationConfigChanged` | camera.calibration 配置变化 | `CalibrationConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `camera.calibrationConfigChanged`

**触发条件**：camera.calibration 配置变化。
...
Capability name: `camera.calibration`。
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetCalibrationCapabilitiesParams` | `camera.getCalibrationCapabilities` request params | 见 `camera.getCalibrationCapabilities` 方法小节。 |
| `GetCalibrationCapabilitiesResult` | `camera.getCalibrationCapabilities` result | 见 `camera.getCalibrationCapabilities` 方法小节。 |
| `GetCalibrationConfigParams` | `camera.getCalibrationConfig` request params | 见 `camera.getCalibrationConfig` 方法小节。 |
| `GetCalibrationConfigResult` | `camera.getCalibrationConfig` result | 见 `camera.getCalibrationConfig` 方法小节。 |
| `SetCalibrationConfigParams` | `camera.setCalibrationConfig` request params | 见 `camera.setCalibrationConfig` 方法小节。 |
| `SetCalibrationConfigResult` | `camera.setCalibrationConfig` result | 见 `camera.setCalibrationConfig` 方法小节。 |
| `ResetCalibrationConfigParams` | `camera.resetCalibrationConfig` request params | 见 `camera.resetCalibrationConfig` 方法小节。 |
| `ResetCalibrationConfigResult` | `camera.resetCalibrationConfig` result | 见 `camera.resetCalibrationConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `CalibrationConfigChangedEvent` | `camera.calibrationConfigChanged` | 见 `camera.calibrationConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `CalibrationState` | 表达 `camera.calibration` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `CalibrationConfig` | 表达 `camera.calibration` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `camera.calibration`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `camera.calibration` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `camera.calibration` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `camera.calibration` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
