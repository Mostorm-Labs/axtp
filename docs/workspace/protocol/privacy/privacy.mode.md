...
| 类型 | 内容 |
|---|---|
| 包含 | privacy.mode 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 privacy.mode 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 privacy.mode 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `privacy.getModeCapabilities` | query | 查询 privacy.mode 能力范围 | `GetModeCapabilitiesParams` | `GetModeCapabilitiesResult` | 否 | draft |
| `privacy.getModeConfig` | query | 查询 privacy.mode 配置 | `GetModeConfigParams` | `GetModeConfigResult` | 否 | draft |
| `privacy.setModeConfig` | command | 设置 privacy.mode 配置 | `SetModeConfigParams` | `SetModeConfigResult` | 是，`privacy.modeConfigChanged` | draft |
| `privacy.resetModeConfig` | action | 恢复 privacy.mode 默认配置 | `ResetModeConfigParams` | `ResetModeConfigResult` | 是，`privacy.modeConfigChanged` | draft |

### 3.1 `privacy.getModeCapabilities`

**用途**：查询 privacy.mode 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetModeCapabilitiesParams` |
| Result Schema | `GetModeCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetModeCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetModeCapabilitiesResult`
...
### 3.2 `privacy.getModeConfig`

**用途**：查询 privacy.mode 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetModeConfigParams` |
| Result Schema | `GetModeConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetModeConfigParams`
...
#### 3.2.3 返回结果 Result：`GetModeConfigResult`
...
### 3.3 `privacy.setModeConfig`

**用途**：设置 privacy.mode 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetModeConfigParams` |
| Result Schema | `SetModeConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `privacy.modeConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetModeConfigParams`
...
#### 3.3.3 返回结果 Result：`SetModeConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `privacy.modeConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `ModeConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "privacy.modeConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `privacy.resetModeConfig`

**用途**：恢复 privacy.mode 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetModeConfigParams` |
| Result Schema | `ResetModeConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `privacy.modeConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetModeConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetModeConfigResult`
...
|---|---|---|---|---|
| `privacy.modeConfigChanged` | privacy.mode 配置变化 | `ModeConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `privacy.modeConfigChanged`

**触发条件**：privacy.mode 配置变化。

#### 4.1.1 Payload：`ModeConfigChangedEvent`
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetModeCapabilitiesParams` | `privacy.getModeCapabilities` request params | 见 `privacy.getModeCapabilities` 方法小节。 |
| `GetModeCapabilitiesResult` | `privacy.getModeCapabilities` result | 见 `privacy.getModeCapabilities` 方法小节。 |
| `GetModeConfigParams` | `privacy.getModeConfig` request params | 见 `privacy.getModeConfig` 方法小节。 |
| `GetModeConfigResult` | `privacy.getModeConfig` result | 见 `privacy.getModeConfig` 方法小节。 |
| `SetModeConfigParams` | `privacy.setModeConfig` request params | 见 `privacy.setModeConfig` 方法小节。 |
| `SetModeConfigResult` | `privacy.setModeConfig` result | 见 `privacy.setModeConfig` 方法小节。 |
| `ResetModeConfigParams` | `privacy.resetModeConfig` request params | 见 `privacy.resetModeConfig` 方法小节。 |
| `ResetModeConfigResult` | `privacy.resetModeConfig` result | 见 `privacy.resetModeConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `ModeConfigChangedEvent` | `privacy.modeConfigChanged` | 见 `privacy.modeConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `ModeState` | 表达 `privacy.mode` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `ModeConfig` | 表达 `privacy.mode` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `privacy.mode` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `privacy.mode` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `privacy.mode` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
