...
| 类型 | 内容 |
|---|---|
| 包含 | privacy.state 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 privacy.state 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 privacy.state 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `privacy.getStateCapabilities` | query | 查询 privacy.state 能力范围 | `GetStateCapabilitiesParams` | `GetStateCapabilitiesResult` | 否 | draft |
| `privacy.getState` | query | 查询 privacy.state 状态 | `GetStateParams` | `GetStateResult` | 否 | draft |

### 3.1 `privacy.getStateCapabilities`

**用途**：查询 privacy.state 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetStateCapabilitiesParams` |
| Result Schema | `GetStateCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetStateCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetStateCapabilitiesResult`
...
### 3.2 `privacy.getState`

**用途**：查询 privacy.state 状态。
...
#### 3.2.1 请求参数 Params：`GetStateParams`
...
#### 3.2.3 返回结果 Result：`GetStateResult`
...
|---|---|---|---|---|
| `privacy.stateChanged` | privacy.state 状态变化 | `StateChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `privacy.stateChanged`

**触发条件**：privacy.state 状态变化。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `privacy.state` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetStateCapabilitiesParams` | `privacy.getStateCapabilities` request params | 见 `privacy.getStateCapabilities` 方法小节。 |
| `GetStateCapabilitiesResult` | `privacy.getStateCapabilities` result | 见 `privacy.getStateCapabilities` 方法小节。 |
| `GetStateParams` | `privacy.getState` request params | 见 `privacy.getState` 方法小节。 |
| `GetStateResult` | `privacy.getState` result | 见 `privacy.getState` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `StateChangedEvent` | `privacy.stateChanged` | 见 `privacy.stateChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `StateState` | 表达 `privacy.state` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `StateConfig` | 表达 `privacy.state` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `privacy.state` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `privacy.state` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `privacy.state` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
