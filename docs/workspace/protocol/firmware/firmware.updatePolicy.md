...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | firmware.updatePolicy 能力的配置、状态和事件。 |
| 当前状态 | draft |
...
`firmware.updatePolicy` 用于firmware.updatePolicy 能力的配置、状态和事件。
...
| 类型 | 内容 |
|---|---|
| 包含 | firmware.updatePolicy 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 firmware.updatePolicy 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 firmware.updatePolicy 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `firmware.getUpdatePolicyConfig` | query | 查询升级策略配置 | `GetUpdatePolicyConfigParams` | `GetUpdatePolicyConfigResult` | 否 | draft |
| `firmware.setUpdatePolicyConfig` | command | 设置升级策略配置 | `SetUpdatePolicyConfigParams` | `SetUpdatePolicyConfigResult` | 是，`firmware.updatePolicyConfigChanged` | draft |

### 3.1 `firmware.getUpdatePolicyConfig`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetUpdatePolicyConfigParams` |
| Result Schema | `GetUpdatePolicyConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetUpdatePolicyConfigParams`
...
#### 3.1.3 返回结果 Result：`GetUpdatePolicyConfigResult`
...
### 3.2 `firmware.setUpdatePolicyConfig`
...
|---|---|
| 调用类型 | command |
| Params Schema | `SetUpdatePolicyConfigParams` |
| Result Schema | `SetUpdatePolicyConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `firmware.updatePolicyConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.2.1 请求参数 Params：`SetUpdatePolicyConfigParams`
...
#### 3.2.3 返回结果 Result：`SetUpdatePolicyConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `firmware.updatePolicyConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `UpdatePolicyConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |
...
|---|---|---|---|---|
| `firmware.updatePolicyConfigChanged` | 升级策略配置变化 | `UpdatePolicyConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `firmware.updatePolicyConfigChanged`

**触发条件**：升级策略配置变化。

#### 4.1.1 Payload：`UpdatePolicyConfigChangedEvent`
...
Capability name: `firmware.updatePolicy`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `firmware.updatePolicy` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetUpdatePolicyConfigParams` | `firmware.getUpdatePolicyConfig` request params | 见 `firmware.getUpdatePolicyConfig` 方法小节。 |
| `GetUpdatePolicyConfigResult` | `firmware.getUpdatePolicyConfig` result | 见 `firmware.getUpdatePolicyConfig` 方法小节。 |
| `SetUpdatePolicyConfigParams` | `firmware.setUpdatePolicyConfig` request params | 见 `firmware.setUpdatePolicyConfig` 方法小节。 |
| `SetUpdatePolicyConfigResult` | `firmware.setUpdatePolicyConfig` result | 见 `firmware.setUpdatePolicyConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `UpdatePolicyConfigChangedEvent` | `firmware.updatePolicyConfigChanged` | 见 `firmware.updatePolicyConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `UpdatePolicyState` | 表达 `firmware.updatePolicy` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `UpdatePolicyConfig` | 表达 `firmware.updatePolicy` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `firmware.updatePolicy`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `firmware.updatePolicy` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `firmware.updatePolicy` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `firmware.updatePolicy` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
