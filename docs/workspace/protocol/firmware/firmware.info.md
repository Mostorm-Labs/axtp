...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | firmware.info 能力的配置、状态和事件。 |
| 当前状态 | draft |
...
`firmware.info` 用于firmware.info 能力的配置、状态和事件。
...
| 类型 | 内容 |
|---|---|
| 包含 | firmware.info 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 firmware.info 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 firmware.info 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `firmware.getInfo` | query | 查询固件版本和组件信息 | `GetInfoParams` | `GetInfoResult` | 否 | draft |

### 3.1 `firmware.getInfo`
...
|---|---|---|---|---|
| `firmware.infoChanged` | 固件信息变化 | `InfoChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `firmware.infoChanged`

**触发条件**：固件信息变化。

#### 4.1.1 Payload：`InfoChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `firmware.info` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetInfoParams` | `firmware.getInfo` request params | 见 `firmware.getInfo` 方法小节。 |
| `GetInfoResult` | `firmware.getInfo` result | 见 `firmware.getInfo` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `InfoChangedEvent` | `firmware.infoChanged` | 见 `firmware.infoChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `InfoState` | 表达 `firmware.info` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `InfoConfig` | 表达 `firmware.info` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `firmware.info` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `firmware.info` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `firmware.info` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
