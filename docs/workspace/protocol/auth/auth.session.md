...
`auth.session` 用于认证会话、登录状态和会话生命周期。
...
| 类型 | 内容 |
|---|---|
| 包含 | auth.session 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 auth.session 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 auth.session 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `auth.getSessionCapabilities` | query | 查询 auth.session 能力范围 | `GetSessionCapabilitiesParams` | `GetSessionCapabilitiesResult` | 否 | draft |
| `auth.getSessionConfig` | query | 查询 auth.session 配置 | `GetSessionConfigParams` | `GetSessionConfigResult` | 否 | draft |
| `auth.setSessionConfig` | command | 设置 auth.session 配置 | `SetSessionConfigParams` | `SetSessionConfigResult` | 是，`auth.sessionConfigChanged` | draft |
| `auth.resetSessionConfig` | action | 恢复 auth.session 默认配置 | `ResetSessionConfigParams` | `ResetSessionConfigResult` | 是，`auth.sessionConfigChanged` | draft |

### 3.1 `auth.getSessionCapabilities`

**用途**：查询 auth.session 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSessionCapabilitiesParams` |
| Result Schema | `GetSessionCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetSessionCapabilitiesParams`
...
{
  "id": 101,
  "method": "auth.getSessionCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetSessionCapabilitiesResult`
...
### 3.2 `auth.getSessionConfig`

**用途**：查询 auth.session 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSessionConfigParams` |
| Result Schema | `GetSessionConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetSessionConfigParams`
...
#### 3.2.3 返回结果 Result：`GetSessionConfigResult`
...
### 3.3 `auth.setSessionConfig`

**用途**：设置 auth.session 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetSessionConfigParams` |
| Result Schema | `SetSessionConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `auth.sessionConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetSessionConfigParams`
...
#### 3.3.3 返回结果 Result：`SetSessionConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `auth.sessionConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `SessionConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "auth.sessionConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `auth.resetSessionConfig`

**用途**：恢复 auth.session 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetSessionConfigParams` |
| Result Schema | `ResetSessionConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `auth.sessionConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetSessionConfigParams`
...
{
  "id": 104,
  "method": "auth.resetSessionConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetSessionConfigResult`
...
|---|---|---|---|---|
| `auth.sessionConfigChanged` | auth.session 配置变化 | `SessionConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `auth.sessionConfigChanged`

**触发条件**：auth.session 配置变化。

#### 4.1.1 Payload：`SessionConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `auth.session` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetSessionCapabilitiesParams` | `auth.getSessionCapabilities` request params | 见 `auth.getSessionCapabilities` 方法小节。 |
| `GetSessionCapabilitiesResult` | `auth.getSessionCapabilities` result | 见 `auth.getSessionCapabilities` 方法小节。 |
| `GetSessionConfigParams` | `auth.getSessionConfig` request params | 见 `auth.getSessionConfig` 方法小节。 |
| `GetSessionConfigResult` | `auth.getSessionConfig` result | 见 `auth.getSessionConfig` 方法小节。 |
| `SetSessionConfigParams` | `auth.setSessionConfig` request params | 见 `auth.setSessionConfig` 方法小节。 |
| `SetSessionConfigResult` | `auth.setSessionConfig` result | 见 `auth.setSessionConfig` 方法小节。 |
| `ResetSessionConfigParams` | `auth.resetSessionConfig` request params | 见 `auth.resetSessionConfig` 方法小节。 |
| `ResetSessionConfigResult` | `auth.resetSessionConfig` result | 见 `auth.resetSessionConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `SessionConfigChangedEvent` | `auth.sessionConfigChanged` | 见 `auth.sessionConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `SessionState` | 表达 `auth.session` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `SessionConfig` | 表达 `auth.session` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `auth.session` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `auth.session` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `auth.session` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
