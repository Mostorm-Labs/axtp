...
#### 3.1.3 返回结果 Result：`GetProfilesResult`
...
#### 3.2.3 返回结果 Result：`GetProfileCapabilitiesResult`
...
| Schema | 用途 | 状态 |
|---|---|---|
| `ProfileState` | 表达 `stream.profile` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `ProfileConfig` | 表达 `stream.profile` 的可写配置。 | `[REVIEW-ASK]` |
...
本 feature 当前草案未定义事件；客户端需要按需调用 query method 校准状态。
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `stream.profile` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `stream.profile` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `stream.profile` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
