...
| 类型 | 内容 |
|---|---|
| 包含 | room.source 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 room.source 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 room.source 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `room.getSourceCapabilities` | query | 查询 room.source 能力范围 | `GetSourceCapabilitiesParams` | `GetSourceCapabilitiesResult` | 否 | draft |
| `room.getSourceConfig` | query | 查询 room.source 配置 | `GetSourceConfigParams` | `GetSourceConfigResult` | 否 | draft |
| `room.setSourceConfig` | command | 设置 room.source 配置 | `SetSourceConfigParams` | `SetSourceConfigResult` | 是，`room.sourceConfigChanged` | draft |
| `room.resetSourceConfig` | action | 恢复 room.source 默认配置 | `ResetSourceConfigParams` | `ResetSourceConfigResult` | 是，`room.sourceConfigChanged` | draft |

### 3.1 `room.getSourceCapabilities`

**用途**：查询 room.source 能力范围。
...
### 3.2 `room.getSourceConfig`

**用途**：查询 room.source 配置。
...
### 3.3 `room.setSourceConfig`

**用途**：设置 room.source 配置。
...
| 是否触发事件 | 是，状态实际变化后触发 `room.sourceConfigChanged`。 |
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `room.sourceConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `SourceConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "room.sourceConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `room.resetSourceConfig`

**用途**：恢复 room.source 默认配置。
...
|---|---|---|---|---|
| `room.sourceConfigChanged` | room.source 配置变化 | `SourceConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `room.sourceConfigChanged`

**触发条件**：room.source 配置变化。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `room.source` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetSourceCapabilitiesParams` | `room.getSourceCapabilities` request params | 见 `room.getSourceCapabilities` 方法小节。 |
| `GetSourceCapabilitiesResult` | `room.getSourceCapabilities` result | 见 `room.getSourceCapabilities` 方法小节。 |
| `GetSourceConfigParams` | `room.getSourceConfig` request params | 见 `room.getSourceConfig` 方法小节。 |
| `GetSourceConfigResult` | `room.getSourceConfig` result | 见 `room.getSourceConfig` 方法小节。 |
| `SetSourceConfigParams` | `room.setSourceConfig` request params | 见 `room.setSourceConfig` 方法小节。 |
| `SetSourceConfigResult` | `room.setSourceConfig` result | 见 `room.setSourceConfig` 方法小节。 |
| `ResetSourceConfigParams` | `room.resetSourceConfig` request params | 见 `room.resetSourceConfig` 方法小节。 |
| `ResetSourceConfigResult` | `room.resetSourceConfig` result | 见 `room.resetSourceConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `SourceConfigChangedEvent` | `room.sourceConfigChanged` | 见 `room.sourceConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `SourceState` | 表达 `room.source` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `SourceConfig` | 表达 `room.source` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `room.source` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `room.source` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `room.source` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
