...
`room.participant` 用于参会人、参会设备和参与者状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | room.participant 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 room.participant 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 room.participant 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `room.getParticipantCapabilities` | query | 查询 room.participant 能力范围 | `GetParticipantCapabilitiesParams` | `GetParticipantCapabilitiesResult` | 否 | draft |
| `room.getParticipantConfig` | query | 查询 room.participant 配置 | `GetParticipantConfigParams` | `GetParticipantConfigResult` | 否 | draft |
| `room.setParticipantConfig` | command | 设置 room.participant 配置 | `SetParticipantConfigParams` | `SetParticipantConfigResult` | 是，`room.participantConfigChanged` | draft |
| `room.resetParticipantConfig` | action | 恢复 room.participant 默认配置 | `ResetParticipantConfigParams` | `ResetParticipantConfigResult` | 是，`room.participantConfigChanged` | draft |

### 3.1 `room.getParticipantCapabilities`

**用途**：查询 room.participant 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetParticipantCapabilitiesParams` |
| Result Schema | `GetParticipantCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetParticipantCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetParticipantCapabilitiesResult`
...
### 3.2 `room.getParticipantConfig`

**用途**：查询 room.participant 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetParticipantConfigParams` |
| Result Schema | `GetParticipantConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetParticipantConfigParams`
...
#### 3.2.3 返回结果 Result：`GetParticipantConfigResult`
...
### 3.3 `room.setParticipantConfig`

**用途**：设置 room.participant 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetParticipantConfigParams` |
| Result Schema | `SetParticipantConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `room.participantConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetParticipantConfigParams`
...
#### 3.3.3 返回结果 Result：`SetParticipantConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `room.participantConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `ParticipantConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "room.participantConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `room.resetParticipantConfig`

**用途**：恢复 room.participant 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetParticipantConfigParams` |
| Result Schema | `ResetParticipantConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `room.participantConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetParticipantConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetParticipantConfigResult`
...
|---|---|---|---|---|
| `room.participantConfigChanged` | room.participant 配置变化 | `ParticipantConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `room.participantConfigChanged`

**触发条件**：room.participant 配置变化。

#### 4.1.1 Payload：`ParticipantConfigChangedEvent`
...
Capability name: `room.participant`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `room.participant` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetParticipantCapabilitiesParams` | `room.getParticipantCapabilities` request params | 见 `room.getParticipantCapabilities` 方法小节。 |
| `GetParticipantCapabilitiesResult` | `room.getParticipantCapabilities` result | 见 `room.getParticipantCapabilities` 方法小节。 |
| `GetParticipantConfigParams` | `room.getParticipantConfig` request params | 见 `room.getParticipantConfig` 方法小节。 |
| `GetParticipantConfigResult` | `room.getParticipantConfig` result | 见 `room.getParticipantConfig` 方法小节。 |
| `SetParticipantConfigParams` | `room.setParticipantConfig` request params | 见 `room.setParticipantConfig` 方法小节。 |
| `SetParticipantConfigResult` | `room.setParticipantConfig` result | 见 `room.setParticipantConfig` 方法小节。 |
| `ResetParticipantConfigParams` | `room.resetParticipantConfig` request params | 见 `room.resetParticipantConfig` 方法小节。 |
| `ResetParticipantConfigResult` | `room.resetParticipantConfig` result | 见 `room.resetParticipantConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `ParticipantConfigChangedEvent` | `room.participantConfigChanged` | 见 `room.participantConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `ParticipantState` | 表达 `room.participant` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `ParticipantConfig` | 表达 `room.participant` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `room.participant` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `room.participant` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `room.participant` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
