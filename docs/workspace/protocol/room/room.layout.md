...
| 类型 | 内容 |
|---|---|
| 包含 | room.layout 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 room.layout 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 room.layout 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `room.getLayoutCapabilities` | query | 查询 room.layout 能力范围 | `GetLayoutCapabilitiesParams` | `GetLayoutCapabilitiesResult` | 否 | draft |
| `room.getLayoutConfig` | query | 查询 room.layout 配置 | `GetLayoutConfigParams` | `GetLayoutConfigResult` | 否 | draft |
| `room.setLayoutConfig` | command | 设置 room.layout 配置 | `SetLayoutConfigParams` | `SetLayoutConfigResult` | 是，`room.layoutConfigChanged` | draft |
| `room.resetLayoutConfig` | action | 恢复 room.layout 默认配置 | `ResetLayoutConfigParams` | `ResetLayoutConfigResult` | 是，`room.layoutConfigChanged` | draft |

### 3.1 `room.getLayoutCapabilities`

**用途**：查询 room.layout 能力范围。
...
### 3.2 `room.getLayoutConfig`

**用途**：查询 room.layout 配置。
...
### 3.3 `room.setLayoutConfig`

**用途**：设置 room.layout 配置。
...
| 是否触发事件 | 是，状态实际变化后触发 `room.layoutConfigChanged`。 |
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `room.layoutConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `LayoutConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "room.layoutConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `room.resetLayoutConfig`

**用途**：恢复 room.layout 默认配置。
...
|---|---|---|---|---|
| `room.layoutConfigChanged` | room.layout 配置变化 | `LayoutConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `room.layoutConfigChanged`

**触发条件**：room.layout 配置变化。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `room.layout` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetLayoutCapabilitiesParams` | `room.getLayoutCapabilities` request params | 见 `room.getLayoutCapabilities` 方法小节。 |
| `GetLayoutCapabilitiesResult` | `room.getLayoutCapabilities` result | 见 `room.getLayoutCapabilities` 方法小节。 |
| `GetLayoutConfigParams` | `room.getLayoutConfig` request params | 见 `room.getLayoutConfig` 方法小节。 |
| `GetLayoutConfigResult` | `room.getLayoutConfig` result | 见 `room.getLayoutConfig` 方法小节。 |
| `SetLayoutConfigParams` | `room.setLayoutConfig` request params | 见 `room.setLayoutConfig` 方法小节。 |
| `SetLayoutConfigResult` | `room.setLayoutConfig` result | 见 `room.setLayoutConfig` 方法小节。 |
| `ResetLayoutConfigParams` | `room.resetLayoutConfig` request params | 见 `room.resetLayoutConfig` 方法小节。 |
| `ResetLayoutConfigResult` | `room.resetLayoutConfig` result | 见 `room.resetLayoutConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `LayoutConfigChangedEvent` | `room.layoutConfigChanged` | 见 `room.layoutConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `LayoutState` | 表达 `room.layout` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `LayoutConfig` | 表达 `room.layout` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `room.layout` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `room.layout` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `room.layout` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
