...
`room.schedule` 用于会议日程、预约信息和日程状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | room.schedule 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 room.schedule 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 room.schedule 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `room.getScheduleCapabilities` | query | 查询 room.schedule 能力范围 | `GetScheduleCapabilitiesParams` | `GetScheduleCapabilitiesResult` | 否 | draft |
| `room.getScheduleConfig` | query | 查询 room.schedule 配置 | `GetScheduleConfigParams` | `GetScheduleConfigResult` | 否 | draft |
| `room.setScheduleConfig` | command | 设置 room.schedule 配置 | `SetScheduleConfigParams` | `SetScheduleConfigResult` | 是，`room.scheduleConfigChanged` | draft |
| `room.resetScheduleConfig` | action | 恢复 room.schedule 默认配置 | `ResetScheduleConfigParams` | `ResetScheduleConfigResult` | 是，`room.scheduleConfigChanged` | draft |

### 3.1 `room.getScheduleCapabilities`

**用途**：查询 room.schedule 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetScheduleCapabilitiesParams` |
| Result Schema | `GetScheduleCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetScheduleCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetScheduleCapabilitiesResult`
...
### 3.2 `room.getScheduleConfig`

**用途**：查询 room.schedule 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetScheduleConfigParams` |
| Result Schema | `GetScheduleConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetScheduleConfigParams`
...
#### 3.2.3 返回结果 Result：`GetScheduleConfigResult`
...
### 3.3 `room.setScheduleConfig`

**用途**：设置 room.schedule 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetScheduleConfigParams` |
| Result Schema | `SetScheduleConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `room.scheduleConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetScheduleConfigParams`
...
#### 3.3.3 返回结果 Result：`SetScheduleConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `room.scheduleConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `ScheduleConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "room.scheduleConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `room.resetScheduleConfig`

**用途**：恢复 room.schedule 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetScheduleConfigParams` |
| Result Schema | `ResetScheduleConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `room.scheduleConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetScheduleConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetScheduleConfigResult`
...
|---|---|---|---|---|
| `room.scheduleConfigChanged` | room.schedule 配置变化 | `ScheduleConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `room.scheduleConfigChanged`

**触发条件**：room.schedule 配置变化。

#### 4.1.1 Payload：`ScheduleConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `room.schedule` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetScheduleCapabilitiesParams` | `room.getScheduleCapabilities` request params | 见 `room.getScheduleCapabilities` 方法小节。 |
| `GetScheduleCapabilitiesResult` | `room.getScheduleCapabilities` result | 见 `room.getScheduleCapabilities` 方法小节。 |
| `GetScheduleConfigParams` | `room.getScheduleConfig` request params | 见 `room.getScheduleConfig` 方法小节。 |
| `GetScheduleConfigResult` | `room.getScheduleConfig` result | 见 `room.getScheduleConfig` 方法小节。 |
| `SetScheduleConfigParams` | `room.setScheduleConfig` request params | 见 `room.setScheduleConfig` 方法小节。 |
| `SetScheduleConfigResult` | `room.setScheduleConfig` result | 见 `room.setScheduleConfig` 方法小节。 |
| `ResetScheduleConfigParams` | `room.resetScheduleConfig` request params | 见 `room.resetScheduleConfig` 方法小节。 |
| `ResetScheduleConfigResult` | `room.resetScheduleConfig` result | 见 `room.resetScheduleConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `ScheduleConfigChangedEvent` | `room.scheduleConfigChanged` | 见 `room.scheduleConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `ScheduleState` | 表达 `room.schedule` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `ScheduleConfig` | 表达 `room.schedule` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `room.schedule` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `room.schedule` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `room.schedule` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
