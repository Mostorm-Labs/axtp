...
generated: false
domain: diagnostic
feature: diagnostic.networkTest
registry:
...
`diagnostic.networkTest` 用于网络连通性、吞吐、BQB 或网络相关测试。
...
| 类型 | 内容 |
|---|---|
| 包含 | diagnostic.networkTest 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 diagnostic.networkTest 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 diagnostic.networkTest 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `diagnostic.getNetworkTestCapabilities` | query | 查询 diagnostic.networkTest 能力范围 | `GetNetworkTestCapabilitiesParams` | `GetNetworkTestCapabilitiesResult` | 否 | draft |
| `diagnostic.getNetworkTestConfig` | query | 查询 diagnostic.networkTest 配置 | `GetNetworkTestConfigParams` | `GetNetworkTestConfigResult` | 否 | draft |
| `diagnostic.setNetworkTestConfig` | command | 设置 diagnostic.networkTest 配置 | `SetNetworkTestConfigParams` | `SetNetworkTestConfigResult` | 是，`diagnostic.networkTestConfigChanged` | draft |
| `diagnostic.resetNetworkTestConfig` | action | 恢复 diagnostic.networkTest 默认配置 | `ResetNetworkTestConfigParams` | `ResetNetworkTestConfigResult` | 是，`diagnostic.networkTestConfigChanged` | draft |

### 3.1 `diagnostic.getNetworkTestCapabilities`

**用途**：查询 diagnostic.networkTest 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetNetworkTestCapabilitiesParams` |
| Result Schema | `GetNetworkTestCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetNetworkTestCapabilitiesParams`
...
{
  "id": 101,
  "method": "diagnostic.getNetworkTestCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetNetworkTestCapabilitiesResult`
...
### 3.2 `diagnostic.getNetworkTestConfig`

**用途**：查询 diagnostic.networkTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetNetworkTestConfigParams` |
| Result Schema | `GetNetworkTestConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetNetworkTestConfigParams`
...
{
  "id": 102,
  "method": "diagnostic.getNetworkTestConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetNetworkTestConfigResult`
...
### 3.3 `diagnostic.setNetworkTestConfig`

**用途**：设置 diagnostic.networkTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetNetworkTestConfigParams` |
| Result Schema | `SetNetworkTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.networkTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetNetworkTestConfigParams`
...
{
  "id": 103,
  "method": "diagnostic.setNetworkTestConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetNetworkTestConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `diagnostic.networkTestConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `NetworkTestConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "diagnostic.networkTestConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `diagnostic.resetNetworkTestConfig`

**用途**：恢复 diagnostic.networkTest 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetNetworkTestConfigParams` |
| Result Schema | `ResetNetworkTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.networkTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetNetworkTestConfigParams`
...
{
  "id": 104,
  "method": "diagnostic.resetNetworkTestConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetNetworkTestConfigResult`
...
|---|---|---|---|---|
| `diagnostic.networkTestConfigChanged` | diagnostic.networkTest 配置变化 | `NetworkTestConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `diagnostic.networkTestConfigChanged`

**触发条件**：diagnostic.networkTest 配置变化。

#### 4.1.1 Payload：`NetworkTestConfigChangedEvent`
...
Capability name: `diagnostic.networkTest`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `diagnostic.networkTest` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetNetworkTestCapabilitiesParams` | `diagnostic.getNetworkTestCapabilities` request params | 见 `diagnostic.getNetworkTestCapabilities` 方法小节。 |
| `GetNetworkTestCapabilitiesResult` | `diagnostic.getNetworkTestCapabilities` result | 见 `diagnostic.getNetworkTestCapabilities` 方法小节。 |
| `GetNetworkTestConfigParams` | `diagnostic.getNetworkTestConfig` request params | 见 `diagnostic.getNetworkTestConfig` 方法小节。 |
| `GetNetworkTestConfigResult` | `diagnostic.getNetworkTestConfig` result | 见 `diagnostic.getNetworkTestConfig` 方法小节。 |
| `SetNetworkTestConfigParams` | `diagnostic.setNetworkTestConfig` request params | 见 `diagnostic.setNetworkTestConfig` 方法小节。 |
| `SetNetworkTestConfigResult` | `diagnostic.setNetworkTestConfig` result | 见 `diagnostic.setNetworkTestConfig` 方法小节。 |
| `ResetNetworkTestConfigParams` | `diagnostic.resetNetworkTestConfig` request params | 见 `diagnostic.resetNetworkTestConfig` 方法小节。 |
| `ResetNetworkTestConfigResult` | `diagnostic.resetNetworkTestConfig` result | 见 `diagnostic.resetNetworkTestConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `NetworkTestConfigChangedEvent` | `diagnostic.networkTestConfigChanged` | 见 `diagnostic.networkTestConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `NetworkTestState` | 表达 `diagnostic.networkTest` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `NetworkTestConfig` | 表达 `diagnostic.networkTest` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.networkTest`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.networkTest` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.networkTest` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.networkTest` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
