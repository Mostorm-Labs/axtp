...
`diagnostic.inputTest` 用于按键、HID、GPIO 和输入通道测试。
...
| 类型 | 内容 |
|---|---|
| 包含 | diagnostic.inputTest 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 diagnostic.inputTest 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 diagnostic.inputTest 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `diagnostic.getInputTestCapabilities` | query | 查询 diagnostic.inputTest 能力范围 | `GetInputTestCapabilitiesParams` | `GetInputTestCapabilitiesResult` | 否 | draft |
| `diagnostic.getInputTestConfig` | query | 查询 diagnostic.inputTest 配置 | `GetInputTestConfigParams` | `GetInputTestConfigResult` | 否 | draft |
| `diagnostic.setInputTestConfig` | command | 设置 diagnostic.inputTest 配置 | `SetInputTestConfigParams` | `SetInputTestConfigResult` | 是，`diagnostic.inputTestConfigChanged` | draft |
| `diagnostic.resetInputTestConfig` | action | 恢复 diagnostic.inputTest 默认配置 | `ResetInputTestConfigParams` | `ResetInputTestConfigResult` | 是，`diagnostic.inputTestConfigChanged` | draft |

### 3.1 `diagnostic.getInputTestCapabilities`

**用途**：查询 diagnostic.inputTest 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetInputTestCapabilitiesParams` |
| Result Schema | `GetInputTestCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetInputTestCapabilitiesParams`
...
{
  "id": 101,
  "method": "diagnostic.getInputTestCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetInputTestCapabilitiesResult`
...
### 3.2 `diagnostic.getInputTestConfig`

**用途**：查询 diagnostic.inputTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetInputTestConfigParams` |
| Result Schema | `GetInputTestConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetInputTestConfigParams`
...
{
  "id": 102,
  "method": "diagnostic.getInputTestConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetInputTestConfigResult`
...
### 3.3 `diagnostic.setInputTestConfig`

**用途**：设置 diagnostic.inputTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetInputTestConfigParams` |
| Result Schema | `SetInputTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.inputTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetInputTestConfigParams`
...
{
  "id": 103,
  "method": "diagnostic.setInputTestConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetInputTestConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `diagnostic.inputTestConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `InputTestConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "diagnostic.inputTestConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `diagnostic.resetInputTestConfig`

**用途**：恢复 diagnostic.inputTest 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetInputTestConfigParams` |
| Result Schema | `ResetInputTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.inputTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetInputTestConfigParams`
...
{
  "id": 104,
  "method": "diagnostic.resetInputTestConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetInputTestConfigResult`
...
|---|---|---|---|---|
| `diagnostic.inputTestConfigChanged` | diagnostic.inputTest 配置变化 | `InputTestConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `diagnostic.inputTestConfigChanged`

**触发条件**：diagnostic.inputTest 配置变化。

#### 4.1.1 Payload：`InputTestConfigChangedEvent`
...
Capability name: `diagnostic.inputTest`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `diagnostic.inputTest` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetInputTestCapabilitiesParams` | `diagnostic.getInputTestCapabilities` request params | 见 `diagnostic.getInputTestCapabilities` 方法小节。 |
| `GetInputTestCapabilitiesResult` | `diagnostic.getInputTestCapabilities` result | 见 `diagnostic.getInputTestCapabilities` 方法小节。 |
| `GetInputTestConfigParams` | `diagnostic.getInputTestConfig` request params | 见 `diagnostic.getInputTestConfig` 方法小节。 |
| `GetInputTestConfigResult` | `diagnostic.getInputTestConfig` result | 见 `diagnostic.getInputTestConfig` 方法小节。 |
| `SetInputTestConfigParams` | `diagnostic.setInputTestConfig` request params | 见 `diagnostic.setInputTestConfig` 方法小节。 |
| `SetInputTestConfigResult` | `diagnostic.setInputTestConfig` result | 见 `diagnostic.setInputTestConfig` 方法小节。 |
| `ResetInputTestConfigParams` | `diagnostic.resetInputTestConfig` request params | 见 `diagnostic.resetInputTestConfig` 方法小节。 |
| `ResetInputTestConfigResult` | `diagnostic.resetInputTestConfig` result | 见 `diagnostic.resetInputTestConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `InputTestConfigChangedEvent` | `diagnostic.inputTestConfigChanged` | 见 `diagnostic.inputTestConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `InputTestState` | 表达 `diagnostic.inputTest` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `InputTestConfig` | 表达 `diagnostic.inputTest` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.inputTest`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.inputTest` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.inputTest` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.inputTest` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
