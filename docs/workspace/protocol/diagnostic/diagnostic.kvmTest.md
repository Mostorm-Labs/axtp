...
#### 3.1.3 返回结果 Result：`GetKvmTestCapabilitiesResult`
...
#### 3.2.3 返回结果 Result：`GetKvmTestConfigResult`
...
#### 3.3.3 返回结果 Result：`SetKvmTestConfigResult`
...
#### 3.4.3 返回结果 Result：`ResetKvmTestConfigResult`
...
| Schema | 用途 | 状态 |
|---|---|---|
| `KvmTestState` | 表达 `diagnostic.kvmTest` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `KvmTestConfig` | 表达 `diagnostic.kvmTest` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.kvmTest`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.kvmTest` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.kvmTest` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.kvmTest` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
