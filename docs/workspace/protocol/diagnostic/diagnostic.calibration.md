...
generated: false
domain: diagnostic
feature: diagnostic.calibration
registry:
...
`diagnostic.calibration` 用于设备校准、标定流程和校准结果。
...
| 类型 | 内容 |
|---|---|
| 包含 | diagnostic.calibration 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 diagnostic.calibration 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 diagnostic.calibration 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `diagnostic.getCalibrationCapabilities` | query | 查询 diagnostic.calibration 能力范围 | `GetCalibrationCapabilitiesParams` | `GetCalibrationCapabilitiesResult` | 否 | draft |
| `diagnostic.getCalibrationConfig` | query | 查询 diagnostic.calibration 配置 | `GetCalibrationConfigParams` | `GetCalibrationConfigResult` | 否 | draft |
| `diagnostic.setCalibrationConfig` | command | 设置 diagnostic.calibration 配置 | `SetCalibrationConfigParams` | `SetCalibrationConfigResult` | 是，`diagnostic.calibrationConfigChanged` | draft |
| `diagnostic.resetCalibrationConfig` | action | 恢复 diagnostic.calibration 默认配置 | `ResetCalibrationConfigParams` | `ResetCalibrationConfigResult` | 是，`diagnostic.calibrationConfigChanged` | draft |

### 3.1 `diagnostic.getCalibrationCapabilities`

**用途**：查询 diagnostic.calibration 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetCalibrationCapabilitiesParams` |
| Result Schema | `GetCalibrationCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetCalibrationCapabilitiesParams`
...
{
  "id": 101,
  "method": "diagnostic.getCalibrationCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetCalibrationCapabilitiesResult`
...
### 3.2 `diagnostic.getCalibrationConfig`

**用途**：查询 diagnostic.calibration 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetCalibrationConfigParams` |
| Result Schema | `GetCalibrationConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetCalibrationConfigParams`
...
{
  "id": 102,
  "method": "diagnostic.getCalibrationConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetCalibrationConfigResult`
...
### 3.3 `diagnostic.setCalibrationConfig`

**用途**：设置 diagnostic.calibration 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetCalibrationConfigParams` |
| Result Schema | `SetCalibrationConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.calibrationConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetCalibrationConfigParams`
...
{
  "id": 103,
  "method": "diagnostic.setCalibrationConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetCalibrationConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `diagnostic.calibrationConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `CalibrationConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "diagnostic.calibrationConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `diagnostic.resetCalibrationConfig`

**用途**：恢复 diagnostic.calibration 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetCalibrationConfigParams` |
| Result Schema | `ResetCalibrationConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.calibrationConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetCalibrationConfigParams`
...
{
  "id": 104,
  "method": "diagnostic.resetCalibrationConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetCalibrationConfigResult`
...
|---|---|---|---|---|
| `diagnostic.calibrationConfigChanged` | diagnostic.calibration 配置变化 | `CalibrationConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `diagnostic.calibrationConfigChanged`

**触发条件**：diagnostic.calibration 配置变化。

#### 4.1.1 Payload：`CalibrationConfigChangedEvent`
...
Capability name: `diagnostic.calibration`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `diagnostic.calibration` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetCalibrationCapabilitiesParams` | `diagnostic.getCalibrationCapabilities` request params | 见 `diagnostic.getCalibrationCapabilities` 方法小节。 |
| `GetCalibrationCapabilitiesResult` | `diagnostic.getCalibrationCapabilities` result | 见 `diagnostic.getCalibrationCapabilities` 方法小节。 |
| `GetCalibrationConfigParams` | `diagnostic.getCalibrationConfig` request params | 见 `diagnostic.getCalibrationConfig` 方法小节。 |
| `GetCalibrationConfigResult` | `diagnostic.getCalibrationConfig` result | 见 `diagnostic.getCalibrationConfig` 方法小节。 |
| `SetCalibrationConfigParams` | `diagnostic.setCalibrationConfig` request params | 见 `diagnostic.setCalibrationConfig` 方法小节。 |
| `SetCalibrationConfigResult` | `diagnostic.setCalibrationConfig` result | 见 `diagnostic.setCalibrationConfig` 方法小节。 |
| `ResetCalibrationConfigParams` | `diagnostic.resetCalibrationConfig` request params | 见 `diagnostic.resetCalibrationConfig` 方法小节。 |
| `ResetCalibrationConfigResult` | `diagnostic.resetCalibrationConfig` result | 见 `diagnostic.resetCalibrationConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `CalibrationConfigChangedEvent` | `diagnostic.calibrationConfigChanged` | 见 `diagnostic.calibrationConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `CalibrationState` | 表达 `diagnostic.calibration` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `CalibrationConfig` | 表达 `diagnostic.calibration` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.calibration`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.calibration` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.calibration` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.calibration` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
