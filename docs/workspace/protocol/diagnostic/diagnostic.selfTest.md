...
`diagnostic.selfTest` 用于设备自检任务、自检状态和自检结果。
...
| 类型 | 内容 |
|---|---|
| 包含 | diagnostic.selfTest 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 diagnostic.selfTest 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 diagnostic.selfTest 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `diagnostic.getSelfTestCapabilities` | query | 查询 diagnostic.selfTest 能力范围 | `GetSelfTestCapabilitiesParams` | `GetSelfTestCapabilitiesResult` | 否 | draft |
| `diagnostic.getSelfTestConfig` | query | 查询 diagnostic.selfTest 配置 | `GetSelfTestConfigParams` | `GetSelfTestConfigResult` | 否 | draft |
| `diagnostic.setSelfTestConfig` | command | 设置 diagnostic.selfTest 配置 | `SetSelfTestConfigParams` | `SetSelfTestConfigResult` | 是，`diagnostic.selfTestConfigChanged` | draft |
| `diagnostic.resetSelfTestConfig` | action | 恢复 diagnostic.selfTest 默认配置 | `ResetSelfTestConfigParams` | `ResetSelfTestConfigResult` | 是，`diagnostic.selfTestConfigChanged` | draft |

### 3.1 `diagnostic.getSelfTestCapabilities`

**用途**：查询 diagnostic.selfTest 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSelfTestCapabilitiesParams` |
| Result Schema | `GetSelfTestCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetSelfTestCapabilitiesParams`
...
{
  "id": 101,
  "method": "diagnostic.getSelfTestCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetSelfTestCapabilitiesResult`
...
### 3.2 `diagnostic.getSelfTestConfig`

**用途**：查询 diagnostic.selfTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetSelfTestConfigParams` |
| Result Schema | `GetSelfTestConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetSelfTestConfigParams`
...
{
  "id": 102,
  "method": "diagnostic.getSelfTestConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetSelfTestConfigResult`
...
### 3.3 `diagnostic.setSelfTestConfig`

**用途**：设置 diagnostic.selfTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetSelfTestConfigParams` |
| Result Schema | `SetSelfTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.selfTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetSelfTestConfigParams`
...
{
  "id": 103,
  "method": "diagnostic.setSelfTestConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetSelfTestConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `diagnostic.selfTestConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `SelfTestConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "diagnostic.selfTestConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `diagnostic.resetSelfTestConfig`

**用途**：恢复 diagnostic.selfTest 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetSelfTestConfigParams` |
| Result Schema | `ResetSelfTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.selfTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetSelfTestConfigParams`
...
{
  "id": 104,
  "method": "diagnostic.resetSelfTestConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetSelfTestConfigResult`
...
|---|---|---|---|---|
| `diagnostic.selfTestConfigChanged` | diagnostic.selfTest 配置变化 | `SelfTestConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `diagnostic.selfTestConfigChanged`

**触发条件**：diagnostic.selfTest 配置变化。

#### 4.1.1 Payload：`SelfTestConfigChangedEvent`
...
Capability name: `diagnostic.selfTest`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `diagnostic.selfTest` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetSelfTestCapabilitiesParams` | `diagnostic.getSelfTestCapabilities` request params | 见 `diagnostic.getSelfTestCapabilities` 方法小节。 |
| `GetSelfTestCapabilitiesResult` | `diagnostic.getSelfTestCapabilities` result | 见 `diagnostic.getSelfTestCapabilities` 方法小节。 |
| `GetSelfTestConfigParams` | `diagnostic.getSelfTestConfig` request params | 见 `diagnostic.getSelfTestConfig` 方法小节。 |
| `GetSelfTestConfigResult` | `diagnostic.getSelfTestConfig` result | 见 `diagnostic.getSelfTestConfig` 方法小节。 |
| `SetSelfTestConfigParams` | `diagnostic.setSelfTestConfig` request params | 见 `diagnostic.setSelfTestConfig` 方法小节。 |
| `SetSelfTestConfigResult` | `diagnostic.setSelfTestConfig` result | 见 `diagnostic.setSelfTestConfig` 方法小节。 |
| `ResetSelfTestConfigParams` | `diagnostic.resetSelfTestConfig` request params | 见 `diagnostic.resetSelfTestConfig` 方法小节。 |
| `ResetSelfTestConfigResult` | `diagnostic.resetSelfTestConfig` result | 见 `diagnostic.resetSelfTestConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `SelfTestConfigChangedEvent` | `diagnostic.selfTestConfigChanged` | 见 `diagnostic.selfTestConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `SelfTestState` | 表达 `diagnostic.selfTest` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `SelfTestConfig` | 表达 `diagnostic.selfTest` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.selfTest`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.selfTest` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.selfTest` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.selfTest` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
