...
generated: false
domain: diagnostic
feature: diagnostic.manufacturing
registry:
...
`diagnostic.manufacturing` 用于工厂写入、产测参数和制造流程状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | diagnostic.manufacturing 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 diagnostic.manufacturing 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 diagnostic.manufacturing 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `diagnostic.getManufacturingCapabilities` | query | 查询 diagnostic.manufacturing 能力范围 | `GetManufacturingCapabilitiesParams` | `GetManufacturingCapabilitiesResult` | 否 | draft |
| `diagnostic.getManufacturingConfig` | query | 查询 diagnostic.manufacturing 配置 | `GetManufacturingConfigParams` | `GetManufacturingConfigResult` | 否 | draft |
| `diagnostic.setManufacturingConfig` | command | 设置 diagnostic.manufacturing 配置 | `SetManufacturingConfigParams` | `SetManufacturingConfigResult` | 是，`diagnostic.manufacturingConfigChanged` | draft |
| `diagnostic.resetManufacturingConfig` | action | 恢复 diagnostic.manufacturing 默认配置 | `ResetManufacturingConfigParams` | `ResetManufacturingConfigResult` | 是，`diagnostic.manufacturingConfigChanged` | draft |

### 3.1 `diagnostic.getManufacturingCapabilities`

**用途**：查询 diagnostic.manufacturing 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetManufacturingCapabilitiesParams` |
| Result Schema | `GetManufacturingCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetManufacturingCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetManufacturingCapabilitiesResult`
...
### 3.2 `diagnostic.getManufacturingConfig`

**用途**：查询 diagnostic.manufacturing 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetManufacturingConfigParams` |
| Result Schema | `GetManufacturingConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetManufacturingConfigParams`
...
#### 3.2.3 返回结果 Result：`GetManufacturingConfigResult`
...
### 3.3 `diagnostic.setManufacturingConfig`

**用途**：设置 diagnostic.manufacturing 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetManufacturingConfigParams` |
| Result Schema | `SetManufacturingConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.manufacturingConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetManufacturingConfigParams`
...
#### 3.3.3 返回结果 Result：`SetManufacturingConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `diagnostic.manufacturingConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `ManufacturingConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "diagnostic.manufacturingConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `diagnostic.resetManufacturingConfig`

**用途**：恢复 diagnostic.manufacturing 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetManufacturingConfigParams` |
| Result Schema | `ResetManufacturingConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.manufacturingConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetManufacturingConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetManufacturingConfigResult`
...
|---|---|---|---|---|
| `diagnostic.manufacturingConfigChanged` | diagnostic.manufacturing 配置变化 | `ManufacturingConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `diagnostic.manufacturingConfigChanged`

**触发条件**：diagnostic.manufacturing 配置变化。

#### 4.1.1 Payload：`ManufacturingConfigChangedEvent`
...
Capability name: `diagnostic.manufacturing`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `diagnostic.manufacturing` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetManufacturingCapabilitiesParams` | `diagnostic.getManufacturingCapabilities` request params | 见 `diagnostic.getManufacturingCapabilities` 方法小节。 |
| `GetManufacturingCapabilitiesResult` | `diagnostic.getManufacturingCapabilities` result | 见 `diagnostic.getManufacturingCapabilities` 方法小节。 |
| `GetManufacturingConfigParams` | `diagnostic.getManufacturingConfig` request params | 见 `diagnostic.getManufacturingConfig` 方法小节。 |
| `GetManufacturingConfigResult` | `diagnostic.getManufacturingConfig` result | 见 `diagnostic.getManufacturingConfig` 方法小节。 |
| `SetManufacturingConfigParams` | `diagnostic.setManufacturingConfig` request params | 见 `diagnostic.setManufacturingConfig` 方法小节。 |
| `SetManufacturingConfigResult` | `diagnostic.setManufacturingConfig` result | 见 `diagnostic.setManufacturingConfig` 方法小节。 |
| `ResetManufacturingConfigParams` | `diagnostic.resetManufacturingConfig` request params | 见 `diagnostic.resetManufacturingConfig` 方法小节。 |
| `ResetManufacturingConfigResult` | `diagnostic.resetManufacturingConfig` result | 见 `diagnostic.resetManufacturingConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `ManufacturingConfigChangedEvent` | `diagnostic.manufacturingConfigChanged` | 见 `diagnostic.manufacturingConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `ManufacturingState` | 表达 `diagnostic.manufacturing` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `ManufacturingConfig` | 表达 `diagnostic.manufacturing` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.manufacturing`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.manufacturing` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.manufacturing` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.manufacturing` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
