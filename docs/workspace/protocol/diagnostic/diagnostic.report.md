...
`diagnostic.report` 用于诊断报告生成、导出和报告状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | diagnostic.report 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 diagnostic.report 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 diagnostic.report 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `diagnostic.getReportCapabilities` | query | 查询 diagnostic.report 能力范围 | `GetReportCapabilitiesParams` | `GetReportCapabilitiesResult` | 否 | draft |
| `diagnostic.createReportExport` | action | 创建 diagnostic.report 导出任务 | `CreateReportExportParams` | `CreateReportExportResult` | 是，`diagnostic.reportExportStateChanged` | draft |
| `diagnostic.getReportExportState` | query | 查询 diagnostic.report 导出状态 | `GetReportExportStateParams` | `GetReportExportStateResult` | 否 | draft |

### 3.1 `diagnostic.getReportCapabilities`

**用途**：查询 diagnostic.report 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetReportCapabilitiesParams` |
| Result Schema | `GetReportCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetReportCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetReportCapabilitiesResult`
...
### 3.2 `diagnostic.createReportExport`

**用途**：创建 diagnostic.report 导出任务。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `CreateReportExportParams` |
| Result Schema | `CreateReportExportResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.reportExportStateChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.2.1 请求参数 Params：`CreateReportExportParams`
...
#### 3.2.3 返回结果 Result：`CreateReportExportResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `diagnostic.reportExportStateChanged` | 该方法导致状态、配置或动作状态实际变化。 | `ReportExportStateChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "diagnostic.reportExportStateChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.3 `diagnostic.getReportExportState`

**用途**：查询 diagnostic.report 导出状态。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetReportExportStateParams` |
| Result Schema | `GetReportExportStateResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.3.1 请求参数 Params：`GetReportExportStateParams`
...
#### 3.3.3 返回结果 Result：`GetReportExportStateResult`
...
|---|---|---|---|---|
| `diagnostic.reportExportStateChanged` | diagnostic.report 导出状态变化 | `ReportExportStateChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `diagnostic.reportExportStateChanged`

**触发条件**：diagnostic.report 导出状态变化。

#### 4.1.1 Payload：`ReportExportStateChangedEvent`
...
Capability name: `diagnostic.report`。
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetReportCapabilitiesParams` | `diagnostic.getReportCapabilities` request params | 见 `diagnostic.getReportCapabilities` 方法小节。 |
| `GetReportCapabilitiesResult` | `diagnostic.getReportCapabilities` result | 见 `diagnostic.getReportCapabilities` 方法小节。 |
| `CreateReportExportParams` | `diagnostic.createReportExport` request params | 见 `diagnostic.createReportExport` 方法小节。 |
| `CreateReportExportResult` | `diagnostic.createReportExport` result | 见 `diagnostic.createReportExport` 方法小节。 |
| `GetReportExportStateParams` | `diagnostic.getReportExportState` request params | 见 `diagnostic.getReportExportState` 方法小节。 |
| `GetReportExportStateResult` | `diagnostic.getReportExportState` result | 见 `diagnostic.getReportExportState` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `ReportExportStateChangedEvent` | `diagnostic.reportExportStateChanged` | 见 `diagnostic.reportExportStateChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `ReportState` | 表达 `diagnostic.report` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `ReportConfig` | 表达 `diagnostic.report` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.report`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.report` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.report` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.report` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
