...
`diagnostic.audioTest` 用于音频采集、播放、算法和产测音频测试。
...
| 类型 | 内容 |
|---|---|
| 包含 | diagnostic.audioTest 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 diagnostic.audioTest 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 diagnostic.audioTest 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `diagnostic.getAudioTestCapabilities` | query | 查询 diagnostic.audioTest 能力范围 | `GetAudioTestCapabilitiesParams` | `GetAudioTestCapabilitiesResult` | 否 | draft |
| `diagnostic.getAudioTestConfig` | query | 查询 diagnostic.audioTest 配置 | `GetAudioTestConfigParams` | `GetAudioTestConfigResult` | 否 | draft |
| `diagnostic.setAudioTestConfig` | command | 设置 diagnostic.audioTest 配置 | `SetAudioTestConfigParams` | `SetAudioTestConfigResult` | 是，`diagnostic.audioTestConfigChanged` | draft |
| `diagnostic.resetAudioTestConfig` | action | 恢复 diagnostic.audioTest 默认配置 | `ResetAudioTestConfigParams` | `ResetAudioTestConfigResult` | 是，`diagnostic.audioTestConfigChanged` | draft |

### 3.1 `diagnostic.getAudioTestCapabilities`

**用途**：查询 diagnostic.audioTest 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetAudioTestCapabilitiesParams` |
| Result Schema | `GetAudioTestCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetAudioTestCapabilitiesParams`
...
{
  "id": 101,
  "method": "diagnostic.getAudioTestCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetAudioTestCapabilitiesResult`
...
### 3.2 `diagnostic.getAudioTestConfig`

**用途**：查询 diagnostic.audioTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetAudioTestConfigParams` |
| Result Schema | `GetAudioTestConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetAudioTestConfigParams`
...
{
  "id": 102,
  "method": "diagnostic.getAudioTestConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetAudioTestConfigResult`
...
### 3.3 `diagnostic.setAudioTestConfig`

**用途**：设置 diagnostic.audioTest 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetAudioTestConfigParams` |
| Result Schema | `SetAudioTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.audioTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetAudioTestConfigParams`
...
{
  "id": 103,
  "method": "diagnostic.setAudioTestConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetAudioTestConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `diagnostic.audioTestConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `AudioTestConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "diagnostic.audioTestConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `diagnostic.resetAudioTestConfig`

**用途**：恢复 diagnostic.audioTest 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetAudioTestConfigParams` |
| Result Schema | `ResetAudioTestConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `diagnostic.audioTestConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetAudioTestConfigParams`
...
{
  "id": 104,
  "method": "diagnostic.resetAudioTestConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetAudioTestConfigResult`
...
|---|---|---|---|---|
| `diagnostic.audioTestConfigChanged` | diagnostic.audioTest 配置变化 | `AudioTestConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `diagnostic.audioTestConfigChanged`

**触发条件**：diagnostic.audioTest 配置变化。

#### 4.1.1 Payload：`AudioTestConfigChangedEvent`
...
Capability name: `diagnostic.audioTest`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `diagnostic.audioTest` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetAudioTestCapabilitiesParams` | `diagnostic.getAudioTestCapabilities` request params | 见 `diagnostic.getAudioTestCapabilities` 方法小节。 |
| `GetAudioTestCapabilitiesResult` | `diagnostic.getAudioTestCapabilities` result | 见 `diagnostic.getAudioTestCapabilities` 方法小节。 |
| `GetAudioTestConfigParams` | `diagnostic.getAudioTestConfig` request params | 见 `diagnostic.getAudioTestConfig` 方法小节。 |
| `GetAudioTestConfigResult` | `diagnostic.getAudioTestConfig` result | 见 `diagnostic.getAudioTestConfig` 方法小节。 |
| `SetAudioTestConfigParams` | `diagnostic.setAudioTestConfig` request params | 见 `diagnostic.setAudioTestConfig` 方法小节。 |
| `SetAudioTestConfigResult` | `diagnostic.setAudioTestConfig` result | 见 `diagnostic.setAudioTestConfig` 方法小节。 |
| `ResetAudioTestConfigParams` | `diagnostic.resetAudioTestConfig` request params | 见 `diagnostic.resetAudioTestConfig` 方法小节。 |
| `ResetAudioTestConfigResult` | `diagnostic.resetAudioTestConfig` result | 见 `diagnostic.resetAudioTestConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `AudioTestConfigChangedEvent` | `diagnostic.audioTestConfigChanged` | 见 `diagnostic.audioTestConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `AudioTestState` | 表达 `diagnostic.audioTest` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `AudioTestConfig` | 表达 `diagnostic.audioTest` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `diagnostic.audioTest`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `diagnostic.audioTest` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `diagnostic.audioTest` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `diagnostic.audioTest` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
