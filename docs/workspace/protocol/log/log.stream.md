...
`log.stream` 用于实时日志订阅、日志流状态和日志流数据面入口。
...
| 类型 | 内容 |
|---|---|
| 包含 | log.stream 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 log.stream 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 log.stream 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `log.openStream` | action | 创建实时日志流 | `OpenStreamParams` | `OpenStreamResult` | 是，`log.streamStateChanged` | draft |
| `log.closeStream` | action | 关闭实时日志流 | `CloseStreamParams` | `CloseStreamResult` | 是，`log.streamStateChanged` | draft |
| `log.getStreamState` | query | 查询实时日志流状态 | `GetStreamStateParams` | `GetStreamStateResult` | 否 | draft |
...
| 是否触发事件 | 是，状态实际变化后触发 `log.streamStateChanged`。 |
...
|---|---|---|---|
| `log.streamStateChanged` | 该方法导致状态、配置或动作状态实际变化。 | `StreamStateChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |
...
#### 3.2.1 请求参数 Params：`CloseStreamParams`
...
#### 3.2.3 返回结果 Result：`CloseStreamResult`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetStreamStateParams` |
| Result Schema | `GetStreamStateResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.3.1 请求参数 Params：`GetStreamStateParams`
...
#### 3.3.3 返回结果 Result：`GetStreamStateResult`
...
|---|---|---|---|---|
| `log.streamStateChanged` | 日志流状态变化 | `StreamStateChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `log.streamStateChanged`

**触发条件**：日志流状态变化。

#### 4.1.1 Payload：`StreamStateChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `log.stream` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `OpenStreamParams` | `log.openStream` request params | 见 `log.openStream` 方法小节。 |
| `OpenStreamResult` | `log.openStream` result | 见 `log.openStream` 方法小节。 |
| `CloseStreamParams` | `log.closeStream` request params | 见 `log.closeStream` 方法小节。 |
| `CloseStreamResult` | `log.closeStream` result | 见 `log.closeStream` 方法小节。 |
| `GetStreamStateParams` | `log.getStreamState` request params | 见 `log.getStreamState` 方法小节。 |
| `GetStreamStateResult` | `log.getStreamState` result | 见 `log.getStreamState` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `StreamStateChangedEvent` | `log.streamStateChanged` | 见 `log.streamStateChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `StreamState` | 表达 `log.stream` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `StreamConfig` | 表达 `log.stream` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `log.stream` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `log.stream` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `log.stream` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
