...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 日志导出任务、导出状态、导出进度和导出结果文件引用。 |
| 当前状态 | draft |
...
`log.export` 用于日志导出任务、导出状态、导出进度和导出结果文件引用。
...
| 类型 | 内容 |
|---|---|
| 包含 | log.export 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 log.export 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 log.export 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `log.createExport` | action | 创建日志导出任务 | `CreateExportParams` | `CreateExportResult` | 是，`log.exportStateChanged` | draft |
| `log.getExportState` | query | 查询日志导出状态 | `GetExportStateParams` | `GetExportStateResult` | 否 | draft |
| `log.cancelExport` | action | 取消日志导出任务 | `CancelExportParams` | `CancelExportResult` | 是，`log.exportStateChanged` | draft |

### 3.1 `log.createExport`
...
|---|---|
| 调用类型 | action |
| Params Schema | `CreateExportParams` |
| Result Schema | `CreateExportResult` |
| 是否触发事件 | 是，状态实际变化后触发 `log.exportStateChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.1.1 请求参数 Params：`CreateExportParams`
...
|---|---|---|---|
| `log.exportStateChanged` | 该方法导致状态、配置或动作状态实际变化。 | `ExportStateChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetExportStateParams` |
| Result Schema | `GetExportStateResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetExportStateParams`
...
|---|---|
| 调用类型 | action |
| Params Schema | `CancelExportParams` |
| Result Schema | `CancelExportResult` |
| 是否触发事件 | 是，状态实际变化后触发 `log.exportStateChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`CancelExportParams`
...
|---|---|---|---|---|
| `log.exportStateChanged` | 日志导出状态变化 | `ExportStateChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |
| `log.exportProgressReported` | 日志导出进度上报 | `ExportProgressReportedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `log.exportStateChanged`

**触发条件**：日志导出状态变化。

#### 4.1.1 Payload：`ExportStateChangedEvent`
...
### 4.2 `log.exportProgressReported`

**触发条件**：日志导出进度上报。

#### 4.2.1 Payload：`ExportProgressReportedEvent`
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `CreateExportParams` | `log.createExport` request params | 见 `log.createExport` 方法小节。 |
| `CreateExportResult` | `log.createExport` result | 见 `log.createExport` 方法小节。 |
| `GetExportStateParams` | `log.getExportState` request params | 见 `log.getExportState` 方法小节。 |
| `GetExportStateResult` | `log.getExportState` result | 见 `log.getExportState` 方法小节。 |
| `CancelExportParams` | `log.cancelExport` request params | 见 `log.cancelExport` 方法小节。 |
| `CancelExportResult` | `log.cancelExport` result | 见 `log.cancelExport` 方法小节。 |
...
| Schema | 用途 | 状态 |
|---|---|---|
| `ExportState` | 表达 `log.export` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `ExportConfig` | 表达 `log.export` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `log.export` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `log.export` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `log.export` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
