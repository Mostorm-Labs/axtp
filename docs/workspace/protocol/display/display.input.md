...
| 类型 | 内容 |
|---|---|
| 包含 | display.input 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 display.input 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 display.input 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `display.getInputCapabilities` | query | 查询 display.input 能力范围 | `GetInputCapabilitiesParams` | `GetInputCapabilitiesResult` | 否 | draft |
| `display.getInputConfig` | query | 查询 display.input 配置 | `GetInputConfigParams` | `GetInputConfigResult` | 否 | draft |
| `display.setInputConfig` | command | 设置 display.input 配置 | `SetInputConfigParams` | `SetInputConfigResult` | 是，`display.inputConfigChanged` | draft |
| `display.resetInputConfig` | action | 恢复 display.input 默认配置 | `ResetInputConfigParams` | `ResetInputConfigResult` | 是，`display.inputConfigChanged` | draft |

### 3.1 `display.getInputCapabilities`

**用途**：查询 display.input 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetInputCapabilitiesParams` |
| Result Schema | `GetInputCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetInputCapabilitiesParams`
...
{
  "id": 101,
  "method": "display.getInputCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetInputCapabilitiesResult`
...
### 3.2 `display.getInputConfig`

**用途**：查询 display.input 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetInputConfigParams` |
| Result Schema | `GetInputConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetInputConfigParams`
...
#### 3.2.3 返回结果 Result：`GetInputConfigResult`
...
### 3.3 `display.setInputConfig`

**用途**：设置 display.input 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetInputConfigParams` |
| Result Schema | `SetInputConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.inputConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetInputConfigParams`
...
#### 3.3.3 返回结果 Result：`SetInputConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `display.inputConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `InputConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "display.inputConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `display.resetInputConfig`

**用途**：恢复 display.input 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetInputConfigParams` |
| Result Schema | `ResetInputConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.inputConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetInputConfigParams`
...
{
  "id": 104,
  "method": "display.resetInputConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetInputConfigResult`
...
|---|---|---|---|---|
| `display.inputConfigChanged` | display.input 配置变化 | `InputConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `display.inputConfigChanged`

**触发条件**：display.input 配置变化。

#### 4.1.1 Payload：`InputConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `display.input` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetInputCapabilitiesParams` | `display.getInputCapabilities` request params | 见 `display.getInputCapabilities` 方法小节。 |
| `GetInputCapabilitiesResult` | `display.getInputCapabilities` result | 见 `display.getInputCapabilities` 方法小节。 |
| `GetInputConfigParams` | `display.getInputConfig` request params | 见 `display.getInputConfig` 方法小节。 |
| `GetInputConfigResult` | `display.getInputConfig` result | 见 `display.getInputConfig` 方法小节。 |
| `SetInputConfigParams` | `display.setInputConfig` request params | 见 `display.setInputConfig` 方法小节。 |
| `SetInputConfigResult` | `display.setInputConfig` result | 见 `display.setInputConfig` 方法小节。 |
| `ResetInputConfigParams` | `display.resetInputConfig` request params | 见 `display.resetInputConfig` 方法小节。 |
| `ResetInputConfigResult` | `display.resetInputConfig` result | 见 `display.resetInputConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `InputConfigChangedEvent` | `display.inputConfigChanged` | 见 `display.inputConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `InputState` | 表达 `display.input` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `InputConfig` | 表达 `display.input` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `display.input` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `display.input` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `display.input` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
