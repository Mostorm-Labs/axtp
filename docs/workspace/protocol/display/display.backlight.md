...
`display.backlight` 用于背光开关、背光亮度和背光策略。
...
| 类型 | 内容 |
|---|---|
| 包含 | display.backlight 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 display.backlight 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 display.backlight 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `display.getBacklightCapabilities` | query | 查询 display.backlight 能力范围 | `GetBacklightCapabilitiesParams` | `GetBacklightCapabilitiesResult` | 否 | draft |
| `display.getBacklightConfig` | query | 查询 display.backlight 配置 | `GetBacklightConfigParams` | `GetBacklightConfigResult` | 否 | draft |
| `display.setBacklightConfig` | command | 设置 display.backlight 配置 | `SetBacklightConfigParams` | `SetBacklightConfigResult` | 是，`display.backlightConfigChanged` | draft |
| `display.resetBacklightConfig` | action | 恢复 display.backlight 默认配置 | `ResetBacklightConfigParams` | `ResetBacklightConfigResult` | 是，`display.backlightConfigChanged` | draft |

### 3.1 `display.getBacklightCapabilities`

**用途**：查询 display.backlight 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetBacklightCapabilitiesParams` |
| Result Schema | `GetBacklightCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetBacklightCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetBacklightCapabilitiesResult`
...
### 3.2 `display.getBacklightConfig`

**用途**：查询 display.backlight 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetBacklightConfigParams` |
| Result Schema | `GetBacklightConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetBacklightConfigParams`
...
#### 3.2.3 返回结果 Result：`GetBacklightConfigResult`
...
### 3.3 `display.setBacklightConfig`

**用途**：设置 display.backlight 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetBacklightConfigParams` |
| Result Schema | `SetBacklightConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.backlightConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetBacklightConfigParams`
...
#### 3.3.3 返回结果 Result：`SetBacklightConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `display.backlightConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `BacklightConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "display.backlightConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `display.resetBacklightConfig`

**用途**：恢复 display.backlight 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetBacklightConfigParams` |
| Result Schema | `ResetBacklightConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.backlightConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetBacklightConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetBacklightConfigResult`
...
|---|---|---|---|---|
| `display.backlightConfigChanged` | display.backlight 配置变化 | `BacklightConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `display.backlightConfigChanged`

**触发条件**：display.backlight 配置变化。

#### 4.1.1 Payload：`BacklightConfigChangedEvent`
...
Capability name: `display.backlight`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `display.backlight` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetBacklightCapabilitiesParams` | `display.getBacklightCapabilities` request params | 见 `display.getBacklightCapabilities` 方法小节。 |
| `GetBacklightCapabilitiesResult` | `display.getBacklightCapabilities` result | 见 `display.getBacklightCapabilities` 方法小节。 |
| `GetBacklightConfigParams` | `display.getBacklightConfig` request params | 见 `display.getBacklightConfig` 方法小节。 |
| `GetBacklightConfigResult` | `display.getBacklightConfig` result | 见 `display.getBacklightConfig` 方法小节。 |
| `SetBacklightConfigParams` | `display.setBacklightConfig` request params | 见 `display.setBacklightConfig` 方法小节。 |
| `SetBacklightConfigResult` | `display.setBacklightConfig` result | 见 `display.setBacklightConfig` 方法小节。 |
| `ResetBacklightConfigParams` | `display.resetBacklightConfig` request params | 见 `display.resetBacklightConfig` 方法小节。 |
| `ResetBacklightConfigResult` | `display.resetBacklightConfig` result | 见 `display.resetBacklightConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `BacklightConfigChangedEvent` | `display.backlightConfigChanged` | 见 `display.backlightConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `BacklightState` | 表达 `display.backlight` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `BacklightConfig` | 表达 `display.backlight` 的可写配置。 | `[REVIEW-ASK]` |
...
### 7.1 场景：读取或修改 `display.backlight`
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `display.backlight` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `display.backlight` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `display.backlight` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
