...
`display.power` 用于显示电源、休眠、唤醒和省电状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | display.power 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 display.power 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 display.power 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `display.getPowerCapabilities` | query | 查询 display.power 能力范围 | `GetPowerCapabilitiesParams` | `GetPowerCapabilitiesResult` | 否 | draft |
| `display.getPowerConfig` | query | 查询 display.power 配置 | `GetPowerConfigParams` | `GetPowerConfigResult` | 否 | draft |
| `display.setPowerConfig` | command | 设置 display.power 配置 | `SetPowerConfigParams` | `SetPowerConfigResult` | 是，`display.powerConfigChanged` | draft |
| `display.resetPowerConfig` | action | 恢复 display.power 默认配置 | `ResetPowerConfigParams` | `ResetPowerConfigResult` | 是，`display.powerConfigChanged` | draft |

### 3.1 `display.getPowerCapabilities`

**用途**：查询 display.power 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetPowerCapabilitiesParams` |
| Result Schema | `GetPowerCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetPowerCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetPowerCapabilitiesResult`
...
### 3.2 `display.getPowerConfig`

**用途**：查询 display.power 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetPowerConfigParams` |
| Result Schema | `GetPowerConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetPowerConfigParams`
...
#### 3.2.3 返回结果 Result：`GetPowerConfigResult`
...
### 3.3 `display.setPowerConfig`

**用途**：设置 display.power 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetPowerConfigParams` |
| Result Schema | `SetPowerConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.powerConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetPowerConfigParams`
...
#### 3.3.3 返回结果 Result：`SetPowerConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `display.powerConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `PowerConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "display.powerConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `display.resetPowerConfig`

**用途**：恢复 display.power 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetPowerConfigParams` |
| Result Schema | `ResetPowerConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.powerConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetPowerConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetPowerConfigResult`
...
|---|---|---|---|---|
| `display.powerConfigChanged` | display.power 配置变化 | `PowerConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `display.powerConfigChanged`

**触发条件**：display.power 配置变化。

#### 4.1.1 Payload：`PowerConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `display.power` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetPowerCapabilitiesParams` | `display.getPowerCapabilities` request params | 见 `display.getPowerCapabilities` 方法小节。 |
| `GetPowerCapabilitiesResult` | `display.getPowerCapabilities` result | 见 `display.getPowerCapabilities` 方法小节。 |
| `GetPowerConfigParams` | `display.getPowerConfig` request params | 见 `display.getPowerConfig` 方法小节。 |
| `GetPowerConfigResult` | `display.getPowerConfig` result | 见 `display.getPowerConfig` 方法小节。 |
| `SetPowerConfigParams` | `display.setPowerConfig` request params | 见 `display.setPowerConfig` 方法小节。 |
| `SetPowerConfigResult` | `display.setPowerConfig` result | 见 `display.setPowerConfig` 方法小节。 |
| `ResetPowerConfigParams` | `display.resetPowerConfig` request params | 见 `display.resetPowerConfig` 方法小节。 |
| `ResetPowerConfigResult` | `display.resetPowerConfig` result | 见 `display.resetPowerConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `PowerConfigChangedEvent` | `display.powerConfigChanged` | 见 `display.powerConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `PowerState` | 表达 `display.power` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `PowerConfig` | 表达 `display.power` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `display.power` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `display.power` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `display.power` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
