...
`display.output` 用于显示输出模式、输出端口和输出状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | display.output 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 display.output 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 display.output 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `display.getOutputCapabilities` | query | 查询 display.output 能力范围 | `GetOutputCapabilitiesParams` | `GetOutputCapabilitiesResult` | 否 | draft |
| `display.getOutputConfig` | query | 查询 display.output 配置 | `GetOutputConfigParams` | `GetOutputConfigResult` | 否 | draft |
| `display.setOutputConfig` | command | 设置 display.output 配置 | `SetOutputConfigParams` | `SetOutputConfigResult` | 是，`display.outputConfigChanged` | draft |
| `display.resetOutputConfig` | action | 恢复 display.output 默认配置 | `ResetOutputConfigParams` | `ResetOutputConfigResult` | 是，`display.outputConfigChanged` | draft |

### 3.1 `display.getOutputCapabilities`

**用途**：查询 display.output 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetOutputCapabilitiesParams` |
| Result Schema | `GetOutputCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetOutputCapabilitiesParams`
...
{
  "id": 101,
  "method": "display.getOutputCapabilities",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.1.3 返回结果 Result：`GetOutputCapabilitiesResult`
...
### 3.2 `display.getOutputConfig`

**用途**：查询 display.output 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetOutputConfigParams` |
| Result Schema | `GetOutputConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetOutputConfigParams`
...
{
  "id": 102,
  "method": "display.getOutputConfig",
  "params": {
    "target": "default",
    "sections": [
      "summary"
...
#### 3.2.3 返回结果 Result：`GetOutputConfigResult`
...
### 3.3 `display.setOutputConfig`

**用途**：设置 display.output 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetOutputConfigParams` |
| Result Schema | `SetOutputConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.outputConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetOutputConfigParams`
...
{
  "id": 103,
  "method": "display.setOutputConfig",
  "params": {
    "target": "default",
    "config": {
      "enabled": true
...
#### 3.3.3 返回结果 Result：`SetOutputConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `display.outputConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `OutputConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "display.outputConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `display.resetOutputConfig`

**用途**：恢复 display.output 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetOutputConfigParams` |
| Result Schema | `ResetOutputConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `display.outputConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetOutputConfigParams`
...
{
  "id": 104,
  "method": "display.resetOutputConfig",
  "params": {
    "target": "default",
    "reason": "user_request"
  }
...
#### 3.4.3 返回结果 Result：`ResetOutputConfigResult`
...
|---|---|---|---|---|
| `display.outputConfigChanged` | display.output 配置变化 | `OutputConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `display.outputConfigChanged`

**触发条件**：display.output 配置变化。

#### 4.1.1 Payload：`OutputConfigChangedEvent`
...
Capability name: `display.output`。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `display.output` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetOutputCapabilitiesParams` | `display.getOutputCapabilities` request params | 见 `display.getOutputCapabilities` 方法小节。 |
| `GetOutputCapabilitiesResult` | `display.getOutputCapabilities` result | 见 `display.getOutputCapabilities` 方法小节。 |
| `GetOutputConfigParams` | `display.getOutputConfig` request params | 见 `display.getOutputConfig` 方法小节。 |
| `GetOutputConfigResult` | `display.getOutputConfig` result | 见 `display.getOutputConfig` 方法小节。 |
| `SetOutputConfigParams` | `display.setOutputConfig` request params | 见 `display.setOutputConfig` 方法小节。 |
| `SetOutputConfigResult` | `display.setOutputConfig` result | 见 `display.setOutputConfig` 方法小节。 |
| `ResetOutputConfigParams` | `display.resetOutputConfig` request params | 见 `display.resetOutputConfig` 方法小节。 |
| `ResetOutputConfigResult` | `display.resetOutputConfig` result | 见 `display.resetOutputConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `OutputConfigChangedEvent` | `display.outputConfigChanged` | 见 `display.outputConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `OutputState` | 表达 `display.output` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `OutputConfig` | 表达 `display.output` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `display.output` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `display.output` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `display.output` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
