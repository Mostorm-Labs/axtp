...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 多路音频混音、输入输出混合矩阵和 mixer 状态。 |
| 当前状态 | draft |
...
`audio.mixer` 用于多路音频混音、输入输出混合矩阵和 mixer 状态。
...
| 类型 | 内容 |
|---|---|
| 包含 | audio.mixer 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 audio.mixer 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 audio.mixer 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `audio.getMixerCapabilities` | query | 查询 audio.mixer 能力范围 | `GetMixerCapabilitiesParams` | `GetMixerCapabilitiesResult` | 否 | draft |
| `audio.getMixerConfig` | query | 查询 audio.mixer 配置 | `GetMixerConfigParams` | `GetMixerConfigResult` | 否 | draft |
| `audio.setMixerConfig` | command | 设置 audio.mixer 配置 | `SetMixerConfigParams` | `SetMixerConfigResult` | 是，`audio.mixerConfigChanged` | draft |
| `audio.resetMixerConfig` | action | 恢复 audio.mixer 默认配置 | `ResetMixerConfigParams` | `ResetMixerConfigResult` | 是，`audio.mixerConfigChanged` | draft |

### 3.1 `audio.getMixerCapabilities`

**用途**：查询 audio.mixer 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetMixerCapabilitiesParams` |
| Result Schema | `GetMixerCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetMixerCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetMixerCapabilitiesResult`
...
### 3.2 `audio.getMixerConfig`

**用途**：查询 audio.mixer 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetMixerConfigParams` |
| Result Schema | `GetMixerConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetMixerConfigParams`
...
#### 3.2.3 返回结果 Result：`GetMixerConfigResult`
...
### 3.3 `audio.setMixerConfig`

**用途**：设置 audio.mixer 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetMixerConfigParams` |
| Result Schema | `SetMixerConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `audio.mixerConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetMixerConfigParams`
...
#### 3.3.3 返回结果 Result：`SetMixerConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `audio.mixerConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `MixerConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "audio.mixerConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `audio.resetMixerConfig`

**用途**：恢复 audio.mixer 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetMixerConfigParams` |
| Result Schema | `ResetMixerConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `audio.mixerConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetMixerConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetMixerConfigResult`
...
|---|---|---|---|---|
| `audio.mixerConfigChanged` | audio.mixer 配置变化 | `MixerConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `audio.mixerConfigChanged`

**触发条件**：audio.mixer 配置变化。

#### 4.1.1 Payload：`MixerConfigChangedEvent`
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetMixerCapabilitiesParams` | `audio.getMixerCapabilities` request params | 见 `audio.getMixerCapabilities` 方法小节。 |
| `GetMixerCapabilitiesResult` | `audio.getMixerCapabilities` result | 见 `audio.getMixerCapabilities` 方法小节。 |
| `GetMixerConfigParams` | `audio.getMixerConfig` request params | 见 `audio.getMixerConfig` 方法小节。 |
| `GetMixerConfigResult` | `audio.getMixerConfig` result | 见 `audio.getMixerConfig` 方法小节。 |
| `SetMixerConfigParams` | `audio.setMixerConfig` request params | 见 `audio.setMixerConfig` 方法小节。 |
| `SetMixerConfigResult` | `audio.setMixerConfig` result | 见 `audio.setMixerConfig` 方法小节。 |
| `ResetMixerConfigParams` | `audio.resetMixerConfig` request params | 见 `audio.resetMixerConfig` 方法小节。 |
| `ResetMixerConfigResult` | `audio.resetMixerConfig` result | 见 `audio.resetMixerConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `MixerConfigChangedEvent` | `audio.mixerConfigChanged` | 见 `audio.mixerConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `MixerState` | 表达 `audio.mixer` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `MixerConfig` | 表达 `audio.mixer` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `audio.mixer` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `audio.mixer` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `audio.mixer` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
