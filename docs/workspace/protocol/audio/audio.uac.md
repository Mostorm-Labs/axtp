...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | USB Audio Class 音频能力和服务配置。 |
| 当前状态 | draft |
...
`audio.uac` 用于USB Audio Class 音频能力和服务配置。
...
| 类型 | 内容 |
|---|---|
| 包含 | audio.uac 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 audio.uac 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 audio.uac 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `audio.getUacCapabilities` | query | 查询 audio.uac 能力范围 | `GetUacCapabilitiesParams` | `GetUacCapabilitiesResult` | 否 | draft |
| `audio.getUacConfig` | query | 查询 audio.uac 配置 | `GetUacConfigParams` | `GetUacConfigResult` | 否 | draft |
| `audio.setUacConfig` | command | 设置 audio.uac 配置 | `SetUacConfigParams` | `SetUacConfigResult` | 是，`audio.uacConfigChanged` | draft |
| `audio.resetUacConfig` | action | 恢复 audio.uac 默认配置 | `ResetUacConfigParams` | `ResetUacConfigResult` | 是，`audio.uacConfigChanged` | draft |

### 3.1 `audio.getUacCapabilities`

**用途**：查询 audio.uac 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetUacCapabilitiesParams` |
| Result Schema | `GetUacCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetUacCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetUacCapabilitiesResult`
...
|---|---|
| 调用类型 | query |
| Params Schema | `GetUacConfigParams` |
| Result Schema | `GetUacConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetUacConfigParams`
...
#### 3.2.3 返回结果 Result：`GetUacConfigResult`
...
|---|---|
| 调用类型 | command |
| Params Schema | `SetUacConfigParams` |
| Result Schema | `SetUacConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `audio.uacConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetUacConfigParams`
...
#### 3.3.3 返回结果 Result：`SetUacConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `audio.uacConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `UacConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "audio.uacConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `audio.resetUacConfig`

**用途**：恢复 audio.uac 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetUacConfigParams` |
| Result Schema | `ResetUacConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `audio.uacConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetUacConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetUacConfigResult`
...
|---|---|---|---|---|
| `audio.uacConfigChanged` | audio.uac 配置变化 | `UacConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `audio.uacConfigChanged`

**触发条件**：audio.uac 配置变化。

#### 4.1.1 Payload：`UacConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `audio.uac` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetUacCapabilitiesParams` | `audio.getUacCapabilities` request params | 见 `audio.getUacCapabilities` 方法小节。 |
| `GetUacCapabilitiesResult` | `audio.getUacCapabilities` result | 见 `audio.getUacCapabilities` 方法小节。 |
| `GetUacConfigParams` | `audio.getUacConfig` request params | 见 `audio.getUacConfig` 方法小节。 |
| `GetUacConfigResult` | `audio.getUacConfig` result | 见 `audio.getUacConfig` 方法小节。 |
| `SetUacConfigParams` | `audio.setUacConfig` request params | 见 `audio.setUacConfig` 方法小节。 |
| `SetUacConfigResult` | `audio.setUacConfig` result | 见 `audio.setUacConfig` 方法小节。 |
| `ResetUacConfigParams` | `audio.resetUacConfig` request params | 见 `audio.resetUacConfig` 方法小节。 |
| `ResetUacConfigResult` | `audio.resetUacConfig` result | 见 `audio.resetUacConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `UacConfigChangedEvent` | `audio.uacConfigChanged` | 见 `audio.uacConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `UacState` | 表达 `audio.uac` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `UacConfig` | 表达 `audio.uac` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `audio.uac` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `audio.uac` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `audio.uac` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
