...
`audio.output` 用于音频输出端口、输出目标和输出能力。
...
| 类型 | 内容 |
|---|---|
| 包含 | audio.output 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 audio.output 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 audio.output 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `audio.getOutputCapabilities` | query | 查询 audio.output 能力范围 | `GetOutputCapabilitiesParams` | `GetOutputCapabilitiesResult` | 否 | draft |
| `audio.getOutputConfig` | query | 查询 audio.output 配置 | `GetOutputConfigParams` | `GetOutputConfigResult` | 否 | draft |
| `audio.setOutputConfig` | command | 设置 audio.output 配置 | `SetOutputConfigParams` | `SetOutputConfigResult` | 是，`audio.outputConfigChanged` | draft |
| `audio.resetOutputConfig` | action | 恢复 audio.output 默认配置 | `ResetOutputConfigParams` | `ResetOutputConfigResult` | 是，`audio.outputConfigChanged` | draft |

### 3.1 `audio.getOutputCapabilities`

**用途**：查询 audio.output 能力范围。
...
### 3.2 `audio.getOutputConfig`

**用途**：查询 audio.output 配置。
...
### 3.3 `audio.setOutputConfig`

**用途**：设置 audio.output 配置。
...
| 是否触发事件 | 是，状态实际变化后触发 `audio.outputConfigChanged`。 |
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `audio.outputConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `OutputConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "audio.outputConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `audio.resetOutputConfig`

**用途**：恢复 audio.output 默认配置。
...
|---|---|---|---|---|
| `audio.outputConfigChanged` | audio.output 配置变化 | `OutputConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `audio.outputConfigChanged`

**触发条件**：audio.output 配置变化。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `audio.output` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetOutputCapabilitiesParams` | `audio.getOutputCapabilities` request params | 见 `audio.getOutputCapabilities` 方法小节。 |
| `GetOutputCapabilitiesResult` | `audio.getOutputCapabilities` result | 见 `audio.getOutputCapabilities` 方法小节。 |
| `GetOutputConfigParams` | `audio.getOutputConfig` request params | 见 `audio.getOutputConfig` 方法小节。 |
| `GetOutputConfigResult` | `audio.getOutputConfig` result | 见 `audio.getOutputConfig` 方法小节。 |
| `SetOutputConfigParams` | `audio.setOutputConfig` request params | 见 `audio.setOutputConfig` 方法小节。 |
| `SetOutputConfigResult` | `audio.setOutputConfig` result | 见 `audio.setOutputConfig` 方法小节。 |
| `ResetOutputConfigParams` | `audio.resetOutputConfig` request params | 见 `audio.resetOutputConfig` 方法小节。 |
| `ResetOutputConfigResult` | `audio.resetOutputConfig` result | 见 `audio.resetOutputConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `OutputConfigChangedEvent` | `audio.outputConfigChanged` | 见 `audio.outputConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `OutputState` | 表达 `audio.output` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `OutputConfig` | 表达 `audio.output` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `audio.output` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `audio.output` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `audio.output` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
