...
#### 3.1.3 返回结果 Result：`GetRoutingCapabilitiesResult`
...
#### 3.2.3 返回结果 Result：`GetRoutingConfigResult`
...
#### 3.3.3 返回结果 Result：`SetRoutingConfigResult`
...
#### 3.4.3 返回结果 Result：`ResetRoutingConfigResult`
...
| Schema | 用途 | 状态 |
|---|---|---|
| `RoutingState` | 表达 `audio.routing` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `RoutingConfig` | 表达 `audio.routing` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `audio.routing` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `audio.routing` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `audio.routing` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
