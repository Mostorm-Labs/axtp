...
| 类型 | 内容 |
|---|---|
| 包含 | audio.input 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 audio.input 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 audio.input 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `audio.getInputCapabilities` | query | 查询 audio.input 能力范围 | `GetInputCapabilitiesParams` | `GetInputCapabilitiesResult` | 否 | draft |
| `audio.getInputConfig` | query | 查询 audio.input 配置 | `GetInputConfigParams` | `GetInputConfigResult` | 否 | draft |
| `audio.setInputConfig` | command | 设置 audio.input 配置 | `SetInputConfigParams` | `SetInputConfigResult` | 是，`audio.inputConfigChanged` | draft |
| `audio.resetInputConfig` | action | 恢复 audio.input 默认配置 | `ResetInputConfigParams` | `ResetInputConfigResult` | 是，`audio.inputConfigChanged` | draft |

### 3.1 `audio.getInputCapabilities`

**用途**：查询 audio.input 能力范围。
...
### 3.2 `audio.getInputConfig`

**用途**：查询 audio.input 配置。
...
### 3.3 `audio.setInputConfig`

**用途**：设置 audio.input 配置。
...
| 是否触发事件 | 是，状态实际变化后触发 `audio.inputConfigChanged`。 |
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `audio.inputConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `InputConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "audio.inputConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `audio.resetInputConfig`

**用途**：恢复 audio.input 默认配置。
...
|---|---|---|---|---|
| `audio.inputConfigChanged` | audio.input 配置变化 | `InputConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `audio.inputConfigChanged`

**触发条件**：audio.input 配置变化。
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `audio.input` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetInputCapabilitiesParams` | `audio.getInputCapabilities` request params | 见 `audio.getInputCapabilities` 方法小节。 |
| `GetInputCapabilitiesResult` | `audio.getInputCapabilities` result | 见 `audio.getInputCapabilities` 方法小节。 |
| `GetInputConfigParams` | `audio.getInputConfig` request params | 见 `audio.getInputConfig` 方法小节。 |
| `GetInputConfigResult` | `audio.getInputConfig` result | 见 `audio.getInputConfig` 方法小节。 |
| `SetInputConfigParams` | `audio.setInputConfig` request params | 见 `audio.setInputConfig` 方法小节。 |
| `SetInputConfigResult` | `audio.setInputConfig` result | 见 `audio.setInputConfig` 方法小节。 |
| `ResetInputConfigParams` | `audio.resetInputConfig` request params | 见 `audio.resetInputConfig` 方法小节。 |
| `ResetInputConfigResult` | `audio.resetInputConfig` result | 见 `audio.resetInputConfig` 方法小节。 |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `audio.input` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `audio.input` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `audio.input` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
