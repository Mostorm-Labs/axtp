...
`audio.dante` 用于Dante 网络音频能力和服务配置。
...
| 类型 | 内容 |
|---|---|
| 包含 | audio.dante 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 audio.dante 直接相关的 method/event/schema 草案。 |
| 包含 | 已确认 legacy 协议到 audio.dante 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `audio.getDanteCapabilities` | query | 查询 audio.dante 能力范围 | `GetDanteCapabilitiesParams` | `GetDanteCapabilitiesResult` | 否 | draft |
| `audio.getDanteConfig` | query | 查询 audio.dante 配置 | `GetDanteConfigParams` | `GetDanteConfigResult` | 否 | draft |
| `audio.setDanteConfig` | command | 设置 audio.dante 配置 | `SetDanteConfigParams` | `SetDanteConfigResult` | 是，`audio.danteConfigChanged` | draft |
| `audio.resetDanteConfig` | action | 恢复 audio.dante 默认配置 | `ResetDanteConfigParams` | `ResetDanteConfigResult` | 是，`audio.danteConfigChanged` | draft |

### 3.1 `audio.getDanteCapabilities`

**用途**：查询 audio.dante 能力范围。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetDanteCapabilitiesParams` |
| Result Schema | `GetDanteCapabilitiesResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.1.1 请求参数 Params：`GetDanteCapabilitiesParams`
...
#### 3.1.3 返回结果 Result：`GetDanteCapabilitiesResult`
...
### 3.2 `audio.getDanteConfig`

**用途**：查询 audio.dante 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | query |
| Params Schema | `GetDanteConfigParams` |
| Result Schema | `GetDanteConfigResult` |
| 是否触发事件 | 否 |
| 幂等性 / 异步性 | 幂等；同步返回当前快照。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAVAILABLE` |

#### 3.2.1 请求参数 Params：`GetDanteConfigParams`
...
#### 3.2.3 返回结果 Result：`GetDanteConfigResult`
...
### 3.3 `audio.setDanteConfig`

**用途**：设置 audio.dante 配置。

| 项 | 内容 |
|---|---|
| 调用类型 | command |
| Params Schema | `SetDanteConfigParams` |
| Result Schema | `SetDanteConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `audio.danteConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.3.1 请求参数 Params：`SetDanteConfigParams`
...
#### 3.3.3 返回结果 Result：`SetDanteConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `audio.danteConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `DanteConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |

```json
{
  "event": "audio.danteConfigChanged",
  "intent": 1,
  "data": {
    "changedFields": [
      "state"
...
### 3.4 `audio.resetDanteConfig`

**用途**：恢复 audio.dante 默认配置。

| 项 | 内容 |
|---|---|
| 调用类型 | action |
| Params Schema | `ResetDanteConfigParams` |
| Result Schema | `ResetDanteConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `audio.danteConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`ResetDanteConfigParams`
...
#### 3.4.3 返回结果 Result：`ResetDanteConfigResult`
...
|---|---|---|---|---|
| `audio.danteConfigChanged` | audio.dante 配置变化 | `DanteConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |

### 4.1 `audio.danteConfigChanged`

**触发条件**：audio.dante 配置变化。

#### 4.1.1 Payload：`DanteConfigChangedEvent`
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `audio.dante` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetDanteCapabilitiesParams` | `audio.getDanteCapabilities` request params | 见 `audio.getDanteCapabilities` 方法小节。 |
| `GetDanteCapabilitiesResult` | `audio.getDanteCapabilities` result | 见 `audio.getDanteCapabilities` 方法小节。 |
| `GetDanteConfigParams` | `audio.getDanteConfig` request params | 见 `audio.getDanteConfig` 方法小节。 |
| `GetDanteConfigResult` | `audio.getDanteConfig` result | 见 `audio.getDanteConfig` 方法小节。 |
| `SetDanteConfigParams` | `audio.setDanteConfig` request params | 见 `audio.setDanteConfig` 方法小节。 |
| `SetDanteConfigResult` | `audio.setDanteConfig` result | 见 `audio.setDanteConfig` 方法小节。 |
| `ResetDanteConfigParams` | `audio.resetDanteConfig` request params | 见 `audio.resetDanteConfig` 方法小节。 |
| `ResetDanteConfigResult` | `audio.resetDanteConfig` result | 见 `audio.resetDanteConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `DanteConfigChangedEvent` | `audio.danteConfigChanged` | 见 `audio.danteConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `DanteState` | 表达 `audio.dante` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `DanteConfig` | 表达 `audio.dante` 的可写配置。 | `[REVIEW-ASK]` |
...
Legacy 映射是迁移证据，不是 runtime 合同。当前映射仍需从 `docs/workspace/legacy-migration/classification/` 和旧协议证据中按 `audio.dante` 人工确认。

| legacy 项 | 候选映射 | 状态 | 说明 |
|---|---|---|---|
| AXDP / Rooms / VM33 / Signage legacy command or field | `audio.dante` | `[REVIEW-ASK]` | 采纳前补齐确定的旧协议命令、字段路径、状态码和覆盖范围。 |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `audio.dante` 的 MVP 字段范围是否完整？ | schema / conformance | 进入 registry review 前由产品、设备实现和测试共同确认。 | open |
