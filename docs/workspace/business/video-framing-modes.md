...
本需求只描述用户和产品期望，不作为协议实现合同。后续 flow 会把场景拆成 AXTP 交互步骤，协议草案仍由 `docs/workspace/protocol/video/video.framing.md` 等文档承接。
...
- 交互和协议覆盖由 `docs/workspace/flows/video-framing-modes.md` 承接。
