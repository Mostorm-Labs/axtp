...
|---|---|---|
| Rough product, architecture, customer, legacy, or UI idea not ready for flow/draft | Use `docs/dev/skills/00-business-intake/SKILL.md` | `docs/business/**` only |
...
| Concrete missing capability, method, event, schema, error, profile, or feature semantics need reviewable protocol design | Use `docs/dev/skills/20-draft-business-protocol/SKILL.md` | `docs/protocol/**` only |
...
| Reviewed draft should become formal protocol | Use `docs/dev/skills/30-adopt-protocol-draft/SKILL.md` | `docs/protocol/**`, needed specs, `registry/**`, `registry/domains/**` |
| Already-adopted/generated protocol needs semantic correction, field removal, deprecation, rename, or extension | Use `docs/dev/skills/40-amend-adopted-protocol/SKILL.md` | adopted proposal, needed specs/YAML, generated artifacts via Generator |
...
| Verified spec needs tag/release artifact/runtime notification | Use `docs/dev/skills/60-release-axtp-spec/SKILL.md` | release metadata, tag, artifact, dispatch |
...
```text
business-intake
  rough requirement / product idea -> docs/business/<topic>.md
...
draft-business-protocol
  concrete protocol gap -> docs/protocol/<domain>/<domain.feature>.md
...
- Do not skip `docs/business/**` when the request is still product intent rather than protocol semantics.
...
- New business features default to business intake, flow, or protocol draft first, then `registry/domains/<domain>/domain.yaml` after adoption.
