...
AXTP v1 不要求独立 `rpcVersion` 协商。`axtpVersion` 已覆盖 RPC session envelope、Hello/Identify/Identified、Request/Response/Event、encoding registry 和 JSON_BINARY header 的兼容判断；registry/version.yaml 的 `wire_version` 负责 release artifact 层面的 wire compatibility 追踪。
