...
**方式 B：领域方法隐式建流**（适用于 OTA、文件、视频等；具体 method 必须来自已采纳业务草案）
...
| --- |---| --- |---|
| `firmware.ota` | `byteOffset` | `reliable` | draft，待 `firmware.ota` 草案采纳 |
...
建议：任何已采纳的 OTA/File 类业务都应在建流时根据 MTU 协商 chunkSize，避免频繁触发 Frame 分片。
...
| --- |---| --- |---| --- |
| OTA | 待 `firmware.ota` 草案采纳 | 建流 response | chunkIndex | byteOffset |
...
业务对象信息不应写入 STREAM Header。OTA 类业务的 imageType/totalSize/sha256/chunkCrc32 等信息必须由已采纳 RPC 建流方法或 Stream Context 协商。
...
- 对象级校验（业务层）：OTA/File 类业务通过已采纳建流方法的 `verifyType` + `verifyValue` 字段声明，传输完成后由已采纳校验/完成方法触发端到端校验
...
| --- | --- | --- |
| `md5` | 32 位小写十六进制字符串 | 计算整个传输对象；生产 OTA 推荐 sha256 |
...
| OTA `totalLength` | 已采纳 OTA 建流 params 的 `totalSize` |
