...
8. 新增业务专属 error 默认进入 `registry/domains/<domain>/domain.yaml`；跨 domain、Core/MVP 或协议层错误进入核心 error registry。
...
本文档不列出完整 ErrorCode 规划表。历史 planning/current 表已移至 [appendix/error-candidates.md](appendix/error-candidates.md)，仅用于审计和迁移参考。正式实现 MUST 以 `registry/**/*.yaml`、`protocol/axtp.protocol.yaml` 和 `docs/generated/**` 为准。
