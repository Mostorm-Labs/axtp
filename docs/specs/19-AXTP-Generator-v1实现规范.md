...
Generator 流程外侧使用一个总控 skill 和四个阶段 skill 辅助治理，各阶段写入边界不同：
...
11. 如果已采纳或已生成协议后续需要语义修正，运行 `amend-adopted-protocol` skill，先更新 adopted proposal/specs/YAML，再重新运行 `generate-axtp-protocol`。
12. 研发根据 generated 产物开发和上架 feature。
13. 更新或新增测试。
```
