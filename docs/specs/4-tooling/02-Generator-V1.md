# 4-tooling/02《AXTP Generator v1 实现规范》
...
- `docs/specs/3-codec/01-Type-System.md`
- `docs/specs/3-codec/03-TLV-Encoding.md`
...
Generator v1 的最终机器输入仍是 `registry/**/*.yaml` 和 `registry/domains/**/*.yaml`。协议生成治理链路必须先把 `docs/protocol/<domain>/<domain.feature>.md` 作为协议方案输入：更新协议草案并人工确认后，反向确认 Registry/Capability Types specs，再固化到 YAML 并生成产物。实现者不得手写生成产物来修协议事实。
...
| 调整 wire format 说明 | `docs/specs/1-core/03-Frame-and-Payload.md`, `docs/specs/1-core/05-Control-Session.md`, `docs/specs/1-core/06-RPC-Session.md`, `docs/specs/1-core/07-Stream-Data-Plane.md` | 不由 Generator 直接定义 |
...
Registry and Capability Types specs reverse confirmation
        ↓ update source facts
registry/**/*.yaml + registry/domains/**/*.yaml
        ↓ validate-sources
Source Model
...
| 阶段 | 输入 | 输出 | 责任 |
| ---- | ---- | ---- | ---- |
| 协议方案段 | `docs/protocol/<domain>/<domain.feature>.md`、legacy classification、Registry/Capability Types specs 规则 | 已确认协议草案、Registry/Capability Types specs 反向确认项、YAML 修改计划 | 确认 domain.feature、method/event/schema/error/capability/profile 是否成立 |
...
- 所有 `docs/protocol/<domain>/<domain.feature>.md` 草案都进入方案输入清单；`docs/legacy-migration/classification` 只作为迁移证据，不直接成为 registry 事实。
- 协议草案确认后，必须反向确认 Registry/Capability Types specs：Naming and Taxonomy spec 命名归属，YAML Mapping spec 的 ID/Domain/生成链路，Methods Registry spec，Events Registry spec，Errors Registry spec，Capability Types spec。
- Source YAML 可以有多个输入文件，但 build 后必须合并为一个 Protocol IR。
- Protocol IR 可以生成多个产物，但不得反向手写修改。
- 修改 generated 文档发现的问题时，必须回到协议草案、Source YAML 或 `1-core/` through `4-tooling/` spec，而不是直接改 generated 文件。
...
| `adopt-protocol-draft` | 内部评审确认草案后 | 反向确认 Registry/Capability Types specs、固定草案为正式方案、检查 YAML 冲突、写入 registry/domain YAML | 不采纳 `[REVIEW-ASK]` / `[REVIEW-BLOCKER]`，不手写 generated |
...
`protocol/axtp.protocol.yaml` 的结构化内容来自 Source YAML，但关键 wire 事实必须与 `core/registry/codec specs` 保持一致。当前必须校验：
...
`docs/generated/protocol.md` 面向 SDK 使用者和业务接入者，不承担低层 frame/transport/payload 规范全文复制职责。低层细节留在 `core/registry/codec specs`。
...
4. 读取 `docs/specs/2-registry/01-Naming-and-Taxonomy.md`、Registry/Capability Types/Profiles specs、`docs/specs/4-tooling/01-YAML-Mapping.md` 和 `docs/specs/4-tooling/02-Generator-V1.md`。
...
7. 运行 `adopt-protocol-draft` skill，反向确认 Registry and Capability Types specs：
   - Naming and Taxonomy spec：domain.feature 归属、命名模板、feature 粒度
   - YAML Mapping spec：Domain/ID 规划、Protocol Definition 映射、生成链路
   - Methods Registry spec：methodId、bitOffset、request/response schema
   - Events Registry spec：eventId、eventMasks bitOffset、event schema
   - Errors Registry spec：errorCode 范围、RPC/CONTROL/STREAM 使用位置
   - Capability Types spec：schema fieldId、capabilityId、supportedMethods 关系
...
- 不得从 `docs/protocol` 直接生成最终 `protocol/axtp.protocol.yaml`；必须经过草案确认、Registry/Capability Types specs 反向确认和 YAML 固化。
