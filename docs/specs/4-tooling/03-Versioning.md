...
本文档覆盖 spec version、registry version、schema/wire version、spec tag、release artifact、runtime 绑定、兼容变更、breaking change、deprecated/reserved 和 generated artifact version lock。本文档不承载 legacy adapter 的完整迁移设计；legacy 迁移细节应进入 `docs/legacy-migration/**`。

## 规范规则

1. Runtime 仓库 MUST 绑定 spec tag、明确 commit 或 release artifact；不得依赖浮动 main。
2. `registry/version.yaml` MUST 记录当前 spec、registry、schema、wire 等版本元数据。
3. `protocol/axtp.protocol.yaml` 和 `docs/generated/**` MUST 与同一 source registry 版本生成。
...
registry/version.yaml
  ...
  -> docs/generated/protocol.md/json
...
- Maintainers MUST 在合并 registry/source 变更时检查 version impact。
...
Legacy protocol sniffing、legacy CmdValue 到 AXTP method 的迁移、legacy status code 映射、legacy adapter C++ skeleton 和迁移阶段计划均为非规范迁移内容，应在 `docs/legacy-migration/**` 中维护。本文只规定这些迁移不得污染 AXTP Core wire format、registry 稳定性和 generated implementation contract。
