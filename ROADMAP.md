...
Runtime, SDK, CLI, mock server and adapter implementation delivery happens in external runtime / tool repositories. Runtime implementations must follow released artifacts, protocol/axtp.protocol.yaml, docs/generated/**, docs/specs/** and docs/conformance/**.

AXTP 的目标不是“继续写更多协议文档”，而是把协议库建设成一套业务能真正接入、迁移、验证、发布的工程体系。

## 当前进度与强约束

当前 generated/adopted 状态以 [Product Domain Status](docs/product/domain-status.md) 为准。只有 `registry/domains/<domain>/domain.yaml` 存在，并且 Generator validation 通过，才算进入 runtime 可依赖的 generated 合同；其余 `docs/protocol/**` 内容仍是草案或评审输入。
...
主库聚焦协议语义、治理规则、registry 事实源、generated reference、release artifact、产品状态看板和 conformance cases。Runtime、SDK、CLI、mock server、adapter 和语言专属实现维护在独立 runtime / tool 仓库中。
