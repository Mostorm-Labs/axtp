...
打开 `12-AXTP-Capability注册表-v2.md`，新增 `audio.volume` 能力条目。v1 Core 客户端先通过 `capability.supportedMethods` 判断 `audio.setVolume` 是否可调用；完整能力详情可由 v2/P1 的 `capability.getAll` / `capability.getDomain` 发现。
...
如果新业务有特殊的调用流程（比如需要先建流），在 `03-AXTP-Transport-Profiles.md` 中补充示例。
