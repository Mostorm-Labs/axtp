...
该包应包含机器可读 spec 材料，例如 `contract/registry/`、schemas、generated TypeScript metadata 和 test/conformance assets。Runtime package 可以声明：
...
不要把 AXTP Spec version 隐藏进 Dart package version。应使用 `AXTP_SPEC.lock.yaml` 或项目内的 `axtp_spec.yaml`：
...
该 package 可以包含 generated Dart types、schema metadata、capability registry data 和 conformance fixtures。
