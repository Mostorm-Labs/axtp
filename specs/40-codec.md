...
本文定义 AXTP 数据类型、schema 模型、capability 模型、TLV 编码和 schema field numbering。当前 schema facts 位于 `contract/registry/**` 和生成产物中。

## 类型系统

All AXTP multi-byte wire integers MUST use Big-Endian / network byte order.

| Type | JSON 表达 | Binary/TLV 表达 | 规则 |
|---|---|---|---|
| `bool` | boolean | 1B | `0x00=false`，`0x01=true`；其他值 SHOULD fail validation。 |
| `uint8/16/32/64` | number；大值可用 string | fixed-width unsigned BE | 范围 MUST 匹配 type width 和 schema constraints。 |
| `int8/16/32/64` | number；大值可用 string | fixed-width signed BE | Two's-complement signed integers。 |
...
| `bitmap` | array/string/numeric profile-specific | bitmap8/16/32/64/bytes | bit0 是 least significant bit。 |
| `array<T>` | JSON array | repeated or packed | Element type MUST 已声明。 |
...
JSON、TLV 和生成 SDK 视图是同一个 schema fact 的不同投影。它们 MUST 保留相同的 required/optional 状态、default 语义、enum values、array item type、range、unit 和 deprecation metadata。无法在声明编码之间一致表达的字段 MUST 被 validation 拒绝，或标记为 profile-specific。

## Schema 模型

Object schema fields MUST 拥有 schema-local `fieldId`。Field ids 不是全局 ID。
...
1. Required fields MUST present。
...
7. 生成 SDKs SHOULD 区分 wire types 和 host-language types。
...
2. Receivers MUST 在读取 value bytes 前校验 length。
...
| 变更 | 默认兼容性 |
|---|---|
| Add optional field | 兼容。 |
| Add required field | 通常 breaking。 |
| Add enum/bitmap value | 只有定义 unknown-value policy 时兼容。 |
| Change type/unit/range/requiredness | 通常 breaking。 |
| Reuse removed field id | 对 stable contracts 禁止。 |

Runtime 或 generated codecs MUST 校验 numeric ranges、UTF-8 strings、enum/bitmap values、required fields、object recursion、array element types 和 length boundaries。

Unknown-field policy：

- Unknown TLV fields 只要 length 有效，MUST 被 forward-compatible decoders 跳过。
...
- Deprecated fields MAY 为兼容性发出，但新 senders SHOULD NOT 依赖它们。
