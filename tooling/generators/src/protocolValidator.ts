...
function fail(entry: string, field: string, message: string): never {
  throw new GeneratorError({
    code: "AXTP-GEN-1004",
    file: "contract/protocol/axtp.protocol.yaml",
    entry,
    field,
    message
  });
...
function assertNoUnsupportedProfileKeys(value: unknown): void {
  ...
  profiles.forEach((profile, index) => {
    if (profile && typeof profile === "object" && "requiredCapabilities" in profile) {
      fail(`profiles[${index}]`, "requiredCapabilities", "profiles[].requiredCapabilities is not defined by specs/30-registry.md");
    }
  });
}
...
function assertMethodReferences(methods: MethodDefinition[], typeNames: Set<string>, eventNames: Set<string>, errorNames: Set<string>, capabilityNames: Set<string>): void {
  for (const method of methods) {
    ...
    if (!method.description || method.description.trim() === "") fail(method.name, "description", "method description is required by specs/30-registry.md");
...
function assertSchemaDefinitions(schemas: SchemaDefinition[]): void {
  ...
  const builtins = new Set(["bool", "uint8", "uint16", "uint32", "uint64", "int8", "int16", "int32", "int64", "number", "string", "bytes", "enum", "bitmap", "array", "object"]);
  const schemaNames = new Set(schemas.map((schema) => schema.name));
  const objectSchemaNames = new Set(schemas.filter((schema) => schema.kind === "object").map((schema) => schema.name));
  for (const schema of schemas) {
    ...
    for (const field of schema.fields) {
      ...
      if (field.variants) {
        ...
        if (!discriminator) {
          fail(schema.name, "variants.discriminator", `field ${field.name} variants.discriminator must reference a sibling field in the same schema: ${variants.discriminator}`);
        } else if (discriminator.type !== "enum" || !discriminator.enumValues?.length) {
          fail(schema.name, "variants.discriminator", `field ${field.name} variants.discriminator must reference an enum field with declared enum values: ${discriminator.name}`);
        } else {
          const declared = new Set(discriminator.enumValues);
          for (const key of Object.keys(variants.mapping)) {
            if (!declared.has(key)) {
              fail(schema.name, "variants.mapping", `field ${field.name} variants.mapping key is not a declared enum value of ${discriminator.name}: ${key}`);
            }
          }
          for (const value of discriminator.enumValues) {
            if (!(value in variants.mapping)) {
              fail(schema.name, "variants.mapping", `field ${field.name} variants.mapping must cover all enum values of ${discriminator.name}: missing ${value}`);
            }
          }
          for (const value of Object.values(variants.mapping)) {
            if (!schemaNames.has(value)) {
              fail(schema.name, "variants.mapping", `field ${field.name} variants.mapping references missing schema: ${value}`);
            } else if (!objectSchemaNames.has(value)) {
              fail(schema.name, "variants.mapping", `field ${field.name} variants.mapping references non-object schema: ${value}`);
            }
          }
        }
      }
...
function assertEmptySchemaUsage(model: ProtocolModel): void {
  ...
  if (!empty || empty.kind !== "object" || empty.fields.length !== 0) {
    fail("schemas.Empty", "schema", "specs/30-registry.md requires empty request/response to use Empty");
  }
...
function assertControlOpcodes(model: ProtocolModel): void {
  for (const opcode of ["OPEN", "ACCEPT", "HEARTBEAT", "HEARTBEAT_ACK", "CLOSE", "CLOSE_ACK"]) {
    if (!model.control.requiredOpcodes.includes(opcode)) fail("control.requiredOpcodes", opcode, `${opcode} is required by specs/20-core.md`);
  }
  ...
  for (const opcode of ["ACK", "NACK", "RESUME", "HEARTBEAT", "HEARTBEAT_ACK", "CLOSE", "CLOSE_ACK"]) {
    if (model.control.reservedOpcodes.includes(opcode)) fail("control.reservedOpcodes", opcode, `${opcode} is defined by specs/20-core.md and must not be listed as reserved`);
  }
}
...
export function validateProtocolDefinition(model: ProtocolModel): string[] {
  ...
  return [
    `[OK] contract/protocol/axtp.protocol.yaml: ${model.methods.length} methods checked`,
    `[OK] contract/protocol/axtp.protocol.yaml: ${model.events.length} events checked`,
    `[OK] contract/protocol/axtp.protocol.yaml: ${model.errors.length} errors checked`,
    `[OK] contract/protocol/axtp.protocol.yaml: ${model.capabilities.length} capabilities checked`,
    `[OK] contract/protocol/axtp.protocol.yaml: ${model.schemas.length} schemas checked`,
    `[OK] contract/protocol/axtp.protocol.yaml: ${model.profiles.length} profiles checked`
  ];
}
