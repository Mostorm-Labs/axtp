...
function assertNoUnsupportedProfileKeys(value: unknown): void {
  ...
  profiles.forEach((profile, index) => {
    if (profile && typeof profile === "object" && "requiredCapabilities" in profile) {
      fail(`profiles[${index}]`, "requiredCapabilities", "profiles[].requiredCapabilities is not defined by specs/2-registry/05-Profiles-Registry.md");
    }
  });
}
...
function assertMethodReferences(methods: MethodDefinition[], typeNames: Set<string>, eventNames: Set<string>, errorNames: Set<string>, capabilityNames: Set<string>): void {
  for (const method of methods) {
    ...
    if (!method.description || method.description.trim() === "") fail(method.name, "description", "method description is required by specs/2-registry/02-Methods-Registry.md");
...
function assertEmptySchemaUsage(model: ProtocolModel): void {
  ...
  if (!empty || empty.kind !== "object" || empty.fields.length !== 0) {
    fail("schemas.Empty", "schema", "specs/2-registry/02-Methods-Registry.md requires empty request/response to use Empty");
  }
...
function assertControlOpcodes(model: ProtocolModel): void {
  for (const opcode of ["OPEN", "ACCEPT", "HEARTBEAT", "HEARTBEAT_ACK", "CLOSE", "CLOSE_ACK"]) {
    if (!model.control.requiredOpcodes.includes(opcode)) fail("control.requiredOpcodes", opcode, `${opcode} is required by specs/1-core/05-Control-Session.md`);
  }
  ...
  for (const opcode of ["ACK", "NACK", "RESUME", "HEARTBEAT", "HEARTBEAT_ACK", "CLOSE", "CLOSE_ACK"]) {
    if (model.control.reservedOpcodes.includes(opcode)) fail("control.reservedOpcodes", opcode, `${opcode} is defined by specs/1-core/05-Control-Session.md and must not be listed as reserved`);
  }
}
