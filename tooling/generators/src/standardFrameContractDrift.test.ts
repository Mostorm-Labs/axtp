...
const driftCases: Array<{ name: string; mutate: (contract: any) => void }> = [
  ...
  { name: "duplicate diagnostic", mutate: (contract) => { contract.fragmentation.duplicate.diagnostic = "RESOURCE_EXHAUSTED"; } },
...
beforeAll(async () => {
  baseModel = await loadProtocolDefinition(repoRoot);
});
