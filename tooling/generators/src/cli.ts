...
function defaultProtocolOut(specPath: string): string {
  return path.join(resolveInputPath(specPath), "contract", "generated");
}
...
program.command("generate-registry")
  .description("generate registry C++, Markdown, JSON and test vectors from contract/registry/domain sources")
  .requiredOption("--spec <path>", "AXTP spec root")
...
program.command("validate-sources")
  .description("validate contract/registry/domain YAML sources and generated Protocol IR")
...
program.command("emit-protocol")
  .description("emit protocol JSON and Markdown from contract/protocol/axtp.protocol.yaml")
  .requiredOption("--spec <path>", "AXTP repository root")
  .option("--out <path>", "output directory")
  .action(async (options) => {
    try {
      const { model, messages } = await loadAndValidateProtocol(options.spec);
      const out = options.out ? resolveOutputPath(options.out) : path.join(resolveInputPath(options.spec), "contract", "generated");
...
program.command("test-vector")
  ...
  .action(async (options) => {
    try {
      const { spec } = await loadAndValidate(options.spec);
      const out = options.out ? resolveOutputPath(options.out) : path.join(resolveInputPath(options.spec), "contract", "test-vectors");
...
program.command("diff")
  .description("compare two contract/registry/schema versions (P1 placeholder)")
