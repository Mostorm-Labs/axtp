...
function mapField(item: any, file: string, schemaName: string): Field {
  ...
  const variants = item.variants === undefined ? undefined : {
    discriminator: String(item.variants.discriminator),
    mapping: Object.fromEntries(Object.entries(item.variants.mapping ?? {}).map(([key, value]) => [String(key), String(value)]))
  };
  return {
...
export async function loadProtocolSources(specRoot: string): Promise<ProtocolSourceModel> {
  ...
  if (legacyDomainFiles.length > 0) {
    throw new GeneratorError({
      code: "AXTP-GEN-1004",
      file: "domains/",
      message: `top-level domains/ is deprecated; move YAML files to contract/registry/domains/. Found: ${legacyDomainFiles.map((file) => path.relative(specRoot, file)).join(", ")}`
    });
  }
