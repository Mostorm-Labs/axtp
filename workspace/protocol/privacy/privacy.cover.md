...
|---|---|---:|---|---|---|
| `target` | string | no | target id | `default` | 示例值 `camera-cover`；查询对象。 |

#### 3.1.2 返回结果 Result：`GetCoverCapabilitiesResult`

| 字段名 | 类型 | 必填 | 取值范围 / 枚举 | 默认值 | 说明 |
|---|---|---:|---|---|---|
| `state` | object | yes | see schema | none | 当前结果对象；示例字段包括 `target`、`positions`、`localOverrideSupported`、`policyLockSupported`。 |
...
| 字段名 | 类型 | 必填 | 取值范围 / 枚举 | 默认值 | 说明 |
|---|---|---:|---|---|---|
| `state` | object | yes | see schema | none | 当前结果对象；示例字段包括 `target`、`position`、`policyLocked`。 |
| `sampledAt` | string timestamp | no | RFC 3339 | omitted | 结果采样时间；客户端可用于缓存和校准。 |
