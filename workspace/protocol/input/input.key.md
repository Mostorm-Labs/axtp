...
|---|---|---:|---|---|---|
| `target` | string | no | target id | `default` | 示例值 `remote-control`；查询对象。 |

#### 3.1.2 返回结果 Result：`GetKeyCapabilitiesResult`

| 字段名 | 类型 | 必填 | 取值范围 / 枚举 | 默认值 | 说明 |
|---|---|---:|---|---|---|
| `state` | object | yes | see schema | none | 当前结果对象；示例字段包括 `target`、`keyIds`、`repeatSupported`、`remappable`。 |
...
#### 3.2.2 返回结果 Result：`GetKeyConfigResult`

| 字段名 | 类型 | 必填 | 取值范围 / 枚举 | 默认值 | 说明 |
|---|---|---:|---|---|---|
| `state` | object | yes | see schema | none | 当前结果对象；示例字段包括 `target`、`mappings`。 |
| `sampledAt` | string timestamp | no | RFC 3339 | omitted | 结果采样时间；客户端可用于缓存和校准。 |
...
#### 3.3.2 返回结果 Result：`SetKeyConfigResult`
...
#### 3.4.2 返回结果 Result：`ResetKeyConfigResult`
