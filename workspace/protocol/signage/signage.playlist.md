...
| 项目 | 内容 |
|---|---|
| 这个能力做什么 | 播放列表、播放项、列表状态和播放项资源 URL 刷新。 |
| 当前状态 | draft |
...
`signage.playlist` 用于播放列表、播放项、列表状态和播放项资源 URL 刷新。

## 2. 能力边界

| 类型 | 内容 |
|---|---|
| 包含 | signage.playlist 的能力发现、配置、状态、动作或事件。 |
| 包含 | 与 signage.playlist 直接相关的 method/event/schema 草案。 |
| 包含 | 播放项资源 URL 刷新（原 signage.media 已合并）。 |
| 包含 | 已确认 legacy 协议到 signage.playlist 的语义归类。 |
...
|---|---|---|---|---|---|---|
| `signage.getPlaylistItemUrl` | query | 查询播放列表条目的可访问 URL | `GetPlaylistItemUrlParams` | `GetPlaylistItemUrlResult` | 否 | draft |
| `signage.getPlaylistCapabilities` | query | 查询 signage.playlist 能力范围 | `PlaylistCapabilitiesParams` | `PlaylistCapabilitiesResult` | 否 | draft |
| `signage.getPlaylistConfig` | query | 查询当前播放列表配置 | `GetPlaylistConfigParams` | `PlaylistConfigResult` | 否 | draft |
| `signage.setPlaylistConfig` | command | 全量同步播放列表配置（替换语义） | `SetPlaylistConfigParams` | `SetPlaylistConfigResult` | 是，`signage.playlistConfigChanged` | draft |
| `signage.resetPlaylistConfig` | action | 恢复默认播放列表配置 | `ResetPlaylistConfigParams` | `SetPlaylistConfigResult` | 是，`signage.playlistConfigChanged` | draft |

### 3.1 `signage.getPlaylistItemUrl`
...
#### 3.1.1 请求参数 Params：`GetPlaylistItemUrlParams`
...
#### 3.1.2 返回结果 Result：`GetPlaylistItemUrlResult`
...
### 3.2 `signage.getPlaylistCapabilities`

**用途**：查询 signage.playlist 能力范围。
...
#### 3.2.1 请求参数 Params：`PlaylistCapabilitiesParams`
...
#### 3.2.2 返回结果 Result：`PlaylistCapabilitiesResult`
...
#### 3.3.1 请求参数 Params：`GetPlaylistConfigParams`
...
#### 3.3.2 返回结果 Result：`PlaylistConfigResult`
...
| Result Schema | `SetPlaylistConfigResult` |
| 是否触发事件 | 是，状态实际变化后触发 `signage.playlistConfigChanged`。 |
| 幂等性 / 异步性 | 建议幂等；重复提交相同目标状态应成功，可不重复触发事件。 |
| 常见错误 | `NOT_SUPPORTED`, `INVALID_ARGUMENT`, `INVALID_STATE`, `BUSY`, `PERMISSION_DENIED` |

#### 3.4.1 请求参数 Params：`SetPlaylistConfigParams`
...
#### 3.4.2 返回结果 Result：`SetPlaylistConfigResult`
...
| Event | 触发条件 | Payload Schema | 客户端处理建议 |
|---|---|---|---|
| `signage.playlistConfigChanged` | 该方法导致状态、配置或动作状态实际变化。 | `PlaylistConfigChangedEvent` | 可直接更新 UI；需要完整状态时调用对应 get method 校准。 |
...
### 3.5 `signage.resetPlaylistConfig`
...
#### 3.5.1 请求参数 Params：`ResetPlaylistConfigParams`
...
#### 3.5.2 返回结果 Result：`SetPlaylistConfigResult`
...
|---|---|---|---|---|
| `signage.playlistConfigChanged` | 播放列表配置发生变更时发出 | `PlaylistConfigChangedEvent` | 更新 UI 或调用对应 get method 校准 | draft |
...
|---|---|---:|---|---|---|
| `capability` | string | yes | fixed `signage.playlist` | none | capability 名称。 |
...
| Schema | 用途 | 字段定义 |
|---|---|---|
| `GetPlaylistItemUrlParams` | `signage.getPlaylistItemUrl` request params | 见 `signage.getPlaylistItemUrl` 方法小节。 |
| `GetPlaylistItemUrlResult` | `signage.getPlaylistItemUrl` result | 见 `signage.getPlaylistItemUrl` 方法小节。 |
| `PlaylistCapabilitiesParams` | `signage.getPlaylistCapabilities` request params | 见 `signage.getPlaylistCapabilities` 方法小节。 |
| `PlaylistCapabilitiesResult` | `signage.getPlaylistCapabilities` result | 见 `signage.getPlaylistCapabilities` 方法小节。 |
| `GetPlaylistConfigParams` | `signage.getPlaylistConfig` request params | 见 `signage.getPlaylistConfig` 方法小节。 |
| `PlaylistConfigResult` | `signage.getPlaylistConfig` result | 见 `signage.getPlaylistConfig` 方法小节。 |
| `SetPlaylistConfigParams` | `signage.setPlaylistConfig` request params | 见 `signage.setPlaylistConfig` 方法小节。 |
| `SetPlaylistConfigResult` | `signage.setPlaylistConfig` result | 见 `signage.setPlaylistConfig` 方法小节。 |
| `ResetPlaylistConfigParams` | `signage.resetPlaylistConfig` request params | 见 `signage.resetPlaylistConfig` 方法小节。 |
| `SetPlaylistConfigResult` | `signage.resetPlaylistConfig` result | 见 `signage.resetPlaylistConfig` 方法小节。 |
...
| Schema | Event | 字段定义 |
|---|---|---|
| `PlaylistConfigChangedEvent` | `signage.playlistConfigChanged` | 见 `signage.playlistConfigChanged` 事件小节。 |

### 6.5 State / Config / Object Schemas

| Schema | 用途 | 状态 |
|---|---|---|
| `PlaylistState` | 表达 `signage.playlist` 的当前状态、配置或摘要。 | `[REVIEW-ASK]` |
| `PlaylistConfig` | 表达 `signage.playlist` 的可写配置。 | `[REVIEW-ASK]` |
...
| 问题 | 影响 | 当前建议 | 状态 |
|---|---|---|---|
| `signage.playlist` 采纳前还需确认哪些 schema、事件和 conformance 细节？ | schema / conformance | 按本文 method/event 示例逐项确认字段、边界错误和测试用例；确认后再进入 registry review。 | open |
