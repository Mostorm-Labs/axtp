...
#### 3.1.2 返回结果 Result：`GetColorCapabilitiesResult`

| 字段名 | 类型 | 必填 | 取值范围 / 枚举 | 默认值 | 说明 |
|---|---|---:|---|---|---|
| `state` | object | yes | see schema | none | 当前结果对象；示例字段包括 `target`、`presets`、`colorTemperatureKelvinRange`。 |
...
#### 3.2.2 返回结果 Result：`GetColorConfigResult`

| 字段名 | 类型 | 必填 | 取值范围 / 枚举 | 默认值 | 说明 |
|---|---|---:|---|---|---|
| `state` | object | yes | see schema | none | 当前结果对象；示例字段包括 `target`、`preset`、`colorTemperatureKelvin`、`saturation`。 |
| `sampledAt` | string timestamp | no | RFC 3339 | omitted | 结果采样时间；客户端可用于缓存和校准。 |
...
#### 3.3.2 返回结果 Result：`SetColorConfigResult`
...
#### 3.4.2 返回结果 Result：`ResetColorConfigResult`
...
```json
{
  ...
  "result": {
    "accepted": true,
    "actionId": "display-resetcolorconfig-20260615-001"
  }
}
```
